...
* ***mein*** → `;N`
 * : *ich*.**b**in.~~der~~.***mein*** → `I>bi.N`
 * *ge****mein*** → `G:N` → `G.N` E
 * *ge****mein***de → `G:Nd.e` → `;Nd.e`
 * *all**ge****mein*** → `.al.G.N` → `.al.G` E
* ***mat*** → `.at`
 * : auto***mat*** → `.aut.at` E
 ...
 * beschäftig → `b.e-sch.eft-ig` !
    * *beschäftig* → `bet-ig` E
 * -*schaft*- → `FT` V
    * **wirt***schaft* → `wit-FT` → `FT` ≠ **wiss**en*schaft*
 ...
 * **v**ent → `w`
    * : e**v**entualität → `.e-w.al.ät` → `.e-v.ät`
    * : evtl. → `.e-v-t-l`
    * : *in***v**entar → `.I.w.ar` → `.I-t-ar`
    * : *in***v**entur → `.I.w-UR` → `.I-t-UR`
    * : **kon****v**ent*ion* → `ko.w-o`
    * : *inter***v**ent*ion* → `I>t-er.w-o`
* -*ver*- → `E` V
* vert → `wet`
