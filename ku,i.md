...
* **ge**ometrie → `ge.m.etr.I`
* theater → `t.e-at-er`

Die Vokale *i* und *u* am Ende eines zweisilbigen lateinischen Wortbestandteils werden ausgestoßen.
...
Die Nachsilbe **ig** wird nach Aufstrichzeichen ausgeschrieben.
...
Zwischen zwei Wörtern wird abgesetzt.
* oder.**au**ch → `.od-er,au`

Ist eine Verschmelzung bei Inlautkürzung nicht möglich (oder wird eine solche zum Zwecke der Unterscheidung vermieden), so wird abgesetzt.
* -schr**i**ft- → `i`
 * : **kurz**schr**i**ft → `ku,i`
 * : *unter**schr****i****ft* → `UNTER,i`
 * : *vor*schr**i**ft → `O-i`
...
* ! firm → `f.im`
 * ! : **kon**firmat*ion* → `ko.f.im-O`

* ! -fiz-
 * ! : effizi*ent* → `.e-f.iz.eNT`
 * ! : effektiv → `.e-f.ekt.iv`
 * ! -fekt-
    * ! : **kon**fekt → `ko.f.ekt`
    * ! : ***p****er**fekt* → `p.fet`
 ...
 * : {famili}är → `f.am.är`
* -***f****er*- → `f`
 * : **kon*****f****er*enz → `ko.f.ez` !
 * : **kon*****f****er*ier → `ko.f.ir`
* -fat-
 * -**fess**- → `fess`
 ...
 * {univers} → `.un`
    ...
    * : {univers}ität → `.un.ät`
* **ven** → `w`
 * : {inter}***v****ent*ier → `I>t-er.w.ir`
 * ***v****ent* → `w`
    * : e***v****ent*ualität → `.e-w.al.ät` !
    * : evtl. → `.e-v-t-l`
    * : *in****v****ent*ar → `.I.w.ar` !
    * : *in****v****ent*ur → `.I.w-UR` !
    * : **kon*****v****ent**ion* → `ko.w-O`
