# Wörterbuch

| [A](#a) | [B](#b) | [C](#c) | [D](#d) | [E](#e) | [F](#f) | [G](#g) | [H](#h) | [I](#i) | [J](#j) | [K](#k) | [L](#l) | [M](#m) | [N](#n) | [O](#o) | [P](#p) | [Q](#q) | [R](#r) | [S](#s) | [T](#t) | [U](#u) | [V](#v) | [W](#w) | [X](#x) | [Y](#y) | [Z](#z) |
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|

## [A](#wörterbuch)

* ab `.ab` `.ab.a`
* *ab****end*** `(.ed)`
* *aber* `ber` `ber.ma-s` __`ber,au`__ `ber+r`
* acht `.acht` `.acht-Sa` `be.acht` `ho.acht-UNG` __`M.O.üg.L:ho.acht-UNG`__
  * -*acht* `.a` __`gu.a.en`__ __`gu.a.er`__
* *adress* `.adr` ← rekt
* aero `.er` __`.er.aut.ik`__
* *afrik* `.afr` `.afr.i`
* ag `.ag` `.ag.eNT` __`.ag.eNT.ur`__
  * → akt
* -age `.asch.e`
* **ak**adem `.ak` `.ak.I` `.ak.I.en` `.ak.ik-er` `.ak.i` `kust.ak.I`
* *akt* __`-t`__ __`-t.I.e`__ __`-t.I.en`__ `-t.iO` __`-t.iO.är`__ `-t.iv` `-t.iv.ir` `-t.al.ät` `-t.ell` `IN-t.iv` `IN-t.iv.ät` `r-t.iO.är` `r-t.iO` __`r-t.or`__ `w.eb-t.iO` ← ag
* akzent __`.ak.zet`__ ← kant
* -al `.al`
  * -*alien* `.I.en`
  * -il `.il`
    * -ilität `.ät`
    * -ilitier `.ir`
      * habilitier `h.ab.il.ir`
* *all* `.al` `wet.al` `O.al`
  * *vor.allem* `O-a`
* als `.as`
* also `.as.o`
* alt `.at` `.at-er` `.at-er.tum` `ki.at-er` `v.at-er.h`
  * ALT `.alt` `.ur.alt`
  * ält `.ät` `.ät-er` `.ät.est`
* *amerik* __`.am`__ `.am.er` `.am.i` __`I E.ig .at.en v .am`__
* amt `.amt` `.amt.ir`
  * *beamt*, *beamter*, *beamten* `be.a` `be.a.in` __`be.a.FT`__
  * -*amt* `.a` `Ts.a` → auswärtig finanz haupt land patent post
  * -*ämter* `.ä`
* an `.an` `.an,u` `.an-t-er` `dar.an` `wo.an`
* -an `.an`
  * -aner `.er`
  * -an*isch* `.i`
* ander `.ad-er` `N.ad-er`
  * *ande****rs*** `(rs)`
  * -*einander* `.ei-ad-er` → auf aus bei für gegen hinter in mit unter zu
  * *unter.anderem* __`u.a`__
  * änder `.äd-er` `.äd-erd` `.äd-ert.e` `.um.äd-er` `UN.ab.äd-er.L` `E.äd-er` `E.äd-er.L`
* angst `.angst`
  * -*angst* `(-st)` → tod
  * ängst `.engst`
* *ant*- `aNT`
* -*ant* `aNT`
* *ant*ik `aNT.ik`
* *arb****eit*** `(T)` `G.T` `(T).ed` `(T).er` `(T).er.in` `(T).er.FT` `(T).g` __`(T).Sa`__ `(T).N` `.S.T` `be.T` `fa.T.er` `ga.T.er` `M.T.er` `E.T` `O.T.er` `Z.Sa.T`
  * *arbeits****los*** `(l.os)`
  * *arbeits*- `Ts` `Ts.N` `Ts.is` → amt gebiet kreis markt platz __tag__ weis zeit
* arch `.ach` `.ach.iv` __`.ach.iv.ar`__
* archä `.ach` `.ach.og.e` `.ach.o-I`
* archi `.ach` `.ach.ekt` __`.ach.ekt.ur`__
* ARG `.arg` `.arg.os` `E.arg` ≠ mag schlag
  * → ärg
* arg `.ag` __`.ag.met`__
* ärg `.äg` `.äg-er` `.äg-er.L` `.äg-er.is` `E.äg-er` `.am.äg-st` ← arg
* arithmetik __`.ar.itm.ik`__
* *art* `.art`
  * -*art* `.a` → ab der wes
* art `.at` __`.at.er.I.e`__
  * *art****ik*** `(.ik)` `(.ik).el` `(.ik).ir` `(.ik).iO` `be.da.ik.el`
    * *artikel*- `(.el)` `(.el),6`
* arz `.az` `.az.n.ei` `za.azt`
  * ärz `.ez` `.ezt.e` `.ezt.L` `.ezt.e-FT`
* asi __`.as.I`__ __`.as.I.at`__ __`.as.I.i`__ __`.as.I.en`__
* astro `.a+str` __`.a+str.o-I`__ `.a+str.aut` `.a+str.om.I`
* athl `.atl` `.atl.et` `.atl.ik` `.atl.i`
* *atlant* `.atl` __`.atl.ik`__ `.atl.i` ← atlas
* atlas `.atl.as`
  * → atlant
* *at****om*** `(.om)` `(.om).ar` `(.om).ko`
* auch `.auch` `.auch,i`
  * -*auch* `au` → aber oder sondern
* *auf* `AU` `AU.ei-ad-er` `.e-AU` `(n)AU`
  * *darauf* `dar-AU` __`dar-AU.in`__
* *aus* `.S` `.S.ei-ad-er` `.S-t.ig` __`.S-t.ig.a`__ `.S-ts` `n.S` `O.-S` __`dar-Ü.n.S`__
  * auß `.auss` `.auss.um` `.auss.ko` `.auss:ha` __`ach.auss.in`__
    * äußer `.äuss` `.äuss.L` `äuss-st` `G.äuss` `E.äuss` `AU-s.äuss.-st` `(bi)z.äuss.-st`
* auß → aus
* aut `.aut` `.aut.o` __`.aut.o-us`__ `.aut.om`

## [B](#wörterbuch)

| [BR](#br) |
|:-:|

* baby `b.eb.I`
* *baden-würtemberg* `b.ad-Ü`
* **bahn** `ba` `ba-t.e` `.an.ba` `.aut.o-ba` `bu:ba` `.eis.ba` `.eis.en.ba`
* **bald** `ba` `ba.i` `.as.ba` `(o)ba` `s.ba`
* ball `b.all`
* *bamberg* `b.am.b`
* band ← bind
* bank `b.ak` `b.ak.en` `bu:b.ak` `zra.b.ak`
  * bänk `b.ek` `b.ek.e`
* bankrott `b.ak-r.ot` ← rupt
* bar `b.ar` `b.ar.FT`
* -*bar* `bar` ← bär
* bär `b.är` `G.b.är`
  * bier `b.ir` `G.b.ir`
  * bar `b.ar` `G.b.ar`
    * → -bar
    * bärd `b.ed` `G.b.ed.e`
  * bor `b.or` `G.b.or`
  * burt `but` `G.but`
  * **behr** `b` `eNT.b`
* bär `b.är`
* **bas** `ba` `ba.is`
* **bau** `bau` `bau.er` `b.au.er.FT` `bau.ko` `IN,u-met,bau`
  * **bäu** `bäu` `bäu.er.in` `bäu.L`
* bass `b.ass` `(ko)tra.b.ass`
* *bayer*, *bayern* `b.ay`
* **be**- `be`
* becken __`b.ek`__ @-en
* **behr** `b` `UN.eNT.b.L`
* **be**<i></i>**ginn** __`be,gi`__ ← ginn
  * **be**<i></i>**gann** `be.ga`
  * **be**<i></i>**gonn** `be.go`
* behörde ← hör
* bei `b.ei` `b.ei-ei-ad-er` `b.ei,i` __`.a-b.ei`__ __`.e-b.ei`__
  * *bei*- `(.ei)`
* ***beid*** `.eid` `.eid.e` `.eid-er.l.ei`
* beiß __`b.eiss`__
  * biss __`b.iss`__
    * *bisschen* `bi.ch.en` ← beiß
* bereit ← reit
* berg `b.eg` `b.eg.e` `E.b.eg`
  * birg `b.ig` `G.b.ig.e`
  * barg `b.ag` `E.b.ag`
  * borg `b.og` `G.b.og` `E.b.og`
* *ber****lin*** `(l.in)` `(l.in).er.i` `w.l.in`
* *besser* `bes`/`br°` `bes-UNG` `G.bes` `.S.bes` `E.bes` `E.bes-Ü-ig.T` ← gut @-er
* best `b.est` ← besser
* bett __`b.et`__
* *bezichtig* `be.z.i` ← zeih
* *bezirk* `be.ik` ← zirk
  * -*bezirk* `.ik` → regierung verwalt
* **bi** `bi` `bi.al`
* *bibel* `bib`
* bibl `b.ibl` __`b.ibl.gra.I`__
  * *biblio****thek*** `(t.ek)` __`(t.ek).ar.i`__
* bieder `b.id-er`
* bieg `b.ig`
  * bog `b.og`
* ***biet*** `.it` `.it.ed` `.an.er.it` `G.it` `Ts.it` __`AU.ab.it`__ `fa.it` __`ho.H.it`__ `r.ur.it` `E.it` `Ü.it` `AU.it` __`AU.m.it`__ __`AU.iS.it`__
  * ***bot*** `.ot` `.ot.e` `.ot.FT` `G.ot` `.an.ot` `Ü.ot` `E.ot`
* *bil****anz*** __`(.az)`__ `(.az.ir-UNG)`
  * *bilanz* `(az)` `(az.ir-UNG)`
* **bild** `bi` `bi.ed` __`bi-er`__ `bi.is` `bi-UNG` `G,bi.e` `.S,bi` `fr,bi` `O,bi.L`
* **bill** `bi` `bi.i`
* bin ← sein
* **bind** __`bi`__ `(E)bi` `(E)bi.L` `UNTER,bi` `bi-o`
  * *verbind* `E,bi` `E,bi.L`
  * *in.verbindung.mit* `(I)E.M`
  * **band** `ba`
    * -*verband* `ba` → land zentral
    * **bänd** `bä` `bä-e` __`b-er`__ __`b.i`__ __`E.bä-e`__
  * **bund** `bu` `bu.um` `bu.ik` `G.w.ek.FT.bu`
    * **bünd** `bü` `bü.e` `bü.el`
* **bi**o `bi` `bi.o-I` __`bi.og.i`__
* **bis** `bi`
* *bischof* `bi.of` `(.es):bi.of`
  * -*bischof* `.of` → landes
* bisschen ← beiß
* bist ← sein
* bitt __`b.it`__ __`b.it.e`__ __`b.it.ed`__ __`f.b.it.e`__ __`UN.er.b.it.L`__
  * -*bitt* `.it` `.I.it.sI` `M-r.it.e` `wr.it.sI`
  * bat `b.at`
  * bet `b.et` `G.b.et` `UN-G.b.et`
* bitter `b.it-er`
* **blatt** `bla` `.om.bla`
  * **blätt** `blä` __`bl.er`__ `bl.ch.en`
* **blau** `blau`
* blei `bl.ei` `bl.ei,(.ei)` `bl.ei,i`
* **bleib** `blei` `.S.blei` `E.blei` @eiie
  * **blieb** __`bli`__ `G.bl.i` `.S.bl.i` `(I)t-er.bl.i` `E.bl.i`
* bleich `bl.eich` `bl.eich,u` `bl.eich-Ü.i`
* ***blick*** `.ik` `.ik.ed` `.ikt.e` `.S.ik` `er.ik` `hn.ik` `rÜ.ik` __`.aug.ik`__ __`.aug.ik.L`__ `CH .um:ik`
* ***boot*** `.ot`
* **bloß** `.oss`
* **blut** `blu`
  * -*blütig* `-Ü.i` → heiß kalt
* **bod**en `bo`
* **börs** `bö` `bö-e` `bö,(l.er)`
* **buch** `bu` `bu.ha` `-G.bu` `ha,bu` `ha.el,bu` __`ja,bu`__ `(.iz).bu`
  * -*gesetzbuch* `bu` → burg hand straf
  * **büch** `bÜ` `bÜ-er` __`GT.bÜ-er`__
* *bühr* `-Ür` `G-Ür`
* bunt `but` `but-er` `but.est` `bu.H`
* **bür** `bÜ` `m.ak-er-Ü.o`
* burg `b.ug` `b.ug.en`
  * -*burg* `bu` → hoch
  * *bürg* `bÜ` `bÜ-er` `bÜ-er.L` __`bÜ-er.L.bu`__ `bÜ.FT` `N.bÜ-er` `E.bÜ` __`M.bÜ-er`__
* bus `b.us`
  * -*bus* `.us` → aut
* butter `b.ut-er`

### [BR](#wörterbuch)

* **bracht** `bra` `(.an)bra` __`(.ei)bra`__ `(er)bra` `Ü.bra` `UN.an.bra` `(E)bra` `Z.Sa.bra` ← bring
  * **brächt** `brä` `brä-e` @-ä
* branch `br.asch` `br.asch.e` `(t.il),br.asch.e`
* **brau** `brau`
* **brauch** `brau` `brau-t.e` `brau.bar` `brau.bar.T` __`(E)brau`__ `(E)brau.er` `G.brau` `G.brau.eil` `ms.brau`
  * **bräuch** `bräu` `G.bräu.L`
* **brech** `br`
  * **brich** `bri`
  * **brach** `bra`
    * **bräch** __`brä`__ @-ä
  * **broch** `bro` __`UN.UNTER.bro`__
  * **bruch** `bru`
    * **brüch** `brÜ`
      * -*brüchig* → kontrakt vertrag
* breit `br.T`
* *brems* `br-s` `br-s.e` `G.br-s` `.ab.br-s` `.eib:br-s.e`
* **brief** `bri`
* -***bring***- `.ing` __`br.ing`__ `.an.ing` `.S.ing` `b.ei.ing` __`er.ing`__ `Ü.ing` `UN.W.ing.L` `E.ing` `Z.Sa.ing` `Z.fa.ing`
  * → bracht
* **brud**er `bru` @-er
  * **brüd**er `brÜ-er` `brÜ-er.l.N` __`brÜ-er.L`__

## [C](#wörterbuch)

| [CH](#ch) | [CHR](#chr) | [CR](#cr) |
|:-:|:-:|:-:|

* *cicero* `ci.o`
* *cit****y*** `(y)`
* **cliqu**e `kli`
* *cottbus* `c.o`

### [CH](#wörterbuch)

* *charakter* `ch.ar` `ch.ar.iS.i` `ch.ar.Ol`
* **chem** `ch` `ch.I` `ch.ik-er` `ch.ik.I.en` `ch.i` `bi.ch.I`
  * *chemisch* `ch.em.i`
  * *chemikalien* `ch.em.ik.I.en`
  * *biochemie* `b.i-ch.I`
* chef `sch.ef` `sch.ef.in` `(.ir)-UNG:sch.ef`
  * *regierung*schef `ir-UNG.sch.ef`
* **chin** `chi` `chi.a` `chi.es.e` `chi.es.in` `chi.es.i`
* chlor `chl.or`

### [CHR](#wörterbuch)

* **christ** `kri`
* **chrom** `kro`

### [CR](#wörterbuch)

* cranach `cr.an.ach`

## [D](#wörterbuch)

| [DR](#dr) |
|:-:|

* *d. h.* `d.h°`
* -***da***- `.a` `d.a` `da.in` `.a-ma-s` → für her hin @dáhin
  * *damals* `da°`
  * *da*- `da` → hinter nach zwischen
  * *da*- `a` → bei durch mit von zu
* dach `d.ach`
  * däch `d.ech` `d.ech-er`
* dacht ← denk
* dam `d.am` `d.am.e`
* dank `d.ak` `d.ak.ed` `gru.G.d.ak.e` `UN.d.ak` `M.b.est.d.ak` `wr.d.ak,i` ← denk
  * *dank*<i></i>*bar* `(bar)` `(bar).T` `s.bar`
  * *gedank* `G.da` `G.da.e` `G.da.os`
* *d****ann*** `(.an)` → denn
* ***dar*** `dar` `dar-i` `dar-u`
  * ***dar***- `dar`
* **d**as `d` `d.i°`
* **dat** `da` `da.ei` `da.en` `da.ir` `da.um`
  * → kartei
* **d** __`d`__ `tra.d.ir`
  * **dit** `di` `tra,di-o` `tra,di.i-eL`
    * → diz
* daub `d.aub` `f.ass.d.aub.e`
* *dauer* `dau` `dau.d` `dau-t.e` `dau.ha` `.an.dau.d` `be.dau` `be.dau.L` `G.dau` `AU.I.dau` `Z.m.ei-em:be.dau` `Z.U°.be.dau`
* *de****m*** `(m)` `.auss.m` `(I)m` `M.m`
  * *mit.dem* `MM` P
* **dem** `d.e` → krat
* *de****n*** `(n)` `(I)n` `M.n` `v.n`
  * *mit.den* `MN` P @3-stufiges-m
  * *von.den* `VN` P @3-stufiges-v
* denk `d.ek` `d.ek.ed` `d.ek-er` `d.ek.ma` `UN.d.ek.bar` `ach.d.ek.L`
  * **dacht** `da` `E.da`
    * **dächt** `dä` __`dä-e`__ `G.d.is` `E.d.i`
  * → dank
* denn `d.en` ← dann
* dens `d.e-s` `ko.d.e-s.at` `ko.d.e-s.ir`
* depot `(d.e)p.o` ← post
* *de****r*** `(r)` `(r).a.i` `.S-er` `(I)-er`
* *d****es*** `(.es)` `(.es)ha` `(.es)w`
* *desto* `d.est`
* **deut** `deu` `deu.L` __`be.deu`__ `.an.deu`
* *deut****sch*** `SCH`
* *deutschland* `SCH-a`
  * *bundesrepublik.deutschland* `bu,SCH-a`
* di- __`d.i-`__
* *dia*- `di` `d.i-b.es` `d.ib.ik-er` `di.os.e` `di.on.al` `d.al.ekt` `d.al.ik` `d.al.i` `di.og` `di.m.aNT` `di.op`
* *didakt* `d.id` `d.id.ik` `d.id.ik-er` `d.id.i`
* *die* `I` `f.I`
  * *dass.die* `d I` ≠ dass.ich
* **dien** `di` `di.ed` `di-t.e` __`di-er`__ `di.bar.T` __`di.L`__ `be,di` `E,di`
* **dienst** `di` `di.bar.T` `di.f.i` `di.L` `be,di.et` `E,di` `E,di-ol` `.auss.d.i` `.auss.d.i-L` `.in.d.i` `.in.d.i-L` `wer.d.i` `.auss,di` `ku:d.i` ← dien @di=d.i
  * *dienstunfähig* `di.UN` `di.UN.T`
  * *im.dienst* `.im.d.i`
* *dies* `.iS` `.iS.em` `.iS-er` `.iS-es` `.iS:ma` `.s.iS-er`
* **digit** `di` `di.al` `di.al.ir-UNG`
* dik `d.ik` `IN.d.ik.iO`
  * diz `d.iz` `IN.d.iz` → dit
  * ***dex*** `ex` `IN.ex`
  * **dikt** `di` `di.at` `di.at.or` `di.at.or.i` `di-ur` `di.ir` __`di-o`__
* **ding** `.ing`
  * *allerdi****ng***s `(ng)-s`
  * *bedi****ng*** `(ng)` __`(UN.ng)-t`__
* *dis*- `ds`
* **dit** `di` `ko,di-o` `ko,di.i-aL` ← diz
* *distrikt* `(ds)tr.ikt` ← strikt
* *diplom* `di.om` `di.om.at` `di.om.I` `di.om.isch`
  * *das.diplomatische.corps* `d,di-om.ko`
* *doch* `d-ch`
  * *jedoch* `j-ch`
* doktor ← doz
* *doll****ar*** `(.ar)`
* dom `d.om` __`d.om.än.e`__ `d.om.il`
  * *domin* `d.om` `d.om.ir` `d.om.aNT`
* **dopp**el `do-el` __`E.do-el`__
* dorf `d.of`
* dort `dot` `dot.ig`
* ***dox*** `.ox` `pa.ox` `pa.ox.I`
* doz `d.oz` __`doz.eNT`__
  * dok `d.ok`
    * *dokument* `d.ok` __`d.ok.ar.i`__ `d.ok.iO` `d.ok.ir`
  * *doktor* `der`
* *du* `dU` `bi.dU` `ha.dU` `w.dU`
* **duk** `du`
  * **duz** `du` `IN.du.ir` `(o)du.ir`
  * **dukt** `du` `IN.du-o` __`(o)du`__ __`(o)du-O`__ `(o)du.iv`
* duld `d.ud` `d.ud.ed` `d.ud.et.e` `d.ud-Sa.T` `d.ud-UNG` `G.d.ud` `UN-G.d.ud.i`
* *dunkel* `d.uk` `d.uk.H` `E.d.uk` `E.d.uk-t.e`
* dunst `d.ust` @`dust`
* *durch* `dr` `.a-dr`
* **dürf** `dÜ` `dÜ-t.e` __`be-dÜ`__ __`be-dÜ.iss.e`__ __`be-dÜ-t.ig`__
  * -*bedürf* `-Ü` `d-Ü.is` `I-Ü.iss.e` `N-Ü.is` → besser dring hilf hohl
  * **darf** `da` `be.da`
  * **durf** `du` `du-t.e` 
* *düsseldorf* `dÜ.o`
* dutz `d.uz` __`d.uz.ed`__
* *dynam* `d.ün` `d.ün.ik` `d.ün.it` `d.ün.i` `d.ün.o` __`.er.d.ün.i`__

### [DR](#wörterbuch)

* dreh `dr.eh` __`ha.um:dr.eh`__
* **dring** `dri` `dri.ed` `dri-est.e` `dri.L` `dri.L.T` `N,dri` `N,dri.L` `O,dri` `O,dri.L` `Z.dri.L` `dri-Ü.is`
  * **drang** __`dra`__ __`dra.Ol`__ __`dra.s.al`__
    * **dräng** __`drä`__ `dr.el` __`be.drä`__ `be.dr.is` __`E.drä`__ `E.dr.UNG` @-ä
  * **drung** `dru` `G.dru` `N.dru`
* **droh** `dr.o` `be.dr.o-L`
* **druck** `dru` `dru.er` `dru.ei` `dru,(.e)` __`.S.dru`__ `N.dru` `ach.dru`
  * **drück** __`drÜ`__ __`.S.drÜ.L`__ __`N.drÜ.e`__ __`ach.drÜ.L`__

## [E](#wörterbuch)

* eben `.eb` `(o).eb`
* echt `.echt`
* effektiv `.e-f.ekt.iv`
* ego __`.eg.o`__ `.eg.iS` `.eg.ist` `.eg.iS.i` @-ist
* ehr `.er`
  * *ich.habe.die.ehre* `.I-a,(.e)`
* eid `.eid`
* eigen, eign `.eig` __`.eig.L`__ `.eig-s` `.eig-Ü.i` `.eig.FT` `.eig.tYm.er` `er.eig.is` `G.eig.et` `UN-G.eig`
* ein `N` `N-t.e` `N.i` `N.i-t.e` `N,u` `E.N` __`E.N.bar`__ __`E.N.i`__ `E.N-Sa` __`W.N.ma`__ __`-Z-n,N.ma`__
  * -ein `N` `d.N` `k.N` `m.N` `s.N`
    * *kein* `(K)`
      * -*kein*- `k` → fall weg
    * -**ein**- `.ei`
      * `.ei-e` `.ei-em` `.ei-en` `.ei-er` `.ei-es`
      * `d.ei-e` `k.ei-e` `m.ei-e` `s.ei-e`
  * *ein****heit*** `H` `UN.H.L` `E.H.L` @H
  * -*verein* `N` → rud sport
  * -*vereinigung* `ig` → wieder
  * *einst* `.eist` `.eist.ig` `.eist.ma-s` `.eist.w.eil`
  * *einzel*, *einzeln* `N.el` `N.el.H` `E.N.el` `.im.N.el`
    * *einzel*- `.ei` → haft handel händler
  * *ein****z*** `(z)` `(z).i`
* ein `N` `h.N` __`n.N`__
  * *von.vornherein* `v.N`
* *einfluss* `N.uss` ← fluss
  * *einflüss* `N.üss` `N.üss.e`
* eis `.eis`
* *eisen* `.eis.en` ≠ eis @-en
* el `.el` `.el.e-met` `.el.e-met.ar`
* *elektr* `.el` `.el.i` `.el.if.ir` `.el.ir` `.el.ät` `.el.od.e` `.el.on.i`
* eltern ← alt
* eltern `.et-ern` `.et-ern.os` `.et-ern.FT` `O.et-ern`
* em
  * emp
    * *exemp*, *exempl* `.ex` `.ex.el` `.ex.ar` `.ex.ar.i`
* emp*f****a****ng* `.emp-a` ← fang
  * emp*f****ä****ng* `.emp-e`
  * emp*f****i****ng* `.emp-i`
* emp*f****e****hl* `.emp-e` ← fehl
  * emp*f****ie****hl* `.emp-i`
  * emp*f****a****hl* `.emp-a`
  * emp*f****o****hl* __`.emp.o`__ @-o
* emp*f****i****nd* `.emp-i` ← find
  * emp*f****a****nd* `.emp-a`
  * emp*f****u****nd* `.emp-u`
* -en `.en`
* end `.ed` `.ed.e` `be.ed.et` `be.ed.i` `Ol.ed` `.ed.L` `UN.ed.L` `UN.ed.L.T` @-lichkeit
* -end `.ed` → dutz jug
* *ener****g*** `(g)` `(g).I` `(g).i`
* *ent*- `eNT`
* *ent*behr ← behr ← bär
* ente `.ent.e`
* -*ent* `eNT` @-ent
* *er* `er`
* -er `.er`
  * -erei `.ei` @-erei
  * -er*lich* `.L` @-erlich → bäuerlich körperlich lächerlich
* erb `.eb` `.eb.e` `.eb.L` `.eb-FT` `G.eb-t` `E.eb-UNG`
* erd `.ed` `.ed.e` `be.ed.i`
  * ird `.id` `.id.i` `.auss.id.i` `Ü.id.i` `UNTER.id.i`
* ernst `.enst` `.enst.est` `.enst.ha` `.enst.ha.i-T` `.enst.L`
* erst `.est` `.est.es` `.est.e-s` `O.est` `Z.est`
* **e**s `.e` `.e-i°`
* ess `.ess`
  * aß `.ass`
  * *ge*gess `G.g.ess`
* ~~ess~~
  * *inter*~~ess~~ `(I)t-er` __`(I)t-er.e`__ `(I)t-er.aNT` `(I)t-er.eNT-en` `(I)t-er.ir` `(I)t-er.os.i-T` `(UN.I)t-er.aNT`
* *essen* `.ess` @-en
* *etw****a*** `(.a)` `(.a)-s`
* *europa*, *europä* `.eu-p` `.eu-p-er` `.eu-p.i`
* **ex**- `.e`
* *exekut* `.ex.ut` __`.ex.ut.ir`__ `.ex.ut.iv.e` `.ex.ut.iO` ← sequ
* exemp ← em
* *ex*plodier `.e-pl.od.ir` ← plod
  * *ex*<i></i>*plos* `.e-pl` __`.e-pl`__ `.e-pl.iv` ← plos @-sion

## [F](#wörterbuch)

| [FR](#fr) |
|:-:|

* **fabr** `fa` `fa.ik` `(arr):fa.ik` `fa.ir`
  * *fabrik* `fa` `fa.aNT` `fa.at` __`fa-o`__ @-ikation
* facette `f.ass.et.e` ← faci
* **fach** `fa` `f-er` `fa.L` `fa.FT` `N.fa` `N.fa-er` `N.fa.H` `eNT.fa` `E.N.fa` `ma.i-fa`
  * *einfacher* `N.fa.er`
* faci
  * → facette
* faden `r.ad`
* **fäh** `f`/`fäh` `f.i` `f.i-T` __`(T).f.i`__ __`(T),UN.f.i-T`__ `er.T.er-UNG:f.i` `l.ei-f.i` `l.ei-f.i-T`
* **fahr** `fa` `fa.bar` __`fa-er`__ `fat` `G.fa` `G.fa-ol` `.ab.fa` `.ab.fat` `.S.fa` `be.fa.bar` `er.fa` `er.fa.G` `.if-fat` `UN.er.fa` `UN.er.fa.H` `E.fa` `Z.fat` `u-rÜ:fa`
  * **fähr** `fä` __`fä-e`__ `ft` `er.f` `G.fd` __`UN-G.fä`__ `G.f.L` `G.f.L.T` `G.f-t.e` `G.f-t.in` `UN-G.f.L` `(.l):f.i` `dU er.fä .e`
  * **fuhr** `fu` `.S.fu` `N.fu`
    * → führ
* *fakt* `fat` `fat.ur.a` ← faz
  * → fekt
* **fall** `fa` `fa.ed` `fa-s` `G.fa` `.al.fa-s` `AU.fa` `.S.fa` `b.ei-fa` __`.eb.fa-s`__ `G.g.fa-s` `gl.fa-s` `k.fa-s` `m.o-ell.fa` `n.ot.fa-s` `n.öt.i-fa-s` `rÜ.fa` `UN.fa` `Z.fa` `zw.fa` `(AU)j.fa` `AU.k.fa` `f.fa.d` `.im.ad-er:fa` `IN.iS:fa` `(I)j.fa` `IN.ei-em,o,fa` `v.fa.fa` `W.i-fa-s`
  * **fäll** `f` `f.e` `f.i` `f.i-T` `G.f.i` `G.f.i-T` `rÜ.f.i` `schw.f.i` `Ü.f.i` `UN-G.f.i` `UN-G.f.i-T` `Z.f.i` `AU.al.f.e` `er fet` `IN.eid:f.en` `(I).en.m.ei=f.en` `IN,o,f.en` `IN.il:f.en`
  * **fiel** `fi` __`AU,fi`__
* falsch `f.asch` `f.asch.H`
  * fälsch `f.esch` `f.esch.L`
* falt `fat` `fat.e` `fat.et.e` `N.fat` `eNT.fat` `eNT.fat.UNG` `o:fat`
  * fält `fet` `N.fet.ig` `hu:fet.ig` `o:fet.ig` __`E.il.fet.ig`__
* *famili* `f.am` `f.am.är` __`f.am.e`__
* -***fang***- `ang` `f.ang` __`.an.ang`__ __`be.ang`__ `N.ang` __`.um.ang`__ __`.um.ang:eich`__ `fi.ang` __`v .an.ang .an`__
  * → empfang
  * -***fäng***- `eng` `f.eng` __`.an.eng.er`__ __`G.eng.is`__ `E.eng.L`
  * -***fing***- `ing` `f.ing`
* fant `fat` `fat.as.I` `fat.as.ir` `fat.a+st` `fat.a+st.i`
* farb `f.ab` `f.ab.e` `f.ab.i` `f.ab.os`
  * färb `f.eb` `f.eb-UNG`
* farm `f.am`
* *f****ass*** __`(.ass)`__ __`G.asst`__ __`G.ass.H`__ __`.an.ass`__ `AU.ass` `be.ass` `er.ass` `.um.ass.ed` `E.ass-er` `Z.Sa-ass.ed` @-heit
  * *fassen* `fass` `G.fass` `G.fass.H` `.an.fass` ≠ anlassen
  * -*auffassung*- `.ass` `(I)bi.ass` `ach.m.ei-ass` `m.ei-ass:ach`
  * → fass
* fass `f.ass`
  * fäss `f.ess` `f.ess.er` `f.ess.ch.en`
  * → fass
* fassade ← faci
* fassade `f.ass.ade`
* fasson `f.ass.ong`
* *fast* `Fa`
* fat, fit → fess
* **faz** `fa` `fa.it`
  * → fiz
  * → fakt
* **fed** `f` `f-er` `f-er-Ü.ed`
* fehl `f.el` `be.f.el`
  * *fehler* `f-er`
  * → empfehl
  * fiehl `f.il` `be.f.il`
  * fahl `f.al` `be.f.al`
  * fohl `f.ol` `be.f.ol`
* *f****eier*** `(.ei-er)` `(.ei-er).L` `G.ei-ert` `j.ub.ei-er`
* fein `f.N` `f.N.H`
* **feind** `f.ei` `f.ei.e` `f.ei-L` `f.ei-FT` `f.ei,(g)` `f.ei,(g).T`
* **fekt** `f` `.a-f` `.e-f` __`.e-f.iv`__ `.e-f-ol` __`ko.f`__ __`p.f`__ ← fekt
  * → fit
* feld `f.ed` `m.in.f.ed`
* fend `f.ed`
* fens `fes` `d.e-fes.iv` `.o-fes.iv` ← fend
  * *offensiv* `.o-f.iv`
* fenster `fest.er`
* ***fer***n `fer` `fer-er` `eNT.fer` `eNT.f-ert` `v.n.ah.fer` __`(o)fer`__ `IN,(o)fer` `IN,w.i-fer`
  * *insofern* `IN.fer`
  * *inwiefern* `IN.i-fer`
* **fer** `f` `d.if.eNT` `d.if.ez` `d.if.ez.ir` `d.if.ir` `ko.f.ez` `ko.f.ir` `pr.f.ez` `r.f.at` `r.f.eNT` `tra.f`
  * differier `d.if.er.ir`
  * *konferenz* `ko.f`
    * -*konferenz* `ko` → rekt
      * -*ministerkonferenz* `ko` → aus kult
  * → lat
* *ferien* `f.I.en` `(.est.er),f.I.en` `s.om.f.I.en`
* fern ← fer
* *fert* `-t` `-t.ig` `-t.ig.T` `.an-t.ig` `.S-t.ig` `UN-t.ig`
* **fess** `f` `ko.f-o` `(o)f-o` `(o)f.i-eL` `(o)f.or` ← fat, fit
    * *konfession* `ko.f`
    * *profess* `(o)f.ess` `(o)f.ess.iO` `(o)f.ess.i-eL` `(o)f.ess.or`
      * *professor* `(o)f`
* *fest* `f°`
* *feuer* `feu` `feu-UNG` `feu-ig` `.an.feu`
  * *feuerung* `feu-er-UNG`
* **fig** `fi` `fi-ur` `ko,fi-ur.iO`
* fili `f.il.I` `f.il.us`
  * *filial* `f.iaL` `f.iaL.e`
* film `f.im` `f.imt.e` `G.f.imt` `E.f.im`
* filter `fit.er` `fit.r.ir`
* **fin** `fi` `fi.al` `fi.al.e` `fi.al.ist` __`fi.al.i+st.in`__ __`d.e,fi.ir`__ __`d.e,fi.iv`__ __`d.e,fi-o`__
* *finanz* `fi` `fi.en` `fi.ieL` `fi.ir` `fi.ir-UNG` `fi.a` `fi.uss` `fi,(er)` `fi.um` `fi.om` `O,fi.ir` ← fin
* **find** `fi` `fi-er` `fi.i` `fi.i-T` `fi.ing` `.ab,fi` `AU,fi` `be,fi` `be,fi.L` `.emp,fi` `er,fi` `er,fi-er` `er,fi-er.i` `O,fi` `u-rÜ,fi` `Z,fi` `Z.Sa,fi`
  * → empfind
  * *find* `f.i` `.ab.f.i` `AU.f.i` `be.f.i` `be.f.i-L` `.emp-f.i` `er.f.i` `O.f.i` `u-rÜ:f.i` `Z.f.i` `Z.Sa.f.i`
  * *zu.finden* `Z.fi` ≠ zufällig
  * **fand** `fa` `be.fa` `.emp-fa`
    * **fänd** __`fä`__
      * *fänd* `f.äd`
    * *fand* `f.ad`
  * **fund** `fu` `G.fu` `.ab.fu` `AU.fu` `.emp-fu`
* **fing**er `fi-er`
* **finst**er `fi-er`
* *firm* `im` `im.a` `im.en` `im-met` `im.ir` `ko,im-o`
  * firm `f.im` `f.im-met` `ko.f.im-o`
* **fisch** `fi` `fi-er` `fi-er` `fi-er.ei` `ho.s.e,fi-er.ei`
  * *fisch* `f.isch`
* fit → fat
* fit `f.it` `(o)f.it` ← fekt
* **f**iz `f` `b.en.e-f` __`d.e-f.it`__ __`.e-f.eNT`__ __`ko.e-f.eNT`__ `.o-f.ieL` ← faz @-ent
* fl __`fl`__ `s.u-fl.ir`
  * → flat
* **flach** `fla`
  * **fläch** `fl`
* **flamm** `fla` __`fla-e`__ `AU.fla` `eNT.fla`
  * **flämm** __`fl`__ __`fl.ch.en`__
* **flat** `fla` __`IN.fla-o`__ __`IN.fla-o.är`__ __`IN.fla-o.iS.i`__ `IN.fla.or.i` ← fl @-istisch
* ***flieg*** `.ig`
  * ***flog*** `.og`
  * ***flug*** `.ug`
* ***fleisch*** `.eisch` `.eisch.i` __`r.id.eisch`__
* **flekt** `fl`
  * flekt `fl.ekt`
  * **flex** `fl` `fl.ib.el` `fl.ib.ät` `fl.iO`
* -***flieh***- `.ih` __`fl.ih`__ `eNT.ih`
  * *flieh* `fli`
  * -***floh***- `.oh` __`fl.oh`__ `G.oh`
  * → flucht
* **fließ** `fli`
  * **floss** `flo`
  * **fluss** `flu`
    * → einfluss
  * **flüss** `flÜ` `flÜ.i`
    * *flüssig* `.üss.i`
* floh __`fl.oh`__
* ***flott*** `.ot` `.ot.e` `ha.el.ot.e`
* **flu** `flu` `IN.flu.ez.a` __`ko.flu.ez`__
  * *influenza* `IN.flu.a`
* ***flucht*** `.ucht` ← flieh
  * ***flücht*** `.ücht` `.S.ücht.e`
    * *flüchtli****ng*** `(ng)` `(ng)-Ü.o` = beding
    * -*flüchtling* `-Ü.ing` `(r)-Ü.ing` `N-Ü.ing`
* *föder* `fö` `fö.iS` `fö.iS.i` `fö.iO`
* **folg** `fo` `fo.L` `fo-Sa` `fo.er-UNG` `G.fo` `er.fo` `er.fo-eich` `ach.fo.er` `ach.fo.FT` `ms.er.fo` `er fo` `er fo-t.e` `IN.fo-e` `IN.fo-e.es.en` `wr fo-t` `zr.fo.ha`
  * *infolge* `IN.o`
  * *erfolg* `er` `er.eich`
    * *erfolgreich* `er,r.ei` ≠ erreich
* foli `f.ol.I` `f.ol.I.e` `f.ol.I.en`
* *forder* `F` `F-d` `F-UNG` `AU.F` `er.F.L` `er F` `er F-t.e` `er F.e` `I F` `I F.en` `(UN.ng)-t er.F.L`
  * *erforderlich* `er.o-L` `(UN.ng)-t er.o-L`
    * *unbedingt.erforderlich* `UN-t.L`
  * *förder* `Fö` `er Fö` `er Fö-t.e` `I Fö` `I be.Fö.en`
* *fort* `fr` `(o)fr`
* ***form*** `.om` `.om.al` `.om.al.iS.i` `.om.al.ät` `.om.at` `.om-o` `.om.el` `.om.ell` `.om.ir` `.om.os` `.om.ar` `.om.ul.ir` `.om,(.e)` `IN.om.ik` `IN.om.ik-er` `IN.om-o` `IN.om.iv` `ko.om` `ko.om.ät` `ko.om.iS` `ko.om.iS.i` `r.om` `r.om-o` `r.om.or.i` `r.om.ir` `tra.om.or` `.um.om` `.un.om`
  * -*reform* `.om` → schul wahr
  * ***förm*** `.öm` `.öm.L` `N.öm.i` `UN.öm.i`
* forsch `f.osch` `f.osch.er` `er.f.osch` `I ach.f.osch.en`
* -*fug*- `-u` `f.ug` `be-u.is`
  * -*füg*- `-Ü` `f.üg` `f.üg-Sa` `.an-Ü` `b.ei-Ü` `N-Ü` `G.üg.e` `G.üg.i` `G.ing-Ü.i` `E-Ü` `Z.üg` `Z.Sa.üg` @ü
    * *beigefügt* `b.ei-Ü-G`
    * *zur.verfügung* `zr*-Ü`
* fug `f.ug` `zr.f.ug.al`
  * *zentrifugal* `z.etr.ug.al`
* -*fühl*- `Ül` `f.ül` `b.e-Ül` `G-Ül` `G-Ül,(g)` `G-Ül.Ol` `.angst-Ül` `N-Ül` `M-Ül` `ach-Ül` `O-Ül` `t.uch-Ül-UNG` `d-Ül` `.I-Ül.e` `CH.E.bu-Ül` `CH.E.icht-Ül` `wr-Ül`
  * *gefühlvoll* `G-Ü.Ol`
  * *tuchfühlung* `tu-Ül-UNG`
* **fuhr** `fu` `fu-e` `er.fu` `.S.fu` `N.fu` `Z.fu` ← fahr
  * *fuhre* `fu.e`
* -*führ*- `.ür` ← fuhr
  * `f.ür` `f.ür-er` `f.ür-UNG`
  * `.S.ür` `.S.ür.L` `dr.ür` `dr.ürt` `Ü.ür` `u-rÜ.ür` `Z.Sa.ür`
  * `G.ürt` `.ab.ür` `.an.ür-er` `AU.ür` `AU.ür-UNG` `N.ür` `N.ürt` `GT.ür-er` `GT.ür-UNG` `.e-b.ei-ür` `UNTER.ür-UNG` `E.ür` `O.ür` __`.a-z.ür`__ `AU,si.ür.ed`
  * *aus*<i></i>*f****ü****hr* `.S-ü` `.S-ü.L`
  * *durch*<i></i>*f****ü****hr* `dr-ü` `dr-ü-t`
  * -*führend* `-Ü.ed` → fed sicht
  * -*führ*- `-Ür` `G-Ürt` `.ab-Ür` `.an-Ür-er` `AU-Ür` `AU-Ür-UNG` `N-Ür` `N-Ürt` `f-er-Ür.ed` `GT-Ür-er` `GT-Ür-UNG` `.e-b.ei-Ür` `UNTER-Ür-UNG` `E-Ür` `O-Ür` `AU-i-Ür.ed` `a-Z-Ür`
* **füll** `fÜ` `fÜ.e` `fÜ.ed` `fÜ-er` `.S.fÜ` `er.fÜ` `er.fÜ-t.e` `er.fÜ-bar` `er.fÜ.UNG` `ach.fÜ` `Ü.fÜ` `I er.fÜ` `.icht.er.fÜ.UNG` ← voll
    * *erfüllen* `er-Ü.en`
* **fund** `fu` `fu.eNT` `fu.ir` `d.i,fu`
  * *fundament* `fu-t`
  * → fus
* fung `f.ung` `f.ung.ir`
  * → funkt
* funk `f.uk` `f.uk.e` `f.uk.el` `f.uk-er` `ru:f.uk`
  * *rundfunk* `u,u`
* **funkt** `fu` `fu-o` `fu-o.är` `fu-o.ir` ← fung
* **f**ür `f` `f.ei-ad-er` `.a-f`
* furcht `f.ucht` `f.ucht.bar` `f.ucht.os` `f.ucht-Sa`
  * fürcht `f.ücht` `f.ücht.et.e` `f.ücht.L` `be.f.ücht`
* fus `f.us` `f.us` `f.us.ir` `f.us.ha*.en` `d.i-f.us` `ko.f.us` `tra.f.us` ← fund @-sion
* **fuß** `fu` `ha.fu`
* **futt** `fu` `fu-er`
  * **fütt** `fü` `fü-er`

### [FR](#wörterbuch)

* fracht ← eig
* **fracht** `fra` `fra.ko`
* **frag** `fra` `fra-e` `fra.ed` `fra.L` `fra.l.os` `G.fra` `.an.fra` `ach.fra` `rÜ.fra` `I be.fra-t.en` `sI fra` `sI fra-t.e` `sI fra-t.en`
* *frankfurt* `fra.fu` `fra.fu-er`
  * *frankfurter* `fra.fru`
  * *frankfurt.am.main* `fra,u-N`
  * *frankfurt.an.der.oder* `fra,u.od-er`
* **frau** `frau`
* *fr****ei*** `(.ei)` `(.ei).H`
* **fre**md `fr` `fr.e` `.es.fr`
* fr**eu** `eu` `eud.e` `er,eu-L`
* *freund* `.eund` `.eund.in` `UN.eund.L` `.eund.FT` `be.eund.et`
  * *freundlich* `eu-L` `UN.eu-L`
  * *freundschaft* `eu.FT` `ga,eu.FT`
  * -*freund* `eu` `.an,eu` → geschäft umwelt
* ***fried*** `.id` `b.ug.id.e` `UN.z.id.en`
  * *unzufried*en `UN.z.id` @-en
* frier __`fr.ir`__
  * fror `fr.or` `Z.fr.or`
  * frost `fr.ost`
* ***frist*** `.ist` `kÜ.ist`
  * -*fristig* `.i` → kurz lang
* **froh** `fro`
  * **fröh** `frö` `frö.L`
* **fro**nt `fr` `fr.o` `ko.fr.ir` `ko.fr.iO` @o
* **frucht** `fru` `fru.bar`
  * **frücht** `frÜ`
    * -*frücht* `-Ü` → süd
* **früh** `frÜ` `frÜ.er` `frÜ.er-er` `frÜ.est.es` `frÜ.est.e-s` `frÜ.ing`
  * -*früh* `-Ü` → heut morg schon

## [G](#wörterbuch)

| [GR](#gr) |
|:-:|

* **gang** `ang` `G.ang` `ang.bar` `.S.ang` `N.ang` `.um.ang` `E.ang` `E.l.or.ang` `GR-Ü.ang` `Ü.ang.od.e` ← geh
  * **gäng** `eng` `rÜ.eng.i` `UN.E.eng.L` `O.eng.er` `UN.z.eng.L`
* **ganz** `ga` `ga.H` `ga-S`
  * **gänz** `gä` `g.L` `er.gä`
* -**gar**- `ga` `g.ar` `ga-t` `(o)ga` `ga.ga`
* *garant* `g.ar` `g.ar.I` `g.ar.ir`
* garten `gat` `ki.gat`
* **gast** `ga` `ga.L` `ga.ir`
  * **gäst** `gä` `gä-e`
* gastr `g.a+str` `g.a+str.om`
* *ge*- `G`
  * *unge*- `UN-G`
* ***geb*** __`.eb.`__
  * __`.eb-er`__ __`.an.eb-er`__
  * __`G.eb`__ __`G.eb.H`__ __`.ab.eb`__ __`be.eb.H`__ __`er.eb`__ __`er.eb-st`__ __`ach.eb`__ __`Ü.eb`__ __`E.eb-s`__ __`E.eb.L`__ __`u-rÜ.eb`__
    * __`er.eb.is`__ __`er.eb.is.os`__
  * __`-G.eb-UNG`__ __`ku.eb`__
  * __`.S.ag.eb.ed`__ __`n.S.ag.eb`__
  * -*geber* `g` → arbeit __rat__ trag
  * ***gib*** `.ib` `ach.ib.i-T` `er.ib.i`
  * ***gab*** `.ab` `.ab.e` `.ab.ab.e` `.an.ab.e` __`AU.ab.e`__ __`AU.ab.it`__ __`.S.ab.e`__ `be.ab-UNG` `b.ei-ab.e` `N.ab.e` `er.ab` `hn.ab.e` `rÜ.ab.e` `Ü.ab.e` `O.ab.e` `Z.ab.e`
    * ***gäb*** `.äb` `ang.äb.e`
  * → gift
* geburt ← gebär ← bär
* gefahr ← fahr
* *gegen* `g`
  * __`g-er`__ __`g-er.in`__ __`g-er.i`__ __`g-er.FT`__ `g.ei-ad-er` `g-t` `g-t.ig` `be.g` `eNT.g` `hn.g` __`.a-g`__ `g.I` `g.d` `g.m.i` `g.U`
  * *geg****end*** `(.ed)` `.um.ed`
* **ge**h `ge` `ge.ed` `.S.ge` `N.ge.ed` `eNT.ge` `E.ge` `O-Ü.ge` `w.T.ge.ed` `Z.gru.ge`
  * → ging
* **geist** `g.ei`
* geld `g.ed` `g.ed-er` `g.ed,(.e)` `b.ar.g.ed` `ki.g.ed` `.ös.e-g.ed`
  * *kindergeld* `ki.g`
* gelt `get` `get.ed` `eNT.get` `UN.eNT.get.L`
  * gilt `git` `E,git`
  * galt `gat`
  * golt `got` `G.got`
  * gült `güt` `güt.ig` `güt.ig.T` `UN,güt.ig` `.ed,güt.ig` `gl,güt.ig` `echt,güt.ig.T`
    * -*gültig* `-Ü.i` → allgemein
* *gemein* `G.N` `G.N.H`
  * *gem****eind*** `(N)-d` `(N)-d.e` `(N)-d.e-hö` `(N)-d.e-o` `(N)-d.e.UNG` `la.N-d.e`
  * *gem****einnützig*** `(N)-Ü.i`
  * *gem****einsam*** `(N)-Sa` `(N)-Sa.T`
  * *gem****einschaft*** `(N).FT`
    * -*gemeinschaft* `N` → arbeit
  * *allgemein* `.a-G` `.a-G-Ü.i` `E.a-G.er` `.im.a-G` ≠ allein
* *ge****nau*** `G.au` ← nau
* gen `g.en` `g.en.iaL`
* *gener* `g.en` `g.en.al` `g.en.ell` `g.en.iO`
  * *general* `g.en`
    * *general*- `g` → sekret streik
* *ge****nug*** `G.ug` ← nug
  * *ge****nüg*** `G.üg` `b.üg` `E.üg`
* geo `g.e` __`g.e-gra`__ __`g.e-og.e`__ __`g.e-o-I`__ __`g.e-I`__ __`g.i`__
* gern `g.en` `s.g.en`
* *gesellschaft* `G.FT` `G.FT-er` `G.FT.L` `E,G.FT` `(.I).en,G.FT`
* gesetz ← setz
* *gest*, *gestern* `g.est` `g.est.r.i` `O.g.est`
* gier `g.ir` `g.ir.i` `be.g.ird.e` `neu=g.ir.i`
  * *neugierig* `n.eu-g.ir.i`
* *gift* `git` `blu.E,git` ← geb
  * *blutvergift* `blu,E.i`
* **ging** `.ing` `O-Ü.ing` `u-rÜ.ing` ← geh
* **ginn** `gi` → beginn
* *giro* `gi.o`
* **glanz** `gla` `gla.O`
  * **glänz** `glä` `gl.ed` `er.glä`
    * *glänzend* `gl.ä-ed`
* glas `gl.as`
* ***glaub*** `.aub` `.aub.e` `.aubt.e` `.aub.ha` `be.aub.i` `G.aubt` `.ab-er.aub.e` `.I.aub` `wr.aub`
  * *ich*.gl**au**b `.I-au`
  * *wir*.gl**au**b `wr-au`
  * ***gläub*** `.äub` `.äub.i-er` `UN.äub.i`
* *gleich* `gl` :`.es.gl`
  * *glich* `gl.i`
* gleit `gl.T`
  * glitt `gl.it`
* ***glied*** `.id` `M.id` `M.id-er.Sa.UNG`
  * *mit*<i></i>*gl****ie****d* `M-i`
* ***glück*** `.ük` `.ük.L` `.ück.L.eis.e` `be.ük.ed` `UN.ük` `UN.ück,(g)` `E.UN.ük`
* **gönn** `gö`
  * **gunst** `gu` `UN.gu` `Z.gu` `Z.m.ei-en:gu` `Z.U°.gu` `Z.m.ei-en,UN.gu` `Z.U°.en,UN.gu`
    * *zu.meinen.ungunsten* `Z.m.ei-en.UN-G.gu`
    * *zu.unseren.ungunsten* `Z.U°.UN-G.gu`
    * **günst** `gÜ` `gÜ.i` `gÜ.i-er` `gÜ.ist` `UN.gÜ.i` `er be.gÜ.i-t.e .in` `I E.gÜ.i` `I E.gÜ.i-en`
* gunst ← gönn
* **gut** `gu`
  * **güt** `gÜ` `gÜ-e` `gÜ.i` `gÜ.L` `gÜ.er` `gÜ.E` `E.gÜ` `E.gÜ.et.e` `E.gÜ.UNG`
    * -*g****ü****t*- `-Ü` `IN.w-O-Ü.er`
  * → besser

### [GR](#wörterbuch)

* ***grad*** `.ad`
  * → gress
* gramm ← graph
* **graph** `gra` `gra.ik` `gra.i` `gra.it` `bi.gra.I` `b.ibl.i-gra.I` `d.gra.I` `f.on.gra.i` `ge-gra` `st.en.gra` `t.e-gra` @-o
  * *steno****graph*** `(gra)`
  * *paragraph* `pa`
  * gramm `gr.am` `gr.am.f` `f.on.gr.am` `t.e-gr.am`
    * *grammophon* `gr.am.on`
    * *programm* `(o)gra` `(o)gra.ir`
      * -*programm* `gra` → regierung wahl
* grav `gr.aw` `gr.aw-ur` `gr.aw-o`
* ***greif*** `.eif`
  * ***griff*** `.if` `-t;an.if:pa`
* *grenz* `GR` `GR.en` `GR-er` `GR.os` `(.an)GR` `(be)GR` `UN.be.GR`
* **grob** `gro` `gro.H`
  * **gröb** `grö`
* **gress** `gr` `(ko)gr` `(o)gr.iv` `(o)gr.iO` `(r)gr` ← grad
* *groß* `gr` `s.gr` `gr.ib` `gr.ha` `M.gr.H` ≠ grob
  * **größ** `grö` `grö.e` `(E)grö.er`
    * *vergrößern* `(E)grö` @-er
* **grund** `gru` `gru.os` `.ab.gru` `hi-er-UNG:gru` `(I)t-er.gru` `kÜ.gru` `UNTER.gru` `O.d-er.gru` `.S-S.gru.e` `v.gru.AU`
  * *aus.diesem.grunde* `.S-S.e`
  * **gründ** `grÜ` `grÜ.e` `grÜ.L` `grÜ.L.T` `grÜ.Lst` `hi-er:grÜ.e` `m.id-er:grÜ.e` `(be)grÜ` `UN.be.grÜ` `UN.er.grÜ.L` `grÜ.er` `grÜ-UNG` `.S-S.grÜ.en` `I (be)grÜ`
    * *hinderungsgründe* `hi-er-UNG-Ü.e`
* **grupp** `gru` `gru.en` `gru.ir-UNG` `gru.eis.e` `.um:gru.ir`
  * *gruppe* `gru.e`
* ***gruß*** `.uss`
  * ***grüß*** `.üss` `M.eund.L:üss.en`
    * *mit.freundlichen.grüßen* `M,eu-Ü`

## [H](#wörterbuch)

* **hab** `ha` `G.ha` `gu.ha` `im.IN.ha-er` ← heb
  * *ich.hab* `.I-a`
  * *wir.hab* `wr-a`
  * *hast* `ha`
  * **hat** `ha`
    * **hat**.*sich* `ha*`
  * *hatt* `t`
    * *hätt* `.ät`
  * → haft
  * → heb
* hab `h.ab` `h.ab.ir`  `h.ab.us` @ilitier
* **haft** `ha` `ha,i` `.ei-ha` = handschrift meine.damen.und.herren ← hab
  * *nam****haft*** __`(ha)`__
* halb `h.ab` `h.ab.ir` `h.ab-er` `h.ab.T` `(r) .od-UNG h.ab-er`
  * -*halb* `ha` → außer des inner unter wes
  * *halb*- `ha` → insel jahr wicht
  * *halb* `ha` `ha.ir` `ha.T`
  * **hälft** `h` `h.e` `.il:h.e`
* *hall* `ha` `ha.e` `schw.im:ha.e`
* hals `h.a-s`
* *hal****t*** `(-t)` `(-t).bar` `(-t).bar.T` `(-t).l.o` `(-t).ed` `(-t).UNG` `G-t` `.an-t` `.an-t.ed` `AU-t` `.S-t` `N-t` `er-t` `IN-t` `.in-t` `ach-t.ig` `rÜ-t.l.o` `sa.E-t` `.aus-t` `(-t).I` `(-t):wr` `dU (-t)` `er (-t)` `.ir (-t).et` `sI (-t)`
  * *haushalt* `.aus,(-t)`
  * *haltung* `-t.UNG`
  * *häl****t*** `(-t)` `E-t.is` `E-t.is.i`
    * -*verhältnis* `.is` → arbeits
  * hielt `hit`
    * *hielt* `hi` `.ab,hi` `be,hi` `eNT+hi` `(er)hi` `UNTER,hi` `u-rÜ,hi`
      * *behielt* `b,hi`
* hammer `h.am.er`
  * hämmer `h.em.er`
* **hand** `ha` `ha.L` `ha,i` `ha,i-L` `ha.ek` `ha.ek-er` `ha.ek.ib` `ha,(.el)` `O.ha` `UN.ha.L` `li:ha` `ha.IN.ha` = haftpflicht
  * **hand**el `ha.el` `ha.el.d` `ha.elt.e` `G.ha.elt` `be.ha.el` `N.ha.el` `.ei-ha.el` `ms.ha.el` `ha.el.bu` `ha.el.um`
    * *handelt.sich.um*, *handelt.es.sich.um* `ha.um`
    * -*handel* `ha` → auß buch groß welt
    * -*handel*- `ha` `be.ha` `N.ha` `ms.ha`
    * *handlung* `ha*` `bu:ha*` `E.ha*`
      * -*verhandlung* `ha*` → fusion vertrag
  * **händ** `h` `h.e` `h.l.er` `.S.h.i` `li.h-er`
    * *händler* `h-er` `.ei-h-er` `gr.h-er` `vi:h-er`
    * *linkshänder* `li:h.ed-er` ≠ linkshändler
* -***häng***- `eng` `h.eng` `ab.eng.i` `UN.ab.eng.i` `.an.eng-er` `.an.eng.i`
  * *unabhängig* `UN.ab.i`
  * -***hing***- `.ing` `h.ing`
  * -***hang***- `ang` `h.ang` `.ab.ang` `.an.ang` `O.ang` `Z.Sa.ang` `IN.iS.em Z.Sa.ang`
    * *zusammenhang* `u-ang` `IN.iS,u-ang`
      * *in.diesem.zusammenhang* `IN.iS.ang`
* harf `h.af` `h.af.e`
* harm `h.am` `h.am.os`
* *harmon* `h.am` `h.am.I` `h.am.ir` `h.am.i` @-isier
* ***harr*** `arr` `be.arr.L.T` @-lichkeit
* **hart** `hat`
  * **härt** `het` `het.e` `het.est` `G.het` `.ab.het` `er.het`
* häs @-sion
* **haupt** `hau` `be.hau.et` `hau.a.L` `hau.uss` `hau,(.e)` `hau.L` `(.es).hau` = hauptstraße hauptsache
  * *oberhaupt* `.o-hau`
  * **häupt** `häu` `häu-er` `häu.ing`
* ***haus*** `.aus` `ho.aus` `ra.aus` `kra.aus` `sta.aus` `.im.aus.e` `v.aus.S` `ho.aus`
  * *hohes.haus* `ho-au` ≠ hochhaus
  * ***häus*** `.äus` `.äus.er` `.äus.L` `.äus.ch.en` `G.äus.e`
    * *ge*häuse `G.h.äus.e`
* heb `h.eb` `h.eb.e` `h.eb.el` `schat.h.eb.el`
  * *heb* `h.e` `.ab.h.e` `.an.h.e` `AU.h.e` `be.h.e` `er.h.e` `er.h.e-L` `Ü.h.e-L`
  * *erheblich* `er.L`
  * hob `h.ob` `AU.h.ob` `er.h.ob`
  * hub `h.ub`
  * hab `h.ab` `er.h.ab`
  * → hab
* *heidelberg* `h.eid.b`
* ***heim*** `.eim`
  * *heim****at*** `(.at)`
* heiß ← heiz
* **heiß** `h.ei` `gu.h.ei`
  * *gutheiß* `gu.eiss`
  * **hieß** `h.i`
    * hieß `h.iss`
* **heiß** `h.ei` __`h.ei-Ü.i`__ ← heiz
* heiz `h.eiz` `zra.h.eiz*`
  * *zentralheizung* `zra.eiz*`
  * → heiß
* -*heit* `H`
  * → keit
* heiter `h.T.er`
* helf `h.ef` `h.ef-er.in` `h.ef-er.h.ef-er` `ach.h.ef`
  * hilf `h.if`
    * → hilf
  * half `h.af`
  * holf `h.of` `G.h.of` `.S.h.of`
* helm `h.em` `IN.egr.al.h.em`
* hemm `h.em` `h.em-UNG.os`
* **her** `h` `(bi)h` `.a-h` `O.h`
  * *mehr.als.vorher* `m.as.h`
  * *her*- `.e` → auf bei unter vor
    * *heraus* `rrs`
* *her****bst*** `(bst)` `(bst).L`
* **herr** `h` `h.in` `h.i` `h.L` `h.FT` `m.ei-h.en` `s-G.ert.h.en` `(r) h` `.ei-a-h.en`
  * *herrisch* `h.err.i`
  * *der.herr* `(r)h`
  * *meine.damen.und.herren* `.ei-a-h` = einzelhaft
* **herr**sch `h.sch` `h.scht.e` `h.sch.er` `be.h.sch` `I -st,be.h.sch` `h.sch.s.ücht.i` ← herr
* *her****z*** `(z)` `(z).ha` `(z).L` `(z).L.T` `(z).os` `kust,(z)` `w.eich,(z).i` @-lichkeit
* heuer `h.eu-er` `UN-G.h.eu-er`
* **heut** `heu` `heu-Ü`
* **hier** `hi` `(hi)AU` `hi.S` `(hi)b.ei` `(hi)dr` `(hi)h` `hi.IN` `hi:M` `h:v` `hi:UNTER` `(hi)-Z`
  * *hier*- `h.i` `h.i-b.ei` `h.i-Z`
  * hies `h.is` `h.is.i`
* hies ← hier
* **hilf** `hi` `hi.e` `hi-Ü-t.ig` `hi.r.ei` `.ab,hi.e` `b.ei-hi.e` ← helf
  * *hilfreich* `hi.eich`
* *hin* `hn` __`.a-hn`__ "dahín"
  * *hin*ten `hn-t` `hnt` `hnt.h.um`
  * *hin*- `n` → auf aus ein wirk zu
  * -*hin* `.in` → auf da immer weit
  * → hinter
* **hind** `hi` `hi-er` `hi.L` `hi.is` `hi.iss.e` `be,hi-er` `E,hi-er`
  * *behinder* `b,hi`
  * *verhinder* `E,hi`
* hinten ← hin
* *hinter* `(I)t-er` `(I)t-er.ei-ad-er` `(I)t-er.h` `(I)t-er.m.i` ← hin
  * *dahinter* `(da)t-er`
* *histor* `hi-o` `hi-o.I.e` `hi-o.i` `hi-o.ik-er` `li.ar,hi-o.i`
  * literar*histor***isch** `l.it-er.ar,hi-o.i`
* **hoch** `ho` `ho.bu` `ho,(g)` `ho-t.ig` `s.ho`
  * **höch** `hö` `hö-t` `hö-t.e-s` @-st
  * *höchstens* `hö°` @-st
  * **hoh**, **ho** `ho` `ho.H` `(.es).ho.H`
    * *hoheit* `o-H` `(.es).o-H`
    * **höh** `hö` `hö-er` `hö-er-er` `hö.e` `hö,(er)` `hö,u` `hö,u.e` `.an.hö.e` `er.hö` `.ei-e 3,(g) er.hö` `I l.o-er.hö`
      * *die.lohnerhöhung* `I l.o-er`
* **hof** `ho` `ba.ho`
* hoh ← hoch
* hohl `h.ol`
* **hol** `ho` `er.ho-Ü.is` `W.ho`
  * *wiederholen* `W-o`
* *honor* `h.on` `h.on.ar` `h.on.ir`
* hor `h.or` `G.h.or-Sa`
  * horch `h.och` `h.och-er` `G.h.och`
* **hör** `hö` `be.hö.d.e` `schwa.hö-er` ← hor
  * *behörde* `be.hö`
    * -*behörde* `hö` → gemein
* horch ← hor
* hort `hot`
* *hundert* `hu` `hu.FT` `3.hu` `5,hu` `ja.hu`
* hut `h.ut` `be.hut-Sa` `be.hut-Sa.T` `.ob.h.ut`
  * *behutsamkeit* `be.h.ut.T`
  * *obhut* `.o-h.ut`
  * hüt `h.üt` `h.üt-er` `be.h.üt` `E.h.üt`

## [I](#wörterbuch)

* -ia
  * → -ien
* -*ial* `.iaL`
  * -*iell* `.ieL`
* *ich* `.I`
  * *ich*. `(I)`
* -id
  * -idier `.ir`
  * -idor `.or`
* ide `.id.e` `.id.e-al` `.id.e-al.iS` `.id.e-al.iS.i` `.id.e-al.ist` `.id.e-ell`
* -*i*en `I.en` ← -ia
  * -*i*ener `I.er`
  * -*i*en*isch* `I.i`
* -ier- `.ir`
* -*ifik*- `.if` `.if-o` ← fak
  * -*ifisch* `.i` @-ifisch
    * *pazifisch* `p.az.if.i`
  * -*ifiz*- `.if` `.if.ir` @-ifizier
    * modifizieren `m.od.if.iz.ir`
    * *qualifizier* `qu.al.ir`
* -*ig* `.i`/`.ig`
* ihn `.in` `.in.en`
  * -*ihnen* `i` → bei mit um dank
* ihr `.ir` `.ir-er`
* *illustr* `.ill` `.ill.ir` `.ill.iO`
* -ik `.ik`
  * -*ikalisch* `.ik.i`
  * -*ikation* `.iO`
* im ← in
* *imm****er*** `(.er)` `(.er):fr` `(.er).in` `(.er),i`
  * n*imm****er*** `(n.er)` `(n.er).s.at`
  * *immerhin* `.i-hn`
* imple*ment* __`.impl.e-met`__ `.impl.e-met.ir` `.impl.e-met.ir-UNG` ← pl
* *import* `.imp` `.imp.ir` `.imp.ör` ← port
* in ← inn
* *in* `IN` `IN.s` `IN.ei-ad-er`
  * *in*- `(I)` `(I)-S.e`
  * -*in* `i` → dar
  * im `.im`
* innen `.in` `in.um` @-en
* *inner* `.i` `.i-ha` `.i-L` `er.i` `er.i-t.e` `er.i-L` `.im.i`
* -in-
  * -*inal* `.i-aL`
  * -*inell* `.i-eL`
* *industr* `IN` `IN.I` `IN.iaL.ir-UNG` `IN.ieL`
  * *industrie* `IN.u-I` `m.et.IN.u-I`
* *ingenieur* `IN.ör` `d.ipl.IN.ör`
* init ← it
* insel `.i-s.el` `ha.i-s.el`
* inser ← ser
* instinkt → stinkt
* *intellig* `IN.t` `IN.t.eNT` `IN.t.ez` ← lig
  * *intelligent* `IN.t-t`
* integer → teger
* *inter*- `(I)t-er`
  * *intell*- `IN.t`
* *intrig* `(I)tr.ig` `(I)tr.ig.e`
* -*ion* `.iO`/`-o`
  * -*ional* `.i-aL`
  * -*ionell* `.i-eL`
  * -*sion* `-s` @-sion
    * *mission* `ms-o` `ms-o.ar` `ms-o.ir`
      * *emission* `.e-ms-o`
    * *prozession* `(o).ess.iO` ≠ prozess
    * *version* `w.es.iO` __`w.es.iO.ir`__ `d.i-w.es.iO` `IN.w.es.iO` `ko.w.es.iO` ≠ vers versiert divers invers konvers wes
      * **kon**versat*ion* `ko.w.es.at.iO` ≠ konversion
* *irgend* `.i`/`.ig` `.ig.N` `.ig.ei-e` `.ig.ei-em` `.ig.ei-en` `.ig.ei-er` `.ig.ei-es`
  * n*irgend* `n.i`/`n.ig`
* irr `.irr`
  * *irr****tum*** `tum` @tum
* irrit `.irr.it` `.irr.it.ir` `.irr.it.iO`
* -is- `.iO`/`-o`
  * -isier `.ir` @-isier
    * *harmonisieren* `h.am.is.ir` ≠ harmonieren
    * *modernisieren* __`m.od-er.is.ir`__ ≠ moderieren
      * *modernisieren* `m.od-ern.ir`
    * *pauschalisieren* __`pau.al.is.ir`__ ≠ pauschalieren
* -**isch** `.i`/`.isch`
* -*ismus* `.iS`
* ist ← sein
* -ist `.ist` @-ist
  * *egoist* `.eg.i+st`
  * -*istisch* `.iS.i` @-istisch
    * *absolutistisch* `.ab.ut.i`
    * *inflationistisch* `IN.fla.i`
    * *nationalistisch* `(n.al).i`
    * *rationalistisch* `ra.i-aL.i`
    * *seminaristisch* `(n.ar).i`
* it `.it` `IN.it.iaL.e` `IN.it.iaL.en` `IN.it.iv.e` `IN.it.or` `tra.it`
  * *initiative* `IN.iv.e`
  * *initiator* `IN.at.or`
* -it- ← -at-
* *itali* `.it` `.it.en` `.it.er` `.it.i`
  * *italien* `.it`
* -iv `.iv` `.iw-o` `.iv.ir`
* iz ← ik

## [J](#wörterbuch)

* ja `j.a`
* **jahr** `ja` `GTs:ja` `frÜ.ja` `ha.ja` `ja.S ja.N`
  * *jahr.für.jahr* `ja.ja`
  * -*jahr* `.ar` `.im.ar.e` `.im.let.ar` `.im(-st).ar` `.im-o.ar` `.im (g).ar` `(I)let.ar.en` `(I)n,(-st).ar.en` `IN.iS.ar`
    * *in.einer.reihe.von.jahren* `IN.ei-er-r.eih.ar.en`
    * im.*ver*~~gang~~enen***jahr*** `.im.E.ar`
  * -*jahre 19****80*** `(80)` `.im,(80)`
  * **jähr** `j` `j.L` `.is:j.i` `(E)j`
    * *diesjährig* `.is.i`
* *japan* `j.ap` `j.ap.i` `j.ap.er`
* je `j.e` `j.e-ma` `j.e-ma-s`
* **jed** `j` `j.e` `j-er` `j.es` `j.es.ma`
  * jede `j.ed.e` ≠ je
* jen `j.en`
  * *jenige*, *jenigem*, *jenigen* `j` `(m):j` `(r):j`
* *jetz* `j` `j.i`
  * *jetzt* `j°` `j°.sch` `bi:j°`
* **j**iz `j` `(o)j.ir`
  * **j**ekt `j` `(IN)j.iO` `(.ob)j` `(.ob)j.iv` `(.ob)j.ir` `(.ob)j.ät` `(o)j` `(o)j.il` `(o)j.iO`
    * *objekt* `.o-j` `.o-j.iv` `.o-j.ir` `.o-j.ät`
    * *projektil* `(o)jet.il`
* jubel ← jubil
* *jubil* `j.ub` `j.ub.um`
  * *jubel* `j.ub` `j.ub.d`
* ***jug*** `.ug` `.ug.ed` `.ug.L` `.ug.L.T` `bu.ug.an` `(.es).ug.an`
  * *jugend* `j.ug`
* jug `j.ug` `(ko)j.ug.iO`
  * junkt `j.ukt` `(ko)j.ukt.iO` `(ko)j.ukt.ur` `ho,(ko)j.ukt.ur`
      * *konjunktur* `o-u-ur` `ho,o-u-ur`
        * *hochkonjunktur* `ho,ko-u`
* jur `j.ur` `j.ur.iS.i`
* just `j.u+st` `j.u+st.ir` `j.u+st.ir-er`
  * *justiz* `j.us` `j.us.er` `j.us.um` `j.us.om` `j.us.UNG`

## [K](#wörterbuch)

| [KR](#kr) |
|:-:|

* *kal****ender*** __`(.ed-er)`__
  * kalender `k.al.ed-er`
* kalk `k.ak`
* *kalkul* `ka,u` `ka,u-O` `ka,u.ir` `ka,u.or.i` 
* kalt `kat` `kat-Ü.i` `er.kat`
  * kält `ket` `ket.e` `ket.er`
* **kamm**er `ka` `ha.ek.ka`
  * *handelskammer* `ha.ka`
    * *industrie.und.handelskammer* `IN.ha.ka`
* ***kampf*** `ampf` `kla.ampf`
  * ***kämpf*** `empf` `empf.e`
* kan `k.an` __`k.an.on`__
  * kant `kat` __`kat.at.e`__
    * → akzent
* *kan****al*** `(.al)` `(.al).iO`
* kandid `k.ad.id` `k.ad.id.at` `k.ad.id-UR` `k.ad.ir`
    * *kandidat* `k.ad` `k.ad-UR`
* kann → könn
* **kann** `ka` `kat.e` `be.kat` `G.kat` ← kenn
* *kap* `aP`
  * `aP.ät` `aP.ell.e` `aP k.en.ed.y` `aP.l.an`
  * `aP.al` `aP.al.I.en` `aP.al.iS` `aP.än` `aP.ul.ir` `aP.el 7`
    * *kapital* `aP` `aP.I.en` `aP.iS`
    * *kapitelle* `aP.it.ell.e` ≠ kapelle
* kart `kat` `kat.e` `v.is,kat.e` `kat.t.ek`
  * *kart****ell*** `(.ell)`
  * → kartei
  * *kartothek* `k.at.o-t.ek`
* *kartei* `ka.ei` ← dat kart
* *kartoffel* `ka.of.el`
* **kass** `ass` `ass.e` `kra.ass.e`
* **kat**a- `ka` `ka.og`
* *katastroph* `k.at` `k.at.e` `k.at.al` ← stroph
* *kateg* `k.at` `k.at.or.I`, `k.at.or.i`
* ***kauf*** `.auf` `.auf.ed` `.auft.e` `.auf.tra` `.an.auf` `.S.E.auf` `E.auf` `u-rÜ.auf` `N.auf` `rÜ.auf` `UN.E.auft` `wr.auf`
  * ***käuf*** `.äuf` `.äuf.e` `.äuf-er` `E.äuf-er` `E.äuf-er.in` `UN.E.äuf.L`
* **kaum** `kau`
* ***kehr*** `.er`
  * `.er.icht` `be.er`
  * `rÜ.er` `.um.er` `.um.ert` `E.er` `E.ert` `W.er` `u-rÜ.er`
  * -*verkehr* `E` → gut
  * um*ge*-~~kehrt~~- `.um.G`
  * kehr `k.er` `k.er.icht` `be.k.er`
    * *kehricht* `k.er.i`
* -***keit*** `T` ← heit
* **ke**nn `ke` `be.ke` `be.ke.is` `er.ke` `er.ke.is` `.an.er.ke`
  * *kenn* `k.en` `k.en.e` ≠ kennt
  * *anerkennen* `.an.er`
  * → kann
* **kind** `ki` `ki.H` `ki.i` `ki.L` `ki.os` `ki.eich` `ki:ki.er` `(N):ki`
  * *kindisch* `k.id.i`
  * *kindlich* `k.id.L` ≠ wirklich
  * *kinderreich* `k.id-er.r.ei`
* *kir****ch*** `(ch)` `(ch)L` `(ch)T.UNG`
* ***klag*** `.ag` `.ag.e` `.ag.bar` `.an.ag` `.an.agt.er` `be.ag`
  * ***kläg*** `.eg` `.eg-er` `.eg-er.in` `.an.eg-er` `n.eb.eg-er`
* ***klam*** `.am` `r.am-o` `r.am.e`
* ***klar*** `.ar` `.ar-er` `.am.arst` `.ar.H`
  * ***klär*** `.är` `.är-UNG` `AU.är-UNG` __`er.är`__ `er.är.L` `I er.är-UNG`
* **klass** `kla` `kla-e` `kla.ik-er` `kla.iS` `kla.i` `kla.if.ir` `kla.if-o`
* ***klaud*** __`.aud`__
* ***klaus*** `.aus` `.aus.e` ← klaud
  * -*klausel* `au.el` → zent
* *kl****ein*** `(N)` `(N).er` `(N)-st` `(N).H` `(N).i-T` `(N).L.T` `E.N.er` `zr.N.er`
* ***klim*** `.im` `.im.a` `.im.i` `.im.ir`
* **klin** `kli` `kli.ik` `.un.ät,kli.ik`
* kling `kl.ing`
  * klang `kl.ang` `N.kl.ang`
    * *in.einklang.bringen* `(I)N.ing`
    * kläng `kl.eng`
  * klung `kl.ung`
* knochen `kn.och`
* **ko**al `ko.al` `ko.al.ir` `ko.al.iO` ← al
  * *koalition* `ko.iO`
* koll
  * *protokoll* `pr.ot.ok`
* *kolleg* `ko` `ko.e` `ko.in` `ko.iaL.ät` `ko.um` `h.ko.e` `ko.in.en,&` ← leg
* **komm** `ko` `.ab.ha.ko` `.S.ko` `N.ko` `ach.ko` `ach.ko.FT` `.um.ko` `(.l).ko` `u-rÜ.ko` `dar-AU u-rÜ.ko` `eNT.g.ko` __`w.T eNT.g.ko`__ `IN.be.tra.ko` `IN.fra.ko` `IN.I.la.ko` `Z.ed.ko` `z.ab.uss.ko` `Z.echt.ko` `zr.get.UNG.ko` `Z.hi.ko` `zr.spra.ko` `Z.r Ü.eug.ko`
  * *darauf.zurückkommen* `dar-AU-Ü.ko`
  * *zugute.komm* `Z.u-ko`
  * *zu.hilfe.kommen* `Z.i-ko`
  * **kömm** `kö` `be.kö.L` `.ab.kö.l.ing`
  * kam `k.am`
    * käm `k.äm` `k.äm.e`
  * → kunft
* *kommun* `ko-u` `ko-u.al` `ko-u.iS` `ko-u.iS.i` `ko-u.ir`
  * *kommun*<i></i>*ion* `ko-u.iO`
  * *kommunikation* `ko-u-o` ≠ kommunion
* **kom**plement `kom-l.e-met` `kom-l.e-met.är` ← pl
* **kom**plett `kom-l.et` `kom-l.et.ir` ← pl
  * *komplett* `kom-l`
* **kom**<i></i>**pl**ik `kom-l` `kom-l-o`
  * **kom**<i></i>**pl**iz `kom-l.ir`
  * **kom**<i></i>**pl**ex `kom-l`
    * *komplex* `(ko)ex` ≠ komplett
* **kon**- `ko`
  * **koll**- `ko`
* *konkurr* `ko-u` `ko-u.eNT` `ko-u.ez` `ko-u.ir` ← kurr
  * *konkurs* `ko.us` ← kurs
* *konsum* `ko-su` `ko-su.FT`
* **kön** `kö` `kö.i` `kö.ig.in` `kö.i-L` `kö.i,eu`
  * *königin* `kö.i-in`
* **könn** `kö`
  * *kann*, *kannst*, *konn* `k` `G.k-t`
  * → kund
  * → kunst
* *kont* `ko` `ds.ko.ir` ← put
  * *konto* `ko` `ko.en` `b.ak.ko` `s.ko`
* *kontr* `(ko)tr` `(ko)tr.är`
  * *konter* `ko-t.er`
  * *kontra* `(ko)tra` `(ko)tra,u`
* *kontroll* `ko` `ko.e` `ko.ir` `ko.ir.bar` `ko.ör` ← kontr
  * *kontroll* `ko,o` `ko,o-e` `ko,o-en` ≠ kollege
* korp `k.op` `k.op-O` `k.op.iv`
  * corps `k.or`
    * -**corps** `ko` → diplom
* **körp**er `kö.er` `kö.er` `kö.L` `kö.er.FT` `E.kö.er` ← korp
  * *körperschaft* `kö.FT`
* kopf `k.opf`
* kosmo `k.osm` `k.osm.os` `k.osm.i` `k.osm.og.I` `k.osm.aut`
* **kost** `kost` `kost.en` `kost.bar` `kost.bar.T` `kost.l.o` `kost.il.i` `.S.kost` `UN.kost.en` `UN.kost,au.al.e` ← st
  * *kostenlos* `ko.os`
  * -*kosten* `ko` → bau fracht mehr neb person selb walt
  * **köst** `köst` `köst.L` `köst.L.T` `be.köst.ig`
* kub `k.ub` `k.ub.i` `k.ub.iS`
* kub `k.ub` `k.ub.a` __`k.ub.i`__
* **küch** `kÜ` `kÜ.e`
* kult `kut` `kut.iv.ir` `kut.ur` `kut.ur.ell` @-t.iv.ir
  * *kultus* `.us` `.us.er` `.us.ko`
* *kummer* `k.um`
  * *kümmer* `k.üm`
* **kund** `ku` `ku-e` `ku.i` `ku.in` `ku.FT` `ku.FT.er` `er.ku` `er.ku.i` `.ur.ku.L` `UN.ku.i` `.ur.ku-e` ← könn
  * *kund* `k.ud` ≠ kunde
  * *kunde* `ku.e` ≠ kund
  * **künd** `kÜ` `kÜ.i` `kÜ.bar` `.an.kÜ` `E.kÜ` `E.kÜ.i`
    * *künd*en `k.üd` ≠ kürzen
* **kunft** `ku` `Ü.N.ku` `Z.ku` `Z.Sa.ku` __`.S.ku:g`__ ← komm
  * **künft** `kÜ` `kÜ.i` `N.kÜ.e` `Z.kÜ.i`
* kunst `kust` ← könn
  * *künst* `kYst` `kYst.e` `kYst.L` `kYst.er` `kYst.er.i` `G.kYst.elt` @-ler
* kur `k.ur` `(o)k.ur.ist`
  * *prokurist* `(o)k.ist`
* kurb `k.ub` `k.ub.el` `.an.k.ub.el`
* **kurr** `ku`
  * → konkurr
  * **kur** `ku` `ku.ir`
  * → kurs
* **kurs** `ku-s` `ku-s.ir` `ku-s.iv` ← kurr
  * → konkurs
* **kurz** `ku` `ku.um` `ku.i` `ku,i` `ku,i-L`
  * *kurzschriftlich* `ku.L`
  * **kürz** `kÜ` `kÜ.e` `kÜ.er` `.am.kÜ.est` `kÜ-t.e` `kÜ.el` `kÜ.L` `kÜ.UNG` `.ab.kÜ` `E.kÜ`
* kuss __`k.uss`__
  * küss `k.üss`
* *kut*, *kuss* `-u-` `ds-u.ir` `ds-u` `p-u` @-sion

### [KR](#wörterbuch)

* **kraft** `kra` `kra.wa` `fli.kra` `M.al.kra`
  * **kräft** `kr` `kr.e` `kr.i` `be.kr.i` `M.al.kr.en` `eNT.kr.et`
* **krank** `kra` `kra.ha` __`kra.H`__ __`kra.wa`__ `er.kra`
  * **kränk** __`kr`__ __`kr.er`__ `kr-st` `kr.L` `kr.L.T` @-st
* kranz `kr.az`
* krass `kr.ass`
* krat `kr.at` `kr.at-er`
* ***krat*** `.at` `bÜ.at` `d.at` `d.at.I` `d.at.i`
  * *sozialdemokrat* `SS.d` `SS.d.I` `SS.d.ei`
* **kreb** `kr` `kr-s`
* **kred** `kr` `kr.it` `.ak-kr.ir` `.ak-kr.iv`
* *kr****eis*** `(.eis)` `Ts.eis` `la.eis`
* **kret** `kr` `ds.kr.iO` `ko.kr` `ko.kr.ir`
* ***kreuz*** `.euz`
* **krieg** `kri` `wet.kri`
* **krim** `kri` `kri.I` `kri.i-aL` `kri.i-aL.ät` `kri.i-eL` `ds,kri.ir`
  * *dis*<i></i>*kr****i****min* `ds-i` `ds-i.ir`
  * *kriminalität* `kri.ät`
* **krit** `kri` `kri.er.um` `kri.er.en` `kri.ik` `kri.ik-er` `kri.ik.os` `kri.i` `kri.ir` `UN,kri.i`

## [L](#wörterbuch)

* lach `l.ach` `l.acht`
  * läch `l.ech` `l.ech.el` `l.ech.L` `l.ech.L.T` @-erlich @-lichkeit
* lad `l.ad`
  * ***last*** `.ast`
* **land** `la` `la.FT` `la,(.e)` `la.UNG` `.S.la` `(hi)z.la` `(r).la`
  * *land****es*** `(.es)` `(.es).ba` `(.es).a` `(.es).of`
    * -*landes* `-es` `(.es)-es` `.U.es-es`
    * *landesamt* `.es.a`
  * **länd** `l`
    * `l.L` `.S.l.er` `G.l.e` `G.l.er`
    * `.ab.ed.l.i` `.S.l.i` `IN.l.i` `v.at-er.l.i`
    * *län****der*** `(d-er)` `(eNT)wi.l.d-er` `(I).N.el.d-er` `bu,&`
    * länd `l.ed` `l.ed.L` `.S.l.ed-er` `G.l.ed.e` `G.l.ed-er`
  * → deutschland
  * → niederlande
* **lang** `ang` `ang-Sa` `ang.i` `E.ang` `E.ang-Sa-UNG` `s.ang.e` `w.i-ang` `ja.ang`
  * *ebenso.lang* `.eb.ang` `.eb.ang.e`
  * **läng** `eng` `eng-st` `eng-st.es` `eng-st.e-s` `UN.z.eng.L`
    * *un*<i></i>*zu*läng*lich* `UN.z.l.eng.L` ≠ unzugänglich
    * *länger* `eNGR` `E.eNGR`
* -***lass***- `.ass` `l.ass` `E.an.ass` `(I)t-er.ass` `.S .an.ass` `.S-s .an.ass`
  * *aus.anlass* `.S.an`
  * *aus.diesem.anlass* `.S-s.an`
  * -***läss***- `.ess` `l.ess.i` `.an.ess.L` `E.ach.ess.i` `fa.ess.i`
    * *anlässlich* `.an.L`
    * *fahrlässig* `fa.l.ess.i`
  * -***ließ***- `.iss` `l.iss` `(I)t-er.iss`
* last ← lad
* lat `l.at` `l.at-er.al` ← fer
  * -*lateral* `.al` → bi mult
  * → relat
* laub `l.aub`
  * -*l****au****b* `-au`
    * *er*<i></i>*l****au****b*en `er-au` `er-au.is`
    * ur<i></i>*l****au****b* `.ur,au`
* ***lauf*** `.auf`
  * ***lief*** `.if` `be.if`
    * *lief* `l.if`
  * ***läuf*** `.äuf` `G.äuf.i`
    * -*läufig* `eu` `O,eu` → zwang
* laut `l.aut` `l.aut-er` `E.l.aut` `E.l.aut.bar`
  * *verlautbar* `E-au-bar`
  * läut `l.äut` `l.äut-er`
    * *läuter* `.äut-er`
* leb `l.eb`
* leer `l.er`
* led `l.ed` `l.ed.i-L`
* leg ← lieg
* leg `l.eg` `l.eg.ed.e`
  * → kolleg
  * → lig
  * → lekt
* lehn `l.en` `.ab.l.en`
  * *abgelehnt* `.ab.G`
* lehr `l.er`
  * *lehrer* `er-er` `fa.er-er`
  * *lehrling* `l.ing`
* **leicht** `l.ei`
  * *vielleicht* `v.ei`
* leid `l.eid`
  * litt __`l.it`__
* leih `l.eih` `.an.l.eih`
  * *anleih* `.an.l.ei`
  * lieh `l.ih`
* **leist** `l.ei` `hö-t.l.ei` `I g.l.ei`
* leit `l.T` `Ü.la-l.T` `.ift,l.T.UNG`
  * *schriftleitung* `i,l.T.UNG`
* lekt `l.ekt` `l.ekt.iO` `l.ekt.or` `l.ekt.ür.e` `IN.t.ekt` `IN.t.ekt.ell` `ko.ekt.iO` `ko.ekt.iv` ← leg
* lern `l.en`
* letz `l.ez` `E.l.ez` `E.l.ez°`
  * → verletz
* *letzt* `let`
* ***leut*** `.eut` `.eut.e` `fa.eut.e` `la.eut.e`
  * *landsleute* `la-s.eut.e` ≠ landleute
* *li* `l` `l.ir` ← lig
* -*lich* `L`
  * -*lich*<i></i>*keit* `L.T` @-lichkeit
    * *unendlichkeit* `UN.ed.T`
    * *beharrlichkeit* `be.arr.T`
    * *herzlichkeit* `(z).T`
    * *lächerlichkeit* `l.ech.T`
* licht `l.icht`
* lieb `l.ib`
* *liefer* `l.i-er` `l.i-er-UNG:tra`
* **lieg** `li` `.an,li` `.an,li.er` `b.ei,li-ed` `er,li` `n.ah,li-ed` `UNTER,li`
  * anlieger `.an.l.ig-er`
  * **lag** `la` `E.la` `.an.la-e` `gru.la-e` `.im.a.an.la-e` `ndr.la-e` `UNTER.la-e` `O.la-e`
    * *lager* `l.ag-er` `l.ag-er-UNG` `.ab.l.ag-er` `G.l.ag-ert` `E.l.ag-er`
  * **leg** `l` `l-t.e` `G.l` `.ab.l` `be.l` `dar-AU.l` `Ü.l` `E.l` __`E.l.er`__ `E.l.H` `zr.l-UNG` `f.est.l` `gru.l.ed`  `n.ah.l` `.ar.l` @-l
    * *geleg* __`-G.l`__ `.an-G.l.L` `b.ei-G.l` `O-G.l` `.an-G.l (N).ass`
    * *ge*<i></i>*legenheit* `G.H` `.an.H` __`.S.wet.ig.an.H.en`__ `b.ei-iS.H` __`(b.ei)j.CH.it.H`__
* lier → verlier
* lig `l.ig.a` ← leg
  * *liga* `l.ig` @-a
  * → intellig
  * → li
  * → relig
* -***ling*** `.ing`
* *lin****i*** `(.I)` `(.I).e` `ri.I.e`
  * *linie* `l.in`
    * *in*.~erster~.*lin****i***e `IN.I.e`
    * *in*.*zweiter*.*lin****i***e `IN.T.I.e`
* list `l.ist` `l.ist.r.ei` `l.ist.ig`
  * *listenreich* `l.i-en.r.ei`
* **li**st `l.i` `l.i-e` `l.i-en` `.an.es.H.l.i` `r.ang.l.i`
* *liter* `li` `li.ar.i` `li.at` `li.ur`
  * literat `l.it-er.at`
* liter `l.it-er`
* **lohn** `l.o` `(T).l.o`
* liquid `l.iqu.id` `l.iqu.id.e` `l.iqu.ir` `l.iqu.ir-UNG` `l.iqu-o` `l.iqu.or` `l.iqu.id.ät`
  * *liquid* `l.iqu` `l.iqu.e` `l.iqu.ät`
* log `l.og` `l.og.ik` `l.og.i`
  * -*log* `.og` -*logie* `.o-I` → archäo astro bio geo kata phraso polito theo
    * *astrologie* `.a+str.o-l.o-I`
    * *geolog* `g.e-l.og` `g.e-l.og.e` `g.e-l.og.I`
    * *theolog* `t.e-l.og` `t.e-l.og.e` `t.e-l.og.I` `t.e-l.og.i` ≠ theorie ← theo
* lok `l.ok` `l.ok.iv`
* *lokal* `l.ok` `l.ok.ir` `l.ok.ät` ← lok
* loqu `l.oqu` `ko.oqu.um`
  * *kolloquium* `ko.oqu`
* ***los*** `.os` `.S.os`
  * -*los* `.os` / `l.o`
  * ***lös*** `.ös` `.ös.bar` `AU.ös` `er.ös` `.os.ös` `UN.ös.L`
* lösch `l.ösch`
* lud `l.ud`
* lus `l.us` `.i-lus` ← lud @-sion
* lust `l.ust` `l.ust.ig`
* lux `l.ux` __`l.ux.us`__ __`l.ux.ur.ös`__

## [M](#wörterbuch)

* **mach** `ma` `be.kat.ma` `.it.ma-er`
  * *schrittmacher* `.it,(er)`
  * → macht
  * → makel
* *macht* `aCHT` `aCHT-o` __`aCHT.CH`__ ← mach mög
  * *macht.sich* __`aCHT°`__
  * *mächt* `eCHT` `wet,eCHT.e`
* magen `m.ag`
  * mägen `m.äg`
* -**mahn**- `ma` `m.an` `m.an.UNG` `.an.ma` `er.ma`
  * *mahnung* `ma-UNG`
* mainz `m.eiz`
* **mak** `ma` `ma.el` `ma.ur`
  * makulatur `m.ak.ur`
* makel ← mach
  * -*mak****ler*** `(l.er)` → börs
* *makel* `m.ak`
  * *makl*er `m.ak-er`
* -**mal**- `ma` `m.al` `m.al.er` `m.al.ei` `G.ma` `N.ma` `.iS:ma` → denk jed manch zu
  * -**mal**s `ma-s` → aber je mehr nie noch oft
  * *manch*es.**mal** `.aCH.es.ma`
  * -**mäl**- `mä` `G.m.ed.e`
* **man** `ma`
  * -**mand** `ma` → je nie
* **man** `ma` `ma.ell` `p.ma.eNT`
  * *manipul* `ma,u-` `ma,u-O` `ma,u.ir`
  * *permanent* `p.m.an.eNT`
* *manch* `aCH` `aCH-er` `aCH.l.ei` `aCH.ma`
  * *mancher* `aCH.er`
  * *manch*mal `aCH.m.al`
* **mand** `ma` `ma.aNT` `ma.aNT.in`
* **mang** `ang` `ang.el`
* **mann** `ma` `l.ig:ma.FT` `.auf.ma` `la.ma`
  * *landsmann* `la-s.ma` ≠ landmann
  * **männ** `m` `m.er` `m.L` `.auf.m.i`
* **mann*****heim*** `ma.eim`
* *mar****kt*** `(kt)` `Ts.kt` `d.is.kt` `schwa.kt` `vi.kt` `wet.kt`
  * *arbeitsmarkt* `T.kt`
  * *weltmarkt* `(-t),(-t)`
* *marokk* `ma.ok` `ma.ok.o`
  * *marokko* `ma.ok` @-o
* *masch****in*** `(.in)` `(.in).e` `(.in).ell` `n.äh,(.in).e`
* ***mass*** `ass` `ass.e` `ass.ha`
  * *massenhaft* `ass.en.ha`
* maß ← mess
* ***mat*** `.at` `.aut.at` `.aut.at.i` `.aut.at.ir-UNG` `.aut.at.iO`
* *mater* `ma` `ma.I.e` `ma.iaL` `ma.iaL.iS` `ma.iaL.ist` `ma.iaL.iS.i` `ma.ieL`
  * -*mater* `.iaL` → weis
  * *matr* `ma` `ma.az.e` `ma.ik.el` `ma.ix` `ma.iz.e`
* mauer `m.au-er` `sch.all.m.au-er`
  * *schallmauer* `scha.m.au-er`
* *maxim* __`m.ax`__ `m.ax.al`
  * *maximal* `m.ax.im-a`
* mech `m.ech`
  * mechan `m.ech` `m.ech.ik` `m.ech.ik-er` `m.ech.iS` `m.ech.ir` `m.ech.i` `m.ech.iS.i`
* med `m.ed` `m.ed.it.ir` `m.ed.it.iO` `m.ed.um` `m.ed.I.en`
  * *medik* `m.ed` `m.ed.eNT` `m.ed.eNT.ös` `m.ed.in` @-ik @-iz
    * *medikamentös* `m.ed.ös`
    * *medizin* `m.ed`
* **mehr** `m` `m.H` `m.ko` `m.ma-s` `E.m-UNG` `nu:m` `(.er)m` `-t.m`
  * *nicht.mehr* __`-t.mer`__ ≠ irrtum
    * *nicht.mehr* `tr.m` @zweistufiges-abstrich-t
  * *mehrer* `MR` `MR.e`
  * -*mehrheit* `H` → groß stimm wieg
* meid `m.eid`
* *m****ein*** `(N)` `(N).UNG` `I .ö.L.N` `(I)bi.N` `ach.m.ei-er.N` `m.ein-er.N:ach` `wr-I.N`
* mein → ein
* mein → gemein
* **meist** `m.ei`
  * **meist**er `m.ei-er`
* meld `m.ed`
* meng `m.eng`
* *mensch* `MSCH` `MSCH.H` `MSCH.L` `E.MSCH.L`
  * *menschheit* `MSCH°`
* ment `met` `met.al.ät` `met.or`
  * *komment* `ko-t` `ko-t.ar` `ko-t.at.or`
  * -*ament* `eNT` → orn perg temp test
    * *ornament* `.on.am.eNT`
* merk `m.ek` `m.ek,(.el)` `be.m.ek`
  * `AU.m.ek-Sa` `AU.m.ek-Sa.T` `UN.AU.m.ek-Sa.T` `dar-AU AU.m.ek-Sa ma` `M b° AU.m.ek-Sa.T`
  * -~~merk~~- `AU-Sa` `AU-Sa.T` `UN.AU-Sa.T` `dar-AU AU-Sa ma` `M-S AU-Sa.T`
    * -~~merksam~~- `AU.T` `UN.AU.T` `dar-AU AU.ma` `M-S AU.T`
      * -~~aufmerksam~~- `dar-AU.ma` `M-S.T`
  * *bemerk* `be.m`
* **mess** `ess` `ess.e` `ess-er`
  * -*messer* `(er)` → hoch strom
  * **miss** `.iss`
    * *miss* `m.iss`
  * **maß** `ass` `N.i-er.ass.en` `G.i-ass` `(I)ho.ass.e`
    * **mäß** `ess` `ess.i`
      * -*mäßig* `.i`/`.ess.i` → verhältnis regel @-mäßig
        * *gesetzmäßig* `-G.ess.i`
      * -*gemäß* `.G` → fahr sitz
* *metall* `m.et` `m.et.i`
* *meth****od*** `(.od)` `(.od).e`
* metr `m.etr` `m.etr.om` `m.etr.um` `m.etr.ik` `m.etr.i`
  * -metrie `m.etr.I` → ge sym
  * meter `m.et-er` `pa.m.et-er`
    * parameter `p.ar.a-m.et-er`
* *mexik* `m.ex` `m.ex-er` `m.ex.i` `m.ex.o`
  * *mexiko* `m.ex` @-o
* *mich* `m.i` `.ass.m.i`
  * *lassen.sie.mich* `ass.m.i`
* milch `m.ich` `m.ich,(.e)`
* *million* `(I),(I)` `(I),(I).är` `(I),(I).en`
* min `m.in` `m.in.e` `UNTER.m.in.ir`
  * *mine****ral*** `(r.al)`
* **min** `mi` `mi.im.al` `mi.im.um` `mi.or.ät` `(o)mi.eNT` `(o)mi.ez`
  * *minim* `m.in` `m.in.al` `m.in.um`
  * *minor* `m.in` `m.in.ät`
  * *min****ut*** `.ut` `.ut.e` @u
  * *prominent* `(o)mi`
* *minist* `mi` `mi.er` `mi.I.en` `mi.um` `mi.at` `mi.iaL.at` ← min
  * *ministerien* `mi.er.I.en`
  * -*minister* `(er)` `(r),(er)` `(r)h,(er)` → aus bund finanz in just kult wirt
  * -*ministerium* `.um`
* mind
  * *minder* `m.i-er`
  * *mindest* `m.i-eS`
* mir `m.ir`
  * .***mir*** `.ir` `(I)-bi.ir,u`
* *miss*- `ms`
* *miss* `ms` `ms` `ms.ar` `ms.ir` `.e-ms` `ko+ms.ar` `ko+ms` `kom.o+ms` `tra.ms` ← mitt @-sion
  * *kompromiss* `kom`
  * *kommiss* `ko-i` `ko-i.ar` `ko-i` @-sion
    * -*kommission* `ko` → atom
* *mit* `M` `M.ei-ad-er` `.a-M` `hi:M`
  * *mitt* `M` `M.e` `M.el`
    * *mittel* `-t.el` `UN-t.el.bar` `er-t.el` `G.uss-t.el` `hi-t.el` `n.är-t.el` `ar-UNG-t.el` `E.er-t.el`
      * *unmittelbar* `UN.bar`
  * *damit* `a-M`
* *mitt* `M` `.e-M.ir` `kom.o+M.ir`
  * emittier `.e-m.it.ir`
  * *kompromitt* `kom.it` `kom.it.ir`
  * → miss
* mob `m.ob`
  * mobil `m.ob.il` `.aut.m.ob.il` `m.ob.ar` `m.ob.ir` `m.ob.ät`
    * *automobil* `.aut.il`
* ***mocht*** `.ocht` ← mög
* mod `m.od` `m.od.e` `m.od.al` `m.od.al.ät` `m.od-er.or` `m.od-er.iO` __`m.od-er.ir`__ `m.od.if-o` `m.od.if.ir` `m.od.ul` `m.od.ul.ar` `m.od.ul.iO` `m.od.us` @-isier @-ifizier
  * modern `m.od-er` `UN.m.od-er` `m.od-er.ir`
* *modell* `m.o-ell` `m.o-ell.ir`
* ***mög*** `.ög`
  * *mög****lich*** `(L)` `(L)-t` `(o)ba.L` `(o)w.T.L` @-st
  * ***mag*** `.ag`
    * → macht
  * → mocht
* *mom****ent*** `(met)` `tr.eg.H,(met)` ← mov
* *mon****at*** `(.at)` `(.at).L` `.im (g)+(.at)` `(I)let,(.at).en` `(I)n,(-st),(.at).en` `v (.at) Z (.at)`
  * *von.monat.zu.monat* `v.at.at`
* mono- `m.on.o`
* -**monstr**- `mo` `most.er` `mo.um` `mo.ät` `d.e-mo-o` `d.e-mo.ir`
  * *monstr* `mo-s` `mos.um` `mos.ät`
* mont `m.ot` `mot` `m.ot.asch.e` `m.ot.ir` `m.ot.ör`
* -**morg**- `mo` `m.og` `mo-s` `Ü.mo` `mo-Ü`
* mot `m.ot` `m.ot.iw-o` `m.ot.iv.ir` `m.ot.or` `.e-m.ot.iO` `.e-m.ot.i-aL` ← mov
  * *motor* `m.ot`
* mov → moment mot
* **müh** `mÜ` `be.mÜ.en`
* mult `mut` `mut.al`
* *münchen* `mÜ.en`
* mus `m.us` `m.us.e` `m.us.e-um` `m.us.i`
  * *museum* `m.us`
  * *musik* `mu.ik` `mu.ik-er` `mu.ik.aNT` `mu.ik.i` `mu.ik.ät` `mu.ik.i-en` @-ik
    * *musiz* `mu.iz` `mu.iz.ir` @-iz
* *müss* `yss` `yss-t.e` `dU,ysst.est` `.ir,ysst.et` `wr,yss` @y
  * *du.müsstest* `dU,ysst`
  * *wir.müssen* `wr.Yss`
  * *muss* `.uss` `.usst.e` `dU.uss` `dU.usst.est` `.I.uss` `wr.usst`
* *muster* `.ust`
* ***mut*** `.ut`
  * ***müt*** `.üt` `gr.üt.i` `ho.üt.i` `w.eich.üt.T`
* mutter `m.ut-er`
  * mütter `m.üt-er` `m.üt-er.L`

## [N](#wörterbuch)

* *nach* `ach` `.a-ach` `hi:ach` `ach.bar` ← nah
  * *nach*<i></i>*bar* `(bar)`
  * *danach* `(da)ach`
  * *näch****st*** `(-st)` `(-st).es` `(-st).e-s` `(m)(-st)` `Z(-st)` ← nah
    * *zunächst* `-Z-n`
* ***nacht*** `acht` `g.est.acht`
* nah `n.ah` `G.n.a-t`
  * *beinahe* `b.ei,(.e)`
  * näh `n.äh` `G.n.ä-t` `n.äh.er-er`
    * *näher* `n.äh` `G.n.äh` `n.äh.es` @-er
      * *annäher* `.an,(er)` `.an,(er).d`
    * → nächst
  * → nach
* nahr `n.ar` `n.ar.ha`
  * *nahrung* `ar-UNG`
  * **nähr** `.är` `n.är` `n.är,-t` __`er.är`__ `er.är-er`
* ~~nam~~ `(.e)` `(-t).L` `ach,(.e)` `.im (en)`
  * nam**haft** `(ha)`
  * *namentlich* `-t.L`
  * *im.namen* `.im.en`
  * ***näm*** `.äm` `.äm.L`
* **nat** __`na`__ __`na.iO`__ __`na.iO.ch.en`__ __`na.i-aL`__ __`na.i-al.ät`__ __`(I)t-er.na.i-aL`__ __`kut.ur.na.iO`__
  * *natio****nal*** `(n.al)` `(n.al).ist` `(n.al).iS.i` `(n.al).ät` ≠ regional
    * *international* `(I)t-er-a`
  * *nat****ur*** `(.ur)` `(.Yr).L`
* ***nau*** → genau
* naut `n.aut` `n.aut.ik` `n.aut.ik-er`
  * -*naut* `.aut` → aero astro kosmo
* neb `n.eb` `n.eb.ko` `n.eb,(.e)` `n.eb.L`
* neg `n.eg` `n.eg.ir` `n.eg.iO` `n.eg.iv`
* ~~nehm~~ `(.e)` `.ab,(en)` `AU,(en)` `.an.bar` `.an.L.T` `.an.G` `(T),(er)` `(.eil),(en)` `(.eil),(er)` `UN.an-G` `E,(en)` `E.L` `N.E,(en)` `O.n.em` `O,(-st)` `O.L` `O.w,(en)` `O.l.ib,(en)` `Ü,(en)` `UNTER,(en)` `UNTER,(er)` `UNTER,UNG` `wa,(en)` `wa,UNG` `IN.acht.en` `IN.auf.en` `AU.CH.en` @-nehm
  * *einvernehmen* `N.E`
  * ~~nehm~~ `Ü.en` `UNTER.en`
    * *unternehmer* `UNTER-er`
    * *unternehmung* `UNTER-UNG`
  * *genehm* `G.n` `G.n.i`
  * ~~n~~imm __`(.im)`__
    * *nimm* `.im` `dU .im`
    * ~~nimm~~t `(-t)` `.ab,(-t)` `N,(-t)` `Ü,(-t)` `O,(-t)` `Z,(-t)`
  * ~~nahm~~ `.an,(.e)` `.an,(en)` `AU,(.e)` `.S,(.e)` `.S,(en)` `.S.eis.e` `N,(.e)` `N,(en)` `ach,(.e)` `ass,(.e)` `(.eil),(.e)` `Z,(.e)`
    * *nahm* `n.am`
  * ~~n~~omm `(.om)` `.an.om` `AU.om` `Ü.om` `E.om` `O.om` `IN.auf.om`
    * *vor*ein*ge*~~nommen~~ `O.N.G`
    * *un*<i></i>*vor*ein~~genommen~~ `UN.O.N`
* -**nenn**- `n` `n.en` `n.en,(-t)` `be.n` `er.n` @nennenswert und nennwert (kein problem)
  * *benenn* `be.n.en`
  * -**nann**- `na` `n.ant.e` `G.na-t` `be.na-t` `er.na-t`
    * *genannt* `G.n.ant`
    * *wurde.ernannt* `(U),er`
* **neu** `neu` `neu-st.e` `neu-er.L` `neu=H` __`neu-ig.T`__ `neu-L`
* neutr `n.eutr` `n.eutr.a` `n.eutr.um` `n.eutr.on` `n.eutr.ir`
  * *neutral* `n.eutr` `n.eutr.ir`
* *nicht* `-t` `ga-t` `-t.r`
* -***nis*** `.is`
* nie `n.i` `n.i-ma-s` `n.i-ma`
* *nieder*, *niedr* `ndr`
* *nieder***land**e `ndr.la-e`
* ***nieß*** `.iss` `G.iss` `nu:iss.er`
  * ***noss*** `.oss` `G.oss` `G.oss.FT`
    * *genossenschaft* `(O).FT`
    * -*genossenschaft* `FT` → konsum
  * ***nuss*** `.uss` `G.uss`
    * ***nüss*** `.üss` `G.üss`
  * → nutz
* *niveau* `.iw.o` `.iw.o-l.o` `kust.iw.o` ← nivell
  * *niveau*los `.iw.o-l.os`
* **niv**ell `n.iw` `n.iw.ir-UNG`
  * → niveau
* *noch* `n-ch` `n-ch:ma-s`
  * *nochmals* `n-ch-s`
* nom `n.om` `n.om.i-aL`
  * -***nom*** `.om` → astr aut gastr metr ök
* nord `n.od`
* *nordrheinwestfalen* `n.o-fa`
* norm `n.om` `n.om.al` `.e-n.om`
* ***not*** `.ot` `(UN.ng)-t .ot.w.i`
  * *unbedingt.notwendig* `UN.be.o-w.i`
  * ***nöt*** `.öt`
* ***not*** `.ot` `.ot.e` `.ot.ir` __`.ot.if-o`__ `.ot.ar` `.ot.ar.at` `.ot.ar.ieL`
  * *not****iz*** `(.iz)`
  * *notariell* `.ot.ieL`
* ***nug*** → genug
* **numer**, **nummer** `nu` `nu.er`
* **nun** `nu`
* *n****ur*** `(.ur)`
  * -*nur* `-r` → aber nicht
* *nürnberg* `nÜ.b`
* nutz ← nieß
* **nutz** `nu`
  * **nütz** `nÜ`
    * -*nützig* `-Ü.i` → eigen gemein

## [O](#wörterbuch)

* -o @-o
  * -o**graph** `-o+gra` `b.ibl.i-o+gra.I` `d.o+gra.I` `g.e-o+gra` `st.en.o+gra`
* ob `.ob`
  * oben `.ob` ← ob
  * ober `.ob-er` ← ob
    * *ober-* `.o-` → gericht haupt
* oben ← ob
* ober ← ob
* oder `.od-er` __`.od-er,au`__
* off `.of` `.of,(z).i`
  * *off*- `.o-` `.o-bar` `.o-H`
  * *öffn*, *öffent* `.ö` `.ö` `.ö-L` `.ö-L.T` `er.ö` `E.ö-L`
* offiz → faz
* oft `.oft` `.oft.ma-s` `.eb.o-oft`
  * *oftmals* `.oft-s`
  * *öfter* `.ö` `.ö-s` `(.es).ö`
* *ohn*, *ohne* `.o°`
* ohr `.or`
* ök `.ök` `.ök.om`
* opt `.opt` `.opt.ir` `.opt.iO`
* *optim* `.op` `.op.al` `.op.iS` `.op.ist` `.op.iS.i` ← opt
  * *optimal* `.opt.im-a`
* ord `.od` `.od.L` `.auss.od.L` `.od.in.är` `.ab.od.et`
  * *abgeordnet* `.ab.G`
  * *außerordentlich* `.auss.L`
  * -*ordnung* `.o` → gemein
* org `.org` `.og.an` __`.og.i`__ `.og.iO` `.og.ir` `.og.iS.en`
* *orient* `.or` `.or.ir` `.or.ir-UNG`
* *orig* `.og` `.og.i-aL`
* orn `.on` `.on.eNT` @-ament
* ort `.ot`
  * ört `.öt` `.öt.L` `er.öt-er`
* ost `ost` `w.ost`
* *österreich* `.ö` `.ö.i`
* oxid __`.ox.id`__ `.ox.ir` `.ox.id-o` ← oxyd
* *oxyd* `.ox` `.ox.iO` `.ox.ir` ← oxid

## [P](#wörterbuch)

| [PR](#pr) |
|:-:|

* **paar** `pa` ← par
* -**pack**- `pa` `p.ak` `(E)pa`
  * -**päck**- `p` `p.ch.en` `G.p`
    * *gepäck* `G.p.ek`
* -*pakt*- `pat` `p.akt` `pat.ir`
  * -*pakt* `pa` → greif
* pand `p.ad`
  * → pass
* ***pans*** `pas` `.e-pas-o` ← pand @-sion
* **pap**, **papp** `pa` __`pa-e`__ `pa.ir` `(ar).et:pa.ir`
  * *pappe* `p.ap.e`
* **par** `pa` `pa.ad.e` `pa.ad.ir` `pa.ad.is` `pa.ät` `.a-pa.at` `.a-pa-ur` `(r)pa.ir` `(r)pa-ur` `tra.pa.eNT` `tra.pa.ez`
  * parität `p.ar.ät`
  * → paar
* *paris* `pa.is`
* *para*- `pa-`
* *parallel* `p.ar.el`
* *paris* `pa.is`
* parl
  * *parlament* `Par` `Par.ar-er` `Par.ar.i`
    * *parlamentarisch* `Par.i`
* part `pat` `pat.I` `pat.ik.el` `pat.ik.är` `pat-er`
  * *partei* `pa-ei` `pa.ei-Sa.UNG`
  * -*partei* `.ei` `I.ei-en` → krat union
* ***pass*** `pass` `pass.ir` `pass.us` ← pand
* pass ← pati
* **p**at `p` `kom.ib.el` `kom.ib.ät`
* *patent* `pa°` Neubauer `pa°.a`
* pati `p.az` `p.az.eNT` `p.az.eNT-in`
  * ***pass*** `pass` `pass-o`
* **pausch** `pau` `pau.al` `pau.al.e` `pau.al.ir` @-isier
* paz `p.az` `p.az.ik` `p.az.ist` `p.az.iS` @-ifisch
  * → pakt
* pend `p.ed` `p.ed.el`
  * pend `p.ang` `p.ang.d.ang`
    * *pendant* `p.ang.da`
  * pens `pe-s` `pe-s-o` `kom-s.ir` `kom-s-o`
    * *pension* `pe-s.iO` @-sion
* **per**- `p`
* perg `p.eg` `p.eg.eNT`
* *peri*- `p` `p.od.e` `p.od.ät`
  * -*periode* `.od.e` → übergang
* *person*, *persön* `p` `p.al` `p.al.ko` `p.L`
* **pfleg** `pfl`
  * **pflog** `pflo`
* ***pflicht*** `.icht` `.icht,u` `.icht,i`
  * -*pflicht* `i` → haft steuer wehr
* phon `f.on` `f.on.ik` `f.on.i`
  * -ph~~on~~ `f` → grammo tel
* phras `fr.as` `fr.as.o-I`
* **phys** `fy` `fy.ik` `fy.ik.i` `fy.i`
  * phys**isch** `f.ys.i`
* pl → komplement komplett implement
* **plan** `pla`
  * -***plan*** `.an`
* **plant** `pla` `pla.asch.e` `tra.pla.O`
* ***platz*** `.az` `Ts.az` `si:az`
  * ***plätz*** `.ez` `.ez.e`
* plaus `pl.aus` `pl.aus.ib.el` `pl.aus.ib.ät`
* plik pliz plex → komplik
* plod → explod
* plos → explos ← plod
* ***plötz*** `.oz` `.oz.L`
* po `p.o`
  * poesie `p.o-es.I`
* pol `p.ol` `m.etr.p.ol.e`
  * *metropole* `m.etr.ol.e`
* *polit* `it` `it.i` `it.i-er` `it.ir` `it.og.e` `it.o-I` `.auss.it.i` `fi.it.i` `.in.it.i` `k.osm.it` `k.osm.it.iS`f`
  * *politik* `tik` `tik-er` `tik.um` `.auss.tik` `bi.tik` `fi.tik` `ha.tik` `eu-er.tik` `wä-tik` `FT.tik`
    * **hand**els*politik* `ha.el-t.ik`
* *poliz* `li` `li.ei` `li.ist` `(.es),li.ei`
  * *landespolizei* `(.es).l.ei`
* ***pon*** `.on` `d.e-on.ir` `ds.on.ir` `kom.on.eNT.e` `kom.on.ir` `tra.on.ir`
  * deponier `(d.e)p.on.ir` @depot
  * *komponente* `kom.o-nt.e`
  * *posit* `pos` `pos.iv`
    * *position* `pos` `(ds)pos` `kom-os`
      * *disposition* `ds-os`
      * *komposition* `kom-o`
    * → post
* port `pot` `pot.al` `(d)pot.ir` `(o)pot.iO` `UN.o-pot.iO.irt` `(o)pot.i-aL` `tra.pot`
  * *proportional* `(o)pot.al`
  * *unproportioniert* `UN.o-port.irt`
  * → import
* **post** `po` `po.al.i` `po.at` `po.ir` `po.a` ← pon
  * postier __`p.o+st.ir`__ ≠ postulieren
  * postulier `p.o+st.ul.ir` ≠ postieren
  * *postulat* `p.o+st.at`
  * *kom***post** `kom.ost` `kom.o+st.ir`
  * → depot
* puber `p.ub-er` `p.ub-er.ir` `p.ub-er.ät`
* *publ* `p.ub` __`p.ub.ik`__ `p.ub.ir` `p.ub-o` `p.ub.um` `p.ub.ist` `p.ub.iS.i` `p.ub.ät` `(r)p.ub.ik` `(r)p.ub.ik-er` `(r)p.ub.ik.i` `bu:(r)p.ub.ik` @-ikation
  * *publikum* `p.ub`
  * *republik* `r.ik` `r.ik-er` `r.ik.i` `bu:r.ik`
    * -*republik* `.ik` → bund
* *punkt*, *pünkt*, pt. `pt` `(I)t-er.pt.iO` `(ko)tra.pt.i` `(ko)tra.pt.iS.i`
  * -p**u**nkt `u` → hoh kontr sicht stand streit
* put `p.ut` `.am-p.ut.iO` `kom.ut-er` `(ds)p.ut`
  * *amputation* `.am-p-o`
  * *computer* `kom-u`
  * *disput* `d.i+sp.ut`
  * → kont

### [PR](#wörterbuch)

* **prä-** `pr`
* ***präm*** `.äm` `.äm.I.e` `.äm.I.en` `.äm.ir` `.äm.ir-UNG`
  * *prämie* `.äm.e`
  * *prämien* `.äm.i-en`
* *präsid* `pr` `pr.eNT` `pr.ir` `pr.iaL` `pr.um` ← sid
  * *präsident* `pr` `pr.in` `pr.FT`
* **prax** `pra` `pra.en` `pra.is`
  * **prakt** `pra` `pra.i` `pra.ik.en` `pra.ik.aNT` `pra.ik.aNT.in` `pra.ik-er` `pra.ir`
  * pragm `pr.agm` `pr.agm.i` `pr.agm.ik-er` `pr.agm.iS`
* präzis @-sion
* *pr****eis*** `(.eis)`
* **press** `pr` `pr-e` ← prim
* prim → press
* prinz → prinzip
* prinzip → zip
* *prior* `pri` `pri.ät`
* *privat* `pri` `pri.ir`
* *privileg* `pri.eg`
* pr**o**- `o`
* **prob** `.ob` `.ob.e` `.ob.ir` `G.obt` `er.obt` `g.ob.e` `hau.ob.e`
  * *generalprobe* `g.en.ob.e` ≠ gegenprobe
* *prob****lem*** `(l.em)` `(l.em).ik` `(l.em).i`
* **prüf** `prÜ` `(bst)-Ü.UNG` (Ü)prÜ` `(en),fa-Ü-Ü`

## [Q](#wörterbuch)

* qual `qu.al` `qu.al.ät` `qu.al.if.ir` @-ifizier
* quant `quat` `quat.en` `quat.um`
  * *quantit* `(.an)` `(.an).ät` `(.an).iv`
* **quell** `qu` `f-er.qu.e`
  * **quoll** `quo`

## [R](#wörterbuch)

* rad `r.ad` `fa.r.ad` `sp.r.ad`
  * *fahrrad* `fa.ad`
  * *gerad* `-G.a` `-G.a.e`
* rad `r.ad` `r.ad.al` `r.ad.iS`
* *raffin* `r.af` `r.af.irt`
* rag `r.ag` `.e-O.r.ag.ed`
  * *hervorragend* `.e-O.ed`
* rahmen `r.am` `.im.r.am`
  * im.rahmen `.im.am`
* *rak****et*** `(.et)` `(.et).e` `tr.eg-er,(.et).e`
* rang `r.ang` `r.ang.ir` `O.r.ang`
* ras `r.as`
* **rat** `ra` __`ra.g`__ `ra.os` `ra-Sa` `G.ra` `be.ra` `er-ra` `O.ra` `AU-i.ra` `bu.ra` `la.ra` `.a-v.ab.ra` @da-
  * **rat**sam `ra-s.am`
  * -*rat* `.at`
  * *rät* `.ät` `.ät.e` `.eib.ät` `o=ba.ät` `di.ir.ät` `(.es),E.ät-er`
    * *landesverräter* `(.es).ät-er`
    * *rätsel* `t-s.el` `t-s.el.ha`
  * riet `r.iet`
* **rat** `ra` `ra-e` `ra.if-o` `ra.if.ir` `ra.iO` `ra.iO.ir` `ra.iO.ir-UNG` `ra.i-aL` `ra.i-aL.ir` `ra.i-aL.ir-UNG` `ra.i-aL.iS.i` `ra.iO.ir` `ra.i-eL` `IN 4 ra.en` @-istisch
  * *rate* `ra.e` ≠ rat
  * *rationalisierung* `ra.ir-UNG`
  * *rationier* `r.at.ir`
* rätsel ← rat
* rauch `r.auch` `r.auch,eu-e`
  * -*rauch* `rau` → zigarr
* ***raum*** `.aum` `h.ub.aum` `wet.aum`
  * *weltraum* `w.aum`
  * ***räum*** `.äum` `.äum.e`
* raup `r.aup` `r.aup.e` `r.aup.eug` `r.aup,(er)`
* **re** `r` `r.al` `r.al.ir` __`r.al.ät`__
* real ← re
* *rech* `ech` `ech.et.e` `ech-er` `ech-er.i` `G.ech` `(.S)ech` `(be)ech` __`(er)ech`__
  * *er*rech `(er)r.ech`
* ***recht*** `echt` `echt.L` `Ts:echt` `(.es):echt` `m.H,wa.echt`
  * *recht* `r` `AU-r` `G.r` `O.r`
* red `r.ed` `r.ed.e` `r.ed-er`
* reg `r.eg` `.an.r.eg.UNG` `be.S.o:er-r.ed`
* **reg** __`r`__ __`r.är`__ __`r.iv`__
  * *reg****el*** `(.el)` __`(G)(.el)-t`__ __`(.el).os`__ __`(.el).i`__ __`(.el).i-T`__ `(.el)-UNG` `.il,(.el)`
    * *in.der.regel* `IN.el`
  * *reg****ier*** `(.ir)` `(.ir)-UNG` `(.ir)-UNG.ik` `(.ir)-UNG:gra` `bu,(.ir)-UNG` `(.es),(.ir)-UNG`
    * *regierung* `ir-UNG` `ir-UNG.ik` `ir-UNG.gra` `bu,ir-UNG` `(.es),ir-UNG`
      * *bundesregierung* `u-r-UNG`
      * *landesregierung* `(.es).ir-UNG`
  * *reg****ion*** __`(.iO)`__ `(.i-aL)`
  * → rig
  * → rekt
* ***reich*** `.eich` `.eich.L` `be.eich` `er.eich` `Ü.eich`
  * *reichlich* `r.ei-L`
  * *erreich* `(er)r.ei`
  * -*reich* __`r.ei`__ → fang folg hilf kind list trag
* reif `r.eif`
  * -*reife* `.eif.e` → schul
* reih `r.eih` `.ei-e r.eih.e v`
  * *eine.reihe.von* `.ei-v`
* rein `r.N`
* **reis** `r.ei` `r.ei-e`
* -***reit***- `T` `r.T` `be.T` `O.be.T` `be.Ts`
  * *bereits* `Ts`
  * *vorbereiten* `O.b`
  * -***ritt***- __`.it`__ `r.it`
* **rekt** `r` ← reg
  * `d.i-r` `IN.d.i-r` `d.i-r.iO` `d.i-r.iv.e` `d.i-r.or` `d.i-r.or.in`
  * `r.or` `ko.r.or` `r.or.at` `r.or.ko` `k.or-r` `k.or-r.H` `k.or-r.ur`
  * *direkt* `di` `IN,di` `di.iO` `di.iv.e` `di.or` `di.or.in`
    * *indirekt* `IN.d-t`
  * *rektorat* `r.ekt.at`
  * *konrektor* `ko.r.ekt.or`
  * → adress
* *relat* `r` __`r.iO`__ `r.iv` `r.iv.ät` ← lat
  * *relativität* `r.ät`
* relig __`r.l.ig`__ __`r.l.ig.iO`__ __`r.l.ig.ös`__ __`r.l.ig.os.ät`__ ← lig ≠ __relation__
* renn `r.en`
* **respond** `r` `k.or-r` `k.or-r.eNT` `k.or-r.ez` `k.or-r.ir`
  * *korrespondenz* `k.o-ez`
* rent `ret` `ret.ab.el` `ret.ab.ät` `ret.e` `ret.ir` `ret.er`
  * *rentabel* `r.et.ab`
* *revier* `r.ir` `li.ei-r.ir`
* *rheinländisch-westfälisch* `(sch).(sch)`
* *rheinland-pfalz* `(N):pfa`
* **richt** `ri` `ri.i` `ri-UNG` `be,ri,(er)` `be,ri,er.sta*` `(be)ach,ri.i` `(er)ri` `IN.iS-er ri-UNG`
  * richten `r.icht` ≠ der
  * *richtung* `i-UNG` ≠ regierung
  * be**richt** `be,ri`
    * *berichterstatter* `be,ri-er`
    * *berichterstattung* `be,ri-UNG`
  * *in.dieser.richtung* `IN.iS-er-UNG`
  * *gericht* `G,i` `G,i-L` `la,i` `.o-la,i`
    * *gerichtlich* `(G)i-L`
    * **amt**s*ge***richt** `.a,i` `.amt,i`
* **r**ig `r` `d.i-r` `d.i-r.eNT` `d.i-r.ir` `d.i-r.iS.i` `k.or-r.ir` ← reg
  * korrigier `k.or-r.ig.ir`
  * *dirigent* `di.eNT`
* rind `r.id` `r.id-er` __`r.id.ih`__
* -***ring***- `.ing` `r.ing` `G.ing` `E.ing`
* *risik* `ri.k` `ri.k.o` `ri.k.en`
  * *risiko* `ri.ko`
* roll `r.oll` `.ei r.oll.e .il`
  * *eine.rolle.spielen* `.ei,o,i`
* rom `r.om` __`r.om.i`__
  * röm `r.öm` `r.öm.i`
* roman `r.om.an` `r.om.an.i+st.ik` `r.om.an.iS.i`
  * *romant* `r.o+mat` `r.o+mat.ik` `r.o+mat.isch`
* **rück** `rÜ` `rÜ-t.ig`
  * -*rück* `-Ü`
  * *zurück* `u-rÜ` `dar-AU u-rÜ.ür`
    * -~~zurück~~- `dar-AU-Ür`
* **rud** `ru` `ru-er` `ru-er.N`
* -***ruf***- `.uf` `r.uf` `AU.uf` `.S.uf` `b.uf`
  * *aufruf* `AU-u`
  * -***rief***- `.if` `r.if` `.S.if`
* *rump*, *rupt* `r.up` `.e-r.up-o` `.e-r.up.iv` `k.or-r.up.ir` `k.or-r.up-o`
  * eruptiv `.e-r.upt.iv`
  * → bankrott
* **rund** `ru`
* *russ* `russ`

## [S](#wörterbuch)

| [SCH](#sch) | [SP](#sp) | [ST](#st) |
|:-:|:-:|:-:|

* **sach** `sa` `sa-e` `sa.en` `sa.G` `sa.L`
  * -~~sach~~ `.ur,(.e)` `E.ur,(en)` → form haupt neb schmack tat
  * *sache* `(.e)` `(en)`
  * *sachlich* `s.ach.L`
  * *verursach* `E.ur-sa`
  * **säch** `s` `s.L` `.ur-s.L`
    * *sächlich* `s.ech.L`
    * -*sächlich* `.L` `.ur.L` → haupt neben
* **sachs** `sa` `sa.en` `ndr.sa.en`
    * *niedersachsen* `ndr-a`
* -**sag**- `sa` `s.ag` `s.agt.e` `s.ag.e` `G.sa` `.ab.sa` `.an.sa` `.an.sa.er` `.S.sa` `.S.sa-e` `.eug.S.sa-e` `be.sa` `eNT.sa.Ol` `.os.sa` `ach.sa` `UN.sa.bar` `UNTER.sa` `E.sa` `E.sa.er` `O.S.sa` `O.h.sa` `w.et-er.O.h.sa` `Z.sa`
  * ansager `.an.s.ag-er`
  * *ver*sager `E.s.ag-er`
  * *aussage* `.S.sa.e`
  * *wettervorhersage* `w.et-er.sa`
  * *zeugenaussage* `.eug.S`
  * -**säg**- `s` `UN.s.L`
    * *unsäglich* `UN.s.eg.L`
* säg `s.äg` `s.äg.e`
* *saison* `s.es` `s.es.al`
* sait `s.ait` `s.ait.e`
* sakr `s.akr` `s.akr.al` `s.akr.eNT`
  * *sakrament* `s.akr`
* -sal `s.al` → scheu schick @scheu
* -*sam* `Sa`
* *samm* `Sa` `Z.Sa`
  * *zusamm* __`u-Sa`__
* **samm**el `sa.el` `sa.l-UNG` ← samm
  * *versammel* `E.Sa` `E.Sa.UNG`
    * -*versammlung* `Sa.UNG` → glied
* *samt* `Sa` `G.Sa` `G.Sa.H` `IN.G.Sa`
  * *insgesamt* `(I).G.Sa`
  * *sämt* `s` `s.L`
* samt `s.amt`
* sand `s.ad`
* ***sass*** `.ass` `IN.ass.en` ← sitz
* saß `s.ass` ← sitz
* sattel `s.at.el` `s.at.l.er` `s.at.l.ei`
  * sättel `s.ät.el`
* satz ← sitz
  * -**satz**- `sa` `s.az` `.ab-sa` `N-sa` `er-sa` `.ad.er-sa` `g.sa` `.um-sa` `Z.sa` `.im:g.sa.Z`
    * *satzung* `.az*` `.az*.en` `.az*.-G` `.az*.i`
      * *satzungsmäßig* `.az*.ess.i`
    * *gegensatz* `g-sa`
    * *schadenersatz* `.ad.er`
    * *im.gegensatz.zu* `.im.Z`
    * -**sätz**- `s` `s.ez.e` `.ab-s.e` `g.s.e` `g.s.L` `gru.s.L` `.um-s.e` `Z.s.e` `Z.s.L` @s statt sä
      * *gegensätz* `g-s` `g-s.e` `g-s.L`
* sau `s.au`
* sauber `s.aub-er`
* -*säul* `eu` `eu-e` → rauch wirbel
* saum `s.aum`
  * säum `s.äum` `E-s.äum.is`
* seel `s.el` `s.el.e` `s.el.i` `s.el,o-e`
  * seelisch `s.el.isch` ≠ selig
* seh `s.eh` `UN.E.s.eh.ens`
  * *unversehens* `UN.E-s`
  * *fernseh* `f-s` `f-s-er`
  * sieh `s.ih`
  * sah `s.ah`
  * → sicht
* **sehr** `s`
* sei ← sein
* seid `s.eid` `s.eid.e`
* *s****ein*** `(N)` `d.a-(N)`
  * **bin** `bi`
  * **bist** `bi`
  * **i**st `i`
    * -*ist*- `i°` → das es
  * *sind* `(I)`
    * -*sind*- `-I` → wir
  * *s****ei*** `(.ei)` `(.ei)-d`
  * *w****ar*** `(.ar)`
    * *w****är*** `(.är)` `(.är).e`
  * ***wes*** `.es` `.es.a` `.es.ha` `.es.H` `.es.L` `.es.os` `.ab.es.ed` `.ab.es.H` `.an.es.ed` `.an.es.H` `.im.es.L`
    * *gewes* `-G` `da.-G`
    * *im.wesentlichen* `.im.L`
* ***seit*** `T` `T.e` `T.en` `T.e-s` `T.ang` `T.L` `T,(.e)` `T.wets` `.ad-er.Ts` `be.T.ig` `b.ei-T.e` `.eid-er.Ts` `g.T.ig` `.ir-er.Ts` `rÜ.T.e` `AU-er .ad-er.en T.e` `AU.al.T.en` `AU.eid.T.en` `v.eid.T.en` `AU.iS-er T.e` `AU-er .ei-en T.e` `v.MR.T.en` `v,u-er-er.T.e`
  * seit `s.eit` `s.T.ang` `s.T.e-s` `s.T.wets` `b.ei-s.T.e` ≠ zeitlang bearbeite bereite
  * *auf.dieser.seite* `AU-i.T.e`
  * *auf.der.einen.seite* `AU.ei-en.T.e`
  * *auf.der.anderen.seite* `AU-a.T.e`
  * → seit
* seit `s.T` ← seit
* *sekretar*, *sekretär* `s.ekr` `s.ekr.at` `s.ekr.in` `g.s.ekr` `g.s.ekr.at`
  * *generalsekret* `g-s` `g-s.är` `g-s.at`
* sel `s.el` `s.el.i` `s.el.i-er` `s.el.ist.e` `s.el.i-T`
  * selig `s.el.ig` ≠ seelisch
  * -*seli****g*** `(g)` `UN,(g)` → fühl glück trau
* selb `s.eb` `s.eb-er` `s.eb.i`
  * -*selb* `l` `d.l.e` `(m).l` `(n).l` `(r).l.e` `(.es).l` `I.l.e`
  * *selbst* `-st` `-st,u-N` `-st.ko` `-st.l.os.i-T`
* selt `set` `set.en` `set.Sa`
  * *seltenheit* `s.H`
* *sem****ester*** `(.est.er)`
* *semi****nar*** `(n.ar)` `(n.ar).iS.i` @-istisch
* sen `s.en` __`s.en.il`__ `s.en.or`
  * *senat* `s.at`
* send `s.ed` __`s.ed-er`__ `.ab.s.ed` `N.s.ed`
  * -*sender* `-s-er` `.ab-s-er` `N-s-er`
  * sand `s.ad` `s.adt.e` `G.s.adt`
* sent `set` `pr.set` `pr.set.ir` `(r)pr.set.aNT`
  * *präsent* `pr.s.eNT`
    * *repräsentant* `(r)pr.aNT`
  * sens
  * senz `s.ez` `pr.s.ez`
* sent __`set`__
  * sens `s.e-s` `s.e-s-o`
* **sequ** `s` `s.ez` `ko-s.eNT` `ko-s.ez` `ko-s.ez.en .ih`
  * *sequenz* `s.equ`
  * *konsequent* `ko-s`
  * -*konsequenz*- `ko-s` `ko-s.en.ih`
  * → exekut
* **ser** `s` `IN.s.at` `IN.s.ir`
* serv `s.ew` `s.ew.ir`
  * *konserv* `ko-s` `ko-s.e` `ko-s.iv` `ko-s.ir`
    * *konserve* `ko-s`
* -**setz**- `s` `s.ez` `s.ezt.e` `s.ez.er` `s.ez.er.ei` `.ab.s` `be.s` `N.s` `er-s` `UN.er-s.L` `fr.s` `eNT.g.s` `O.S-s` `UNTER-r,O-S-s` ← sitz @-erei
  * *entgegensetz* `eNT.g-s`
  * *fortsetz* `fr-s`
  * *gesetzt* `G.s.ezt`
    * -*gesetzt* `-G` `N.-G` `fr-G` `O.S-G`
      * *eingesetzt* `N.-G-s`
  * -*setzung* `s.ez*` `.S.ei-ad-er.s.ez*` `be.s.ez*`  `fr.s.ez*`
    * *auseinandersetzung* `.S.ei-ad-er`
    * *simultanübersetzung* `s.im.an-Ü`
  * *gesetz* `-G` `-G.e` `-G.L` `-G.os.i-T` __`-G.i`__ `-G,i` `gru.-G` `(.es),-G` `UN-G.L` @-mäßig
    * *grundgesetz* `GR`
* **seuch** `seu` `seu-e` `eNT.seu` __`E.seu`__ ← siech
  * *verseuch* `E-seu`
  * -*seuche* `eu`
* *sich* `CH` `ha.CH`
  * *hat.sich* `ha°`
* *sicher* `s.i-er` `s.i-er-er` `s.i-erst.e` `s.i-er-UNG`
  * ~~sich~~er*lich* `(er).L`
  * ~~sich~~er*heit* `(er).H` `(UN.er).H`
  * *versicher* `E-i` `ha,i,E-i`
    * *haftpflichtversicherung* `ha,E-i`
* **sicht** __`si`__ ← seh
  * `si.bar` `si.L` `.an,si` `.S,si` `.S,si.os` `N,si` `UN.N,si.i-T` `O,si` `UN.O,si.i` `Ü,si` `.of,si.L`
  * `G,si` `an-G,si` `.ab,si` `.ab,si.L` `be.ab,si.i` `AU,si` `be.AU,si.i` `be,si.i` __`si,u`__
  * `rÜ,si` `rÜ,si.os` `rÜ,si,(.e)`
  * -*s****i****cht*
    * ab- `.ab-i` `.ab-i.L` `be,.ab-i.i`
      * *beabsichtig* `be.ab`
    * *auf*<i></i>- `AU-i` `AU-i-Ü.ed` `be,AU-i.i` `be,AU-i.i-t.e`
      * *beaufsichtig* `b,AU-i` __`b,AU-i-t.e`__
        * *beaufsichtigte* `b,AU-i.i-t.e`
    * *ge*<i></i>- __`G,i`__ `G,i,u` `an-G,i`
    * *zu*<i></i>*ver*<i></i>- `Z.E-i` `Z.E-i.L` `Z.E-i.L.T`
  * -~~sicht~~
    * an- `(I)bi.an` `wr-I.an` `m.ei-an:ach` `ach.m.ei-an` `ach.U.an`
    * *aus*- `O.S.L` `IN.S.st` `IN.S.ell`
    * *ge*- `G,u`
    * *hin*- `hn` `hn.L` `IN.iS:hn` `(I)j.hn` `(IN)aCH.hn` `IN.ech.hn`
    * **rück**- `rÜ` `rÜ.os` __`be.rÜ.i`__ `wr be.rÜ.i-t` `UNTER be.rÜ.i`
      * *berücksichtig*en `be.rÜ`
      * *rücksichtnahme* `rÜ.icht,(.e)` ≠ rücknahme `rÜ,(.e)`
    * *über*- `Ü.L` `Ü.L.T` `UN-Ü-L`
  * -*sicht*- `.icht` `.icht.bar` `.icht.L` `.an.icht` `.S.icht` `.S.icht.os` `N.icht` `UN.N.icht.i-T` `O.icht` `UN.O.icht.i` `Ü.icht` `.of.icht.L` `.ei-e-Ü.icht-Ü.n,and`
    * *offensichtlich* `.of,i-L`
  * -*r****ü****cksicht*- `M-Ü.AU` `M-Ü.d` `M-Ü.I` `M-Ü-n` `.o°-Ü.AU`
    * *unter*.*berücksichtigung* `UNTER-Ük`
* sid `s.id` `pr.s.id` `pr.s.id.ir` `pr.s.id.um` __`r.s.id.ez`__
  * → präsid
* *sie* `sI`
* siech `s.ich` → sucht
* *siedel* `s.i-el`
* silb `s.ib` `s.ib-er` `s.ib-ern` `s.ib-r.i` `E.s.ib-ert`
* silb `s.ib` `s.ib.e` `N.s.ib.i` `m.s.ib.i` `O.s.ib.en`
* *simul* `s.im` `s.im.aNT` __`s.im-o`__ `s.im.or` `s.im.ir` __`s.im-t.an`__
* sind ← sein
* sinn `s.in` `s.in.os` `s.in.Ol` `be.s.in` `be.s.in.os` `er-s.in` `G.s.int` `G.s.in.UNG.i`
  * *besinn* `be-s.in` `be-s.in.os`
  * *gesinn* `G.in` `G.int` `G.in.UNG.i`
  * -*sinn* `.in` `UN.in.i` `W.in.i` `.IN.iS.in.e` `.im.in.e` `.im.ei-L.in.e` → wahn
  * sann `s.an` `er-s.an`
  * sonn `s.on` `G.s.on` `be.s.on.H` `er-s.on`
* sist `s.i+st` `IN.s.i+st.ir` `ko-s.i+st.ez` `tra.i+st.or`
  * *exist* `.ex` `.ex.ir` `.ex.irt.e` `.ex.ir.ed`  `.ex.eNT` `.ex.ez`
    * *existenz* `.ex`
* sit `s.it` `s.it.ir` `s.it.iO`
  * *situation* `s.iO`
* **sitz** `si` `be,si` `O,si` `sp,si`
  * *vor*<i></i>*s****i****tz* `O-i` `O-i.er` `O-i.ed-er`
  * *besitz* `be.s.i` `be-si`
  * *sitzung* `.iz*`
  * → saß
  * → sass
  * → satz
  * → setz
* -**skal**- `ska` `sk.al.a` `sk.al.en` `.e-ska.ir` `.e-ska.iO`
* *so* `.o` `.eb.o`
  * *so*- `(o)` → eben gar
  * → sonst
* sol `s.ol` `s.ol.id` `s.ol.id.ät` `s.ol.id.ar.i` `s.ol.id.ar.ät` `ko-s.ol.ir-UNG`
* *solch* `(s)ch`
  * -*solch*- `o` → fall
* *soll* `(S)`
* solv `s.ow` `s.ow.eNT` `s.ow.ir` `.ab.s.ow.ir` `.ab.s.ow.eNT` `IN.s.ow.ez`
  * *absolv* `.ab-s.ow` `.ab-s.ow.ir` `.ab-s.ow.eNT`
  * *insolvenz* `IN.s.o`
  * **s**ol `s` `r.s.ut` `r.s.iO`
    * *absol* `ab` `.ab.ut` `.ab.iS` `.ab.iS.i` @-istisch
* *sommer* `s.om`
* son `s.on` `s.on.at.e` __`ds.s.on.az`__ __`ko.s.on.aNT`__
* *sonder* `S` `S.bar` __`S,au`__ `S.barst.e`
  * *besonder*, *besonders* `b°`
    * -*besonder*, -*besonders* `-S` → ganz in merk
* sonn `s.on` `s.on.e`
* *s****onst*** `(.ost)` ← so
* -*sorg*- `S.o` `s.og.e` `s.og.ed` `s.ogt.e` `s.og.os` `b.S.o` `b.S.o.is` `UN.b.S.o` `f.S.o` `f.S.o-L` `E.S.o` `E.S.o-er` `O.S.o` `O.S.o.L` `.a-f-S.o`
  * *versorgte* `E.s.ogt.e`
  * *versorger* `E,o-er`
  * *dafür.sorg* `.a-f.o`
  * -*sorg* `o` `f,o-e` `f,o-L` `.a-f,o` → falt seel
    * -*fürsorge* `-Ü.o` `I-Ü.o` → flucht sozi
* sort `s.ot` `s.ot.e` `s.ot.ir` `ko-s.ot.um`
  * sortiment `s.ot.met`
* *sozi* `SS`
  * *sozial* `SS` `SS-Ü.o`
* **sub** `su:`
* **such** `su` `.S.su` `be.su` `er.su` `UNTER.su` `E.su`
  * *unter*<i></i>*s****u****ch* `UNTER-u` `O.S.o=UNTER-u`
  * *ver*<i></i>*s****u****ch* `E-su`
  * *besuch* `be-su`
* **sucht** `su` ← siech
  * -*sucht* `u` -*sücht* `-Ü` → bleich
  * -*sücht*- `.ücht` `s.ücht.i` `s.ücht.i-er` `s.ücht.i-T` `.eig.ücht.i`
* süd `s.üd` `s.üd-Ü.e`
* **sul** `su` `ko-su`
  * sult `sut` `ko-sut.ir` `ko-sut.iO`
    * *re*sult `r-sut`
      * *result* `r-su` `r-su.ir` `r-su.at`
* **sum** `su` `ko-su` `ko-su.eNT` `ko-su.ir`
* **summ** `su`
* *sund* `.und` `G.und` `G.und.H`
  * *gesundheit* `u-H`
* super `s.upr`
* sym `s.ym` `s.ym.I` `s.ym.i`
* *sym****bol*** `(b.ol)` `(b.ol).ir`
* *syst****em*** `(.em)` `pr.iaL,(.em)` `(.em).ik` `(.em).i` `(.em).ir`

### [SCH](#wörterbuch)

| [SCHM](#schm) | [SCHN](#schn) | [SCHR](#schr) | [SCHW](#schw) |
|:-:|:-:|:-:|:-:|

* ***schad*** `.ad` `Z.ad.ko`
  * ***schäd*** `.äd`
    * **sach**<i></i>**be**~~schädigung~~ `sa.b`
* **schaff** `scha` `scha*`
  * **schaff***ung* `scha.UNG` ≠ krankheit @Eilschrift
* schaft `sch.aft`
  * -*schaft* `FT`
  * *beschäftig* `be-t.ig` `be-t.igt.e` `be-t.igt.en` `be-t.ig.l.os` `I n.eb.be-t.ig`
  * *geschäft* `GT` `GT.e` `GT.L` `GT.ch.en` `GT,eu`
* schall `sch.all` `.utra:sch.all`
  * *ultraschall* `.utra:scha`
* schalt `schat`
* schatt `sch.at`
* -**schatz**- `sch.a` `sch.az` `scha,(er)` `scha.S.el`
  * -**schätz**- `sch` `G.sch` `sch.ez.e` `sch.ez*` `sch.ch.en` `.ab.sch` `ho.sch`
    * *schätzchen* `sch.ez.ch.en`
* **schau** `.au` `G.au`
* ***scheh*** `.eh`
  * ***schieh*** `.ih`
  * ***schah*** `.ah`
  * ***schäh*** `.äh`
  * → schicht
* ***scheib*** `.eib` `.eib.e` `ba.eib.e`
* -**scheid**- `.eid` `sch.eid` `be.eid` `eNT.eid`
  * *scheid* `.eid`
  * *entscheid* `eNT.ei`
  * -**schied**- `.id` `sch.id` `.ab.id` `E.ab.id` `be.id` `eNT.id` `UNTER.id` `E.id` `(N),E.id.H` `M.al.eNT.id.H`
    * *meinungsverschiedenheit* `m.ei-H`
    * *unterschied* `UNTER-i`
    * *verabschied* `E.ab`
    * *mit.aller.entschiedenheit* `M.al.eNT`
* *sch****ein*** `(N)` `(N).bar` `.an.N.ed` `be.N.i` `er.N` `f.ür-er.N` `gu.N` `wa.N.L` `hö-t.wa.N.L`
  * *erschein*en `er.(N)` ≠ rein
  * *höchstwahrscheinlich* `hö-st.N.L`
  * *sch****ien*** `(.in)` `G.in` `er.in`
    * *geschien*en `(G)(.in)`
    * schien `sch.in`
* ***schicht*** `.icht` `G.icht.e` `kust-icht.e` `wet-icht.e`
* **schick** `.ik` `.ik-s.al` ← scheh
* ***schieß*** `.iss` `.iss.ei`
  * ***schoss*** `.oss` `G.oss` `.ab.oss` `er.oss` `Z.oss`
    * ***schöss*** `.öss` `.öss.ing`
  * ***schuss*** `.uss` `.S.uss` `Ü.uss` `Z.uss` `(r)-Ü.uss` `N-Ü.uss` `.S.fu-Ü.uss` `(K)-Ü.uss`
    * -*ausschuss* `.uss` → finanz haupt
    * ***schüss*** `.üss` `.üss.e` `.üss.el` `.S.üss.e` `Ü-üss.e` `Z.üss.e`
* **schiff** `.if`
* *schilli****ng*** `(ng)`
* schirm `sch.im`
* **schlacht** `schla` `schla.ih` ← schlag
  * **schlächt** `schlä` `schl.er`
* schlaf `schl.af` `.S.schl.af`
  * schläf `schl.ef` `schl.ef-er` `schl.ef-r.i`
  * schlief __`schl.if`__
* ***schlag*** `.ag` `.ag-er` `.ag-o` `G.ag` `.an.ag` `AU.ag` `ra.ag` `E.an.ag`
  * ***schläg*** `.äg` `.äg.e` `.äg-er` `O.äg.e`
  * ***schlug*** `.ug`
  * → schlacht
* **schlecht** `schl`
* schlepp `schl.ep` `schl.ep.ed` `schl.ep-er` `E.schl.ep-UNG`
  * -*schlepper* `(er)` → raup
* **schließ** `.iss`
  * **schloss** `.oss`
  * **schluss** `.uss` `ra.uss` `tr.ug.uss`
  * **schlüss** `.üss`
* **sch**on `sch` `sch-Ü-er` __`sch,i`__
* **schön** `schö` `schö.H`
  * *schönheit* `ö-H`
* ***schöpf*** `.öpf`
* **schul** `.ul` `.ul.e` `fa.ul.e` `ho.ul.e` `ho.ul.eif.e`
  * *fahrschule* `fa.sch.ul.e` ≠ fachschule
  * *hochschule* `ho-u` `ho-u.om`
  * **schül** `.ül` `.ül.er` `.ül.er.in`
    * -**schüler** `-Ü.er` `ho-Ü.er`
* **schutz** `.uz`
  * -sch**u**tz `u` → sicht strahl welt
  * **schütz** `.üz`
    * *schütz* `schÜ`

#### [SCHM](#wörterbuch)

* **schmack** `schma` `schma.ha` `G.schma` `G.schma.os.i-T` `G.schma,(.e)`
  * schmeck `schm.ek` `f.N.schm.ek-er` `w.ol.schm.ek.ed`
* schmeck ← schmack
* **schmerz** `schm` `schm.ha`
* schmier `schm.ir`

#### [SCHN](#wörterbuch)

* -***schneid***- `.eid` `schn.eid` `schn.eid-er` `.ab.eid` `.S.eid`
  * *schneider* `.eid-er`
  * *ausschneid* `.S.schn.eid`
  * -***schnitt***- `.it` `schn.it` `G.it` __`.ab.it`__
* **schnell** `schn` `schn,i`

#### [SCHR](#wörterbuch)

* **schrank** `schra` `kÜ-schra`
  * **schränk** `schr`
* **schreck** `schr`
  * **schrick** `.ik`
  * **schrak** `schra`
  * **schrock** `schro`
* schrei `schr.ei`
  * schrie `schr.i`
* ***schreib*** `.eib` `gu.eib`
  * ***schrieb*** `.ib`
  * ***schrift*** `.ift`
    * -*schrift* `i` `.ab,i` `M,i` `UNTER,i` `UNTER,i-en` `Z,i` → hand kurz schnell
* -***schreit***- `T` `schr.T` `schr.T.ed` `.S.T` `N.T` `GR-Ü.T.UNG` `Ü.T` `O-ts.T` `-t-Ü.T`
  * *einschreit* `N,(T)`
  * -***schritt***- `.it` `schr.it` `G.it` `fr.it` `rÜ.it` `rÜ.it.L`
    * *fort*<i></i>*schr****i****tt* `fr-i` `fr-i.L`
    * *geschritt* `G.schr.it` ≠ gebiet
* schrift ← schreib

#### [SCHW](#wörterbuch)

* schwarz `schw.az`
  * *schwarz*- `schwa` → hör markt wald
  * schwärz `schw.ez` `schw.ez.er` `schw.ez.est.e` `schw.ez*` `schw.ez.L` @-st
* **schwed** `schw` `schw.e` `schw.en` `schw.i`
  * *schweden* `schw`
  * *schwedisch* `schw.ed.i`
* schwell `schw.ell` `schw.ell-UNG` `.an.schw.ell` `.an.schw.ell-UNG`
  * *schwellung* `schw.ell.UNG` `.an.schw.ell.UNG`
  * schwill `schw.ill` `schw.illt`
  * schwoll `schw.oll` `G.schw.oll` `.an.schw.oll`
* **schwer** `schw`
  * **schwier** `.ir` `.ir.i`
* *schwester* `schw.est`
* schwier ← schwer
* schwimm `schw.im`
  * schwamm `schw.am`
    * schwämm `schw.äm`
  * schwomm `schw.om`
  * schwemm `schw.em`
* schwind `schw.id`
  * schwand `schw.ad`
  * schwund `schw.ud`
  * schwend `schw.ed` `E.schw.ed`
    * *verschwend* `E.schw`

### [SP](#wörterbuch)

| [SPR](#spr) |
|:-:|

* spalt `spat`
  * spält `spet` `zw.i-spet.ig`
* **spann** `spa`
* **spar** `spa` `.äm:spa`
  * *sparer* `sp.ar-er` `.äm:sp.ar-er`
  * **spär** `sp` `sp.L`
* spaß `sp.ass`
* **sp**ät `sp` `sp.er` `sp.est.e` `sp-t.e-s` __`E.sp*`__
  * später `sp.et-er` ≠ speer
  * *spätestens* `sp.ens`
* speer `sp.er`
* spend `sp.ed` `sp.ed.e`
* -**sperr**- `sp` `sp.err.e` `sp.err-UNG` `G.sp` `.ab.sp` `E.sp`
* **spez** `sp`
  * `sp.iaL` `sp.iaL.ir` `sp.iaL.ist.en` `sp.ieL`
  * `sp.if`
  * *spezif* `sp.ez` `sp.ez.iO` `sp.ez.ir` `sp.ez.ir-UNG` `sp.ez.i` `sp.ez.ät` @-ikation @-izier
* **spiel** `.il` `.il.er` `b.ei-il` `trau.er.il` @sp
* spint __`sp.it`__
* spir `sp.ir` `sp.ir.al` `IN.sp.ir.ir` `ko.sp.ir.iO`
* **spitz** `.iz` @sp
* **sp**iz `sp` `IN.sp.ir`
  * **spekt** `sp` `(o)sp` `IN.sp.iO` `r.sp`
  * spek `sp.ek` `sp.ek.iO` `sp.ek.ir`
* spontan `sp.ot.an` `sp.ot.an.ät`
* *sp****ort*** `(.ot)` `(.ot).N` `wi-er.ot`
* spür `sp.ür`

#### [SPR](#wörterbuch)

* **sprech** `spr` `eNt.spr` `eNT.spr.ed`
  * *entsprechend* `eNT.ed`
  * **sprich** `.ich` `.ich-o` @spr
  * **sprach** `spra` `spra-e` `g.wat,spra-e`
    * **spräch** `spr` `G.spr`
  * **sproch** `.och` @spr
  * **spruch** `.uch` @spr
    * -*spruch* `u` → an ein wider
    * **sprüch** `.üch` @spr
* spreng __`spr.eng`__ ← spring
* -**spring**- `.ing` `spr.ing` `.ab.ing` @spr
  * -**sprang**- `.ang` `spr.ang` @spr
  * -**sprung**- `.ung` `spr.ung` `spr.ung.ha` `eNT.ung` `.ur.ung` `O.ung` `zr.ung` @spr
    * -**sprüng**- __`.üng`__ `spr.üng` `.ur.üng.L` `.ur.üng.L.T` @spr
      * *ursprüng* `.ur-Ü` `.ur-Ü.L` `.ur-Ü.L.T`
  * → spreng
* sprint `spr.it` `spr.it-er`

### [ST](#wörterbuch)

| [STR](#str) |
|:-:|

* st `st` `IN.st.az` `ko.st.aNT` `ko.st.az`
  * ***stat***, ***staat*** `.at` `.at.iO` `.at.ik` `.at.ist` `.at.ko` `ko.at.ir`
    * statik `st.at.ik`
    * *konstatier* `ko.sta.ir`
    * *stat****ist*** `(.i+st)` `(.i+st).ik` `(.i+st).isch`
    * *stat* `sta` `sta.iS.i` `sta.u.e` `sta.u.ir` `sta.us` `sta.ut`
    * **st**it `st` `IN.st.ut` `IN.st.iO` `ko.st.ir` `ko.st.iO`
      * *konstitution* `ko.st.it.iO`
  * ***stab*** `.ab` `.ab.ät`
  * **stall** `sta`
    * *in***stall**at*ion* `IN.sta.iO`
  * → kost
* stab ← st
* **stadt** `sta` `GR,sta` `gr.sta` `hau:sta` `(.es).hau:sta` `(r),sta` ← steh
  * *landeshauptstadt* `(.es).hau`
  * **städt** `st` `st.e` `st.i`
* stall ← steh
* stall ← st
* *stal****t*** `(-t)` `.an-t` `E.an-t`
  * *gestalt* `-G.a-t`
* stamm `st.am`
* *stand* `and` `N.E.and` `and,u` ← steh
  * *einverstanden* `N.E`
  * *standpunkt* `a-pt`
  * *ständ* `end` `end.e` `-st.end.i` `-st.E.end.L`
    * *selbstständig* `-st.ig`
    * *selbstverständlich* `-st.L`
  * → stund
* *standard* `(.ard)`
* stark `st.ak`
  * stärke `st.äk.e`
* start `stat`
* statt ← steh
* staun `st.aun` `st.aun,(-t)` `(er)st.aun`
* stech `st.ech` `be.st.ech.L`
  * *bestechlich* `be.st.L`
  * stich `st.ich`
  * stach `st.ach`
  * stoch `st.och`
* steck `st.ek` ← stech
* **steh** `st` `be.st` `b.ei+st` `E.st` `UN.S.st.L`
  * → stadt
  * → stand
  * **stall** `sta`
  * *stell* `.ell` `.ell-UNG` `GT.ell.e` `.an.ot.ell.e`
    * *stellung* `st*`
    * -*steller* `(er)` → trag
  * **statt** `sta` `sta.ha` `sta.L` `er.sta`
    * -*erstatter* → bericht
    * *stätt* `.ät` `Ts.ät.e`
    * *gestatt* `-G-a` `-G-a.sI-ir` `.I -G-a.ir` `wr -G-a.U`
  * stet `st.et`
    * **stät** `st` `be.st.i`
* stein `st.N`
  * -*stein* `N` → bims nier schorn
* stell ← steh
* ***stell*** `.ell` `ko.ell.iO`
* ***steig*** `.eig` `I .um-sa.eig-er`
  * *die*.*umsatzsteigerung* `I .um.eig-er`
  * ***stieg*** `.ig`
* stet ← steh
* *st****eu***er `eu-er` `eu-er.L` `eu-er,i` `eu-er-UNG` `kö.er.FT,eu-er` `.um-sa,eu-er`
  * *körperschaftssteuer* `kö,eu-er`
  * *umsatzsteuer* `.um.eu-er`
* stick `st.ik`
* **stift** `.ift` @st
  * -*stift* `i` → blei
* stil `st.il`
* **still** `.ill` `t.ot.ill.e` @t.o -> t.ot @st
* **stimm** `.im` `.im:H` @st
  * *bestimm* `b°-i` `b°-i-t` `M,b°-i`
    * *mitstimm* `M°`
* *stinkt* `st.i` `ds.`IN.st.i` `IN.st.i-iv` `IN.st.i-l.o`  ← sting
* stock `st.ok`
* *stockholm* `st.ho`
* **stoff** `.of` `kust-of` `(k).of` @st
* **stolz** `.oz` @st
* *stück* `Ük` `.ük` `frÜ.ük` `G.p-Ük` @Ü
* **stud** `.ud` `.ud.ir` `.ud.I.e` `.ud.um` @st
* **stuf** `.uf` `.uf.e` @st
* st**u**nd `u` `u.e` `u.en` `u-UNG` `(l.ei),u.e` `(ei-er),u.e` `UNTER,ri,u.en` ← steh
  * feier*stund*e `fei-er,u.e`
  * *stünd* `stÜ` `stÜ.ch.en` `stÜ.L`
* sturz `st.uz`
  * stürz `st.üz` `be.st.üz*` `Ü.st.üz*` `.um:st.üz`
* *stuttgart* `st.u-ga`
* **stütz** `stÜ` `stÜ.UNG` `G.stÜ` `.ab.stÜ` `UNTER.stÜ` `(l.os),UNTER.stÜ-UNG`
  * *unterstütz* `UNTER-Ü` `UNTER-Ü-t.e` `(l.os),UNTER-Ü.UNG`
    * *arbeitslosenunterstützung* `(.los)-Ü.UNG`

#### [STR](#wörterbuch)

* **straf** `stra` `stra-e` `stra.bu`
  * -*straf****e*** `(.e)` → geld tod ven
* -**strahl**- `str.al` `str.al.ed` `str.al,u` `(.S)stra`
* **straß** `stra` `stra-e`
  * -*straß****e*** `(.e)` → haupt land milch seite
    * *seitenstraße* `s.T.en,(e)`
* **streb** `str` `(be)str-t` `(er)str,(-t)`
  * *bestrebt* `(be)str`
* -**streck**- `str.ek` `str.ek.e` `(er)str` `ang-ol.str`
  * *zwangsvollstreckung* `ang-ol`
* **streng** `str` `(Ü.an)str`
* *str****eit*** `(T)`
  * *str****itt*** `(.it)`
* ***streich*** `.eich`
  * ***strich*** `.ich` `f-er.ich`
* ***streik*** `.eik`
  * *general****streik*** `g.eik`
* streit `str.T` `str.T,u`
  * stritt `str.it`
* string `str.ing` `str.ing.eNT`
* strikt __`str.ikt`__ `d.i:str.ikt` ← string
  * → distrikt
* stroph `str.of`
  * → katastroph
* **strom** `.om` __`.om,(er)`__ @str
  * **ström** `.öm` `.öm-UNG` @str
* str**u** `u` `IN,u-ir` `IN,u-met` `IN,u-met.al` `IN,u-met.ar.um` `IN,u-met.O` `IN,u-met.ir-UNG` `ko,u.ir` `ko,u.iv` `ko,u.ör` `r.ko,u.irt.e`
  * *instrument* `IN.u-met` `IN.u-met.al` `IN.u-met.ar.um` `IN.u-met.O` `IN.u-met.ir-UNG` `IN.u-met,bau`
  * str**u**kt `u` __`u-ur`__ __`u-ur.ell`__ `IN,u-O` `ko,u-O` `ko,u-ör` `IN.fra,u-ur`

## [T](#wörterbuch)

| [TR](#tr) |
|:-:|

* tab~~ell~~ `t.ab` __`t.ab.e`__ `t.ab.ar.i` `t.ab.ir`
  * *tabelle* `t.ab`
* *tag* `-tag` __`Ts-tag`__ `bu:-tag` `do-tag` `(.ei-er)-tag` `(.ei)-tag` `f.T.tag` `(bst)-tag` `(I)n,(-st)-tag.en`
  * *tag.für.tag* __`tag-tag`__
  * *täg* `teg` `teg.L`
* tang `t.ang` `t.ang.ir` `t.ang.eNT.e`
  * takt `t.akt` `ko.t.akt`
    * *kontakt* `ko.ta`
  * → teger
* *tarif* `ta.if`
* tat ← tun
* **tausch** `tau` `tau-t.e` `.S.tau` `gÜ.S.tau` `.um.tau` `E.tau`
  * -~~tausch~~- `gÜ.S`
  * **täusch** `täu`
* *tausend* `(I)` `(I).e`
* *tech****n*** `(n)` `(n).ik` `(n).ik-er` `(n).i` `.el,(n).i` `(m.in).us,(n).ik.us`
  * *elektrotechnisch* `.el.o-t.en.i`
  * *terminus.technicus* `(m.in).us;us`
  * → tekt
* **teg** `t` `(o)t.ir`
  * **tekt** `t` `(o)t.iO` `(o)t.or` `(o)t.or.at`
* teger `t.eg-er` `IN.t.eg-er` ← tang
  * *integr* `IN.egr` `IN.egr.al` `IN.egr.ir` `IN.egr.iO` `IN.egr.ät`
* *t****eil*** `(.eil)` `(.eil).bar` `(.eil).er` `(.eil)-s` `.ab.eil-UNG` `be.eil.i` `ach.eil.e` `O.eil.ha` `s.az.eil`
  * *ur*~~teil~~ `ur` `ur.e` `t.o.ur`
* tekt `t.ek+t` `t.ek+t.on.ik` `t.ek+t.on.i` ← techn
  * -*tekt* `.ekt` → arch
* **tel** `t` `t.f` `t.f.at` `t.f.ir` `t.f.i`
  * *telephon* `t.el.e-f` `t.el.e-f` `t.el.e-f.ir` `t.el.e-f.i`
* *temp* `emp` `emp.o` `emp.I` `emp.us` `emp.or.al` `emp.or.är`
  * *temporär* `emp.är`
  * *temper* `emp` `emp.eNT` `emp.eNT.Ol` `emp-ur` `emp.ir`
    * *temperament* `t.emp.eNT` `t.emp.eNT.Ol`
      * *temperamentvoll* `t.emp-ol`
* tempel `t.emp.el`
* *tempelhof* `t.emp.ho`
* **tend** `t` __`t.ir`__
  * *tenden****z*** `(z)` __`(z).ös`__
  * **tens** `t` `IN.t.iv` `IN.t.iv.ir-UNG` `IN.t.ät`
* ter***min*** `(m.in)` `(m.in).al` __`d.e,(m.in).ir`__
* test `t.est` `t.e+st.eNT`
  * *testament* `t.e+st`
* **teuer** `teu` `teu-er` `be.teu` `E.teu`
  * *teurer* `t.eur-er`
* ***text*** `ext`
  * *tex****til*** `(t.il)`
    * *textil* `t.il`
* theater `t.e-at-er` `t.e-atr.i`
* thek `t.ek` __`t.ek.e`__
* them `t` __`t.em.a`__ `t.em.en` `t.em.i`
* theo `t.e` `t.e-d.or` `t.e-d.ol.it` `.a-t.e-iS`
  * → log
* theor `t.e-or` `t.e-or.ik-er` `t.e-or.i`
  * *theorie* `t.e-o-I` `t.e-o-I.en` `quat,t.e-o-I` `r.iv.ät-t.e-o-I`
* therm `t.em` `t.em.al`
* **tief** `ti` `(E)ti`
* **tin** `ti` __`ko,ti.eNT`__ __`ko,ti.ir.L`__
* **ti**~~ng~~ `ti` __`ko,ti.eNT`__
* tint __`tit`__ `tit.e` ← ting
* tisch `t.isch`
* tit `t.it` `t.it.el` `t.it.ur` `t.it.ir-UNG`
  * *titel* `t.it`
  * *titel*- `ti` `ti,18`
* *tochter* `t.ocht`
* **tod** `t.o` `t.o,(-st)` `t.o,(.e)`
  * **töd** `t.ö` `t.ö-L`
* *toler* `t.ol` `t.ol.aNT` `t.ol.az` `t.ol.ir` `t.ol.ir-UNG` `IN.t.ol.aNT` `IN.t.ol.az`
* ~~t~~o~~n~~ `(.o)` `(.o)-l.o` `b,(.o)` `d.e,(.o).ir` `IN,(.o).ir` `er b,(.o)-t.e` `er b,(.o).e`
  * ~~t~~o~~n~~los `(.o)-l.os`
  * *in*tonier `IN.t.on.ir`
  * *detonieren* `d.e-t.o`
  * ~~t~~ö~~n~~ `(.ö)` `N,(.ö).i`
    * *eintönig* `N,(.ö)` @-ig
* **tot** `t.o`
  * **töt** `t.ö` `G.t.ö`
* **tot** `t.o` `t.o-al` `t.o-al.är` `t.o-al.ät`
* *tübingen* `t.üb.ing`
* tuch `t.uch`
  * tüch `t.üch` `t.üch-er`
* -*tum* `tum`
  * -*tüm* `tYm`
* **tu** `tu` `ku.tu` `wi.i,tu.er` __`wi.i,tu.ei`__ @-erei
  * **tat** `ta` `ta,(.e)` `ta,(en)` `M.ra.ta`
    * **tät** __`t`__ `t.e` `t.er` `t.i` `t.i-T` `be.t.i`
      * *täter* `t.et-er`
* turt __`tut`__ __`tut.el`__

### [TR](#wörterbuch)

* **tracht** `tra` `(be)tra` `(be)tra.et.e` `(N)tra` ← trag trakt
  * **trächt** `tr` `tr.i` `be.N.tr.i` `be.N.tr.i-t.e` `(N)tr.i` `(ndr)tr.i-T`
* -**trag**- `tra` `tr.ag` `G.tra` `(.an)tra` `(.an)tra,(er)` `(.an)tra.G` `(AU)tra` `(AU)tra.G` `(AU)tra.g` `n.S.tra` `(.ei)tra` `(er)tra` `(er)tra.eich` `ach.tra` `v.is.um.an.tra` `(E)tra` `(E)tra-Ü.i` `(E)tra.ha*.en` `.auf,(E)tra` `l.i-er-UNG,(E)tra` `(E)tra,i` `(Ü)tra` `di-Ü:tra`
  * -*vertrag* `tra` → kauf liefer
  * ~~trag~~ `be.an` `be.AU` `(r) .an` `.im:be.v`
    * ~~tragte~~ `(r),be.AU`
  * *trag* `tra`
  * *vertrag* `E-a`
  * *direktübertragung* `di-t-Ü:tra
  * -**träg**- `tr` `tr.eg` `tr.egt` `tr.eg.e` `tr.eg-er` `tr.eg.H` `(.an)tr.e` `(AU)tr.e` `(be)tr` `(be)tr.e` `(.ei)tr.e` `UN.er.tr.L` `ach.tr` `ach.tr.er.i` `ach.tr.L` `ach.tr.L.T` `(E)tr` `(E)tr.L` `(E)tr.L.T` `UN.E.tr.L.T` @ä-e
    * *nachträgerisch* `ach.tr.eg-er.i`
    * *nachträglich* `ach.L` `ach.L.T`
    * *verträg* `(E)tr.e`
    * *verträge* `E,(.e)`
  * -**trug**- `tru` `tr.ug` `be.tru` `E.tru` `ach.tru`
    * -**trüg**- `trü`
      * *trüg* `tr.üg` `tr.üg.e`
* -**trah**- `tra` `(ko)tra`
* -**trakt**- `tra` `tr.akt` `.a-tra.iv` `(ko)tra` `(ko)tra-Ü.i` ← trah
  * *attraktiv* `.a-tra-t.iv`
* *trans*- `tra`
  * **tran**- `tra`
* **trau** `trau`
  * *misstrau* `ms-au` `ms-au.um`
  * *vertrau* `E-au` `E-au.L` `E-au,(g)` `E-au.um` `E-au-Ü.i`
* *trauer* `trau.er` ≠ trauung ← traur
* **traur** `trau` `trau.i` `trau.i-T`
  * → trauer
* traum `tr.aum`
  * träum `tr.äum`
* -**treff**- `tr` `tr.ef` `tr.ef-er` `tr.ef.L` `(be)tr` `(be)tr.ed` `(be)tr-s` `(N)tr` `(Ü)tr` `UN-Ü.tr.L` `(O)tr.L` `Z.tr.ed` `Z.Sa.tr`
  * *trefflich* `tr.L`
  * *eintreff* `(N)tr.ef` ≠ eintreten
  * *zusammentreff* `Z.Sa.tr.ef` ≠ zusammentreten
  * -**triff**- __`tr.i`__ `tr.if` `(be)tr.i` `(Ü)tr.i`
    * *triff* `.if` `.ift` `be.ift` `Ü.ift`
  * -**traf**- `tra` `tr.af` `(be)tra` `(Ü)tra`
    * *betraf* `(be)tr.af`
    * -**träf**- __`trä`__ `tr.äf.e` __`N.trä-e`__
  * -**troff**- `.of` `G.of` `.an.of` `be.of` `N.of` `Ü.of` `UN-Ü.of`
* **treib** `trei` `trei-ed` `trei-er` `(.an)trei` `(AU)trei` `(.ei)trei`
  * *treib* `tr.eib`
  * **trieb** `tr.i` `G.tr.i` `tr.i-ha` `tr.i-f-er` `tr.i=kra` `tr.i=ess.i` `(.an)tr.i` `(be)tr.i` `(Ü)tr.i` `sta-Ü.tr.i`
    * *trieb* `.ib` `be.ib`
      * -*betrieb* `.ib` → groß hand
* trenn `tr.en` `tr.en.bar` `(.ab)tr.en` `UN.zr.tr.en.L`
  * *unzertrennlich* `UN.zr.tr.L`
* -**tret**- `tr` `tr.et` `G.tr` `(.an)tr` `(.S)tr` `(.ei)tr` `(eNT.g)tr` `(g-Ü)tr` `(E)tr.er` `(E)tr.UNG` `(.e-O)tr` `.a-zw.tr`
  * *dazwischentret* `a-zw.tr`
  * *entgegentret* `(eNT)tr`
  * *gegenübertret* `g-Ü.tr`
  * *vertreter* `E.er`
  * *tritt* `.it` `.S.it` `b.ei-it` `N.it` `rÜ.it` `Ü.it` `E.it` `Z.it` `AU.it.it` `b.eim-Ü.it`
  * *trat* `tr.at` `(E)tr.at`
* **treu** `treu`
  * -*treu* `eu` → könig
* **trotz** `tr.o` __`G.tr.o`__ `tr.o-r` `tr.o-es` `tr.o-m` `tro-al.em` `tro-al.e,(m)`
  * *trotzdem* `(o)(m)`
* trug ← trag
* -**trug**- `tru` `tr.ug` `be.tru`
  * -**trüg**- `trÜ` `tr.üg` `tr.üg-er.i` `(be)trÜ` `(be)trÜ-er` `(be)trÜ.ei-en` `(be)trÜ.i` `UN.trÜ.L`
    * *untrüglich* `UN.tr.üg.L`
  * -**trog**- `tr.o` `G.tr.o` `(be)tr.o`
    * *ge*trog `G.tr.og` ≠ getrotzt

## [U](#wörterbuch)

* üb `.üb` `.üb.L`
* *über*, *übr* `Ü` `Ü.i`
  * -*über*- `-Ü` `dar-Ü` `er-Ü.i` `h-Ü` `hi-Ü` `O-Ü`
* *ultra* `.utra`
* um `.um` `.um,i`
  * -*um* `-u` → dar war
* un `.un` `.un.if-o` `un.at` `un.um` `.un.iO`
  * *union* `u-O` `u-O.ei-en`
* *und* `&`
* -*ung* `UNG`/`*`
* *uns* `.U` `f.U.e` `v.U.em`
  * -**u**ns __`u`__ `.an,u`
    * *an.uns* `.an-u` ≠ anspruch
  * **u**nser __`u-er`__ `u-er.e` `u-er-er` `u-er.es`
  * -*unser* `.U°` `.an.U°`
* *unter* `UNTER` `UNTER.ei-ad-er` `UNTER.ha` `.e-UNTER`
* ur- `.ur`
* -us `.us`
  * -ius `I.us` ⇒ `.us` @⇒

## [V](#wörterbuch)

* *val****id*** `(.id)` `IN.id.e` `IN.id.ät`
* vater `v.at-er`
  * *vate****r***- `(r)` → land
  * väter `v.ät-er`
* **ven** `v` `(I)t-er.w.ir` `(I)t-er.w-o`
* *vent* `v` `.e-v.ät` `ko.w-o` `ko.w.i-aL,(.e)` ← ven
  * evtl. `.evt.l`
  * *invent* `IN-t` `IN-t.ar` `IN-t.ar.ir` `IN-t.ur`
* *ver*- `E`
* *ver***letz** `E.l` `E.l-t` ← letz
* *ver***lier** `E,li` ← lier
  * *verlier* `(E)li`
  * *ver***lor** `E.lo`
    * *verlor* `E.l.or`
  * *ver***lust** `E.lu`
    * *verlust* `E,u` `E,u.e`
* vers `v.es` `v.es.irt` `v.es.ir.H` `v.es` `d.i-w.es` __`d.i-w.es.if-o`__ `IN.w.es` `ko.w.es` `ko.w.es.iO` ← vert @-sion
  * uni~~vers~~ `.un` `.un.al` `.un.ell` `.un.ät` `.un.um`
* vert `vet` `ko.wet.ir` `ko.wet.ib.ät`
  * → vers
* *ver***zeih** `E.zei` ← zeih
  * *ver***zieh** `E.z.i`
* *verzicht* `E-i` ← verzeih
* **vest** `v` `IN.w.ir` `IN.w-o` __`IN.w.met`__
* -**v**id- `v` `d.i-w.ir` `r.w.ir`
  * *divi****dend*** `(d.ed)` `(d.ed).e`
  * *individ* `IN.d` `IN.d.al.ät` `IN.d.ell` `IN.d.um`
  * → vis
  * → vue
* *video* `vi.o` `vi.os` `vi.o-t.ek` ← vid
* **vieh** `vi` `vi.i` `vi,u`
  * -***vieh*** `.ih` → rind schlacht
* ***viel*** `.il` `.o-il`
  * *soviel* `.o-l` ≠ weil
* vier `v.ir`
  * *viert* `vi` `vi.el`
* -**v**is- `v` ← vid @-sion
  * `v.is` `v.is.är`
  * `v.is.e` `v.is.ir` `v.is.iO`
  * `v.is.ir` `v.is.ell` `v.is.um` `v.is.a`
  * `d.e-w.e` `d.e-w.en`
  * `.im-p.o-w.ir` `(o)w-o` `(o)w.or.i` `(o)w.or.um` `(o)w.or.I.en` `r.w-o` `r.w.or`
  * VISIT `v.is.it` `v.is.it.e` `v.is.it.ir` `v.is.it.iO`
  * VISION `v.is.iO` `v.is.iO.är`
  * *devis* `d.e-is` `d.e-is.e` `d.e-is.en`
  * *provisorien* `(o)w.I.en`
* *volk*, *völk*
* *voll* `Ol`
  * -*voll* `-ol` → dienst drang effekt
  * *hochachtungsvoll* `ho.o`
  * *völl* `Öl`
    * *völlig* `ö`
  * → füll
* volv `v.ow` `r.w.ow-er`
  * volt `v.ot` `r.wot.e`
  * *vol****u****t* `u` `.e,u-O` `r.u-O` `ko-t.er,r.u-O` `kut.ur,r.u-O`
* **von** `v` `.a-v` @da-
  * *davon* `a-v` `a-v.ab.ra`
  * *vom* `v°`
* *vor* `O` `O.n` `O.ts` `O.d-er` `be.O` `.e-O` `ach.O`
  * *nach.wie.vor* `ach-o`
  * *vori****g*** `(g)`
* **vot** `v`
  * -*votum* `.um` → trau
* vue `v.ü` `r.w.ü` ← vid

## [W](#wörterbuch)

| [WR](#wr) |
|:-:|

* waag ← wieg
* waag `w.ag` `w.ag.e`
* wach `w.ach` `w.ach.Sa`
* *wachs* `was` `was.tum` `FT.was.tum`
  * *wachstum* `was-u` `FT:was-u`
  * *wächs* `wes`
  * *wuchs* `wus`
    * *wüchs* `wüs`
  * *wechsel* `wes.el`
    * -*wechsel* `S.el` `.ab.S.el` `.S.S.el` `.um.S.el` `.ift.S.el` `E.S.el`
* wagen `w.ag`
  * -*wagen* `wa` → kraft krank
* -***wäg***- `.äg` `w.äg` `.ab.äg` `er.äg`
  * -***wog***- `.og` `er.og`
    * *erwog* `er.w.og`
* **wahl** `wa` `wa,(.el)` `er-sa.wa` `(N)-d.e:wa.en` `l.i=wa`
  * **wähl** __`wä`__ `wä` `G.w-t` `w.bar` `w-er` `w-er.FT`
    * *wähl* `w.el`
* -**wahn**- `wa` `w.an.in.i` `w.an,(g)`
  * -**wähn**- `wä` `er.wä`
    * *erwähn* `(er)wä`
* **wahr** `wa`
  * **währ** `wä` `w.ed` `wä-r.om` `be.wä` `G.wä` `I wä`
* wald `w.ad` `w.ad-UNG.en` `w.ad.ig` `schwa.w.ad`
  * wäld `w.ed` `w.ed-er`
* walt `wat` `G.wat` `pr.iaL:wat` `E.wat`
  * *anwalt* `.an-t` `R.an-t`
    * *rechtsanwalt* `R.an`
  * *verwalt****ung*** `UNG` `UNG.ik` `UNG.ko` `ko-u.al-UNG` `(.es).UNG` `sta.UNG` @-ung
  * wält `wet` `be.wet.ig` `Ü.wet.ig.ed`
* wand `w.ad`
* wand ← wind
* wandel ← wind
* wann `w.an` `.i-w.an`
* **war** `wa` `wa-e`
  * *ware* `wa.e`
* war ← sein
* warn `w.an`
* wart `wat` `wat.et.e` `er.wat` `er.wat.Ol`
  * -*wart* `-t` → gegen
  * wärt `wet` `wet.er`
    * -*wärt* `-t` `-t.er` `-t.ig` `-ts` → an aus gegen rück seit vor
* ***was*** `was`
* *wasser* `was` `was.ek` `was.FT` @-er
  * *wass*er `was-er` ≠ was
  * *wässer* `wes` __`wes.i`__ `eNT.wes-UNG` __`G.wes`__
* wasch `w.asch` `w.esch.e`
* wechsel ← wachs
* -**weg**- `w` `w.eg` `.S.w` `(.es)w` `(K).w-s` `m.N.w` `.U-t.w` `O.w` `AU.m.w`
  * *keineswegs* `k.w`
* wegen `w.eg` ← weg
  * -**weg**en `w` → des
* weh `w.eh`
  * wind `w.id`
* ***wehr*** `wer` `G.wert` `wer.os` `wer.ha` `.ab.wer` `G.wer` `G.wer.e` `wer,i` `bu.wer` `feu:wer`
  * *bundeswehr* `u-wer`
  * *feuerwehr* `eu:wer`
* -**weich**- `wei` `w.eich` `w.eich.ing` `.ab.wei` `u-rÜ:wei`
  * -**wich**- `w.i` `w.ich`
    * *gewichen* `G.w.ich`
* weiger `w.eig-er` `we.d.i,E.w.eig-er-er`
* weil `w.eil` `w.eil.e`
* ***weil*** `.eil` `.eil.I`
* wein `w.N`
  * wen `w.en` `w.en.i` `d.est.w.en.i-er` `m.w.en.i-er`
    * *nichtsdestoweniger* `-ts.o`
    * *mehr.oder.weniger* `m.w`
* *w****eis*** `(.eis)` `Ts.eis.e` @ausnahmsweise `be.eis` `be.eis.bar` `be.eis.iaL` `er.eis` @glücklicherweise `hn.eis` `.ob.eis.e` `Ü.eis` `b.ak-Ü.eis` `dar-AU hn.eis` `Z-Ü.eis` `I-Ü.eis` `wr-Ü.eis`
  * *darauf.hinweis*en `dar-AU-n` `.I.uss dar-AU-n`
    * *ich.muss.darauf.hinweisen* `I.uss.eis`
  * -*w****eis***e `.eis.e` → arbeit glück nehm
  * *w****ies*** `(.is)` `G.is` `.ab.is` `be.is` `er.is` `er.is.ass` `Ü.is` `.I-Ü.is`
    * *wies* `w.is`
* weiß ← wiss
* weiß `w.eiss`
* -***weit***- `T` `w.T` `T.er` `T.est.e` `T.er.in` `er.T.er` `er.T.ert.e` `er.T.er-UNG`
  * *und.so.weiter* `&.o`
* ***welch*** `.ech` `.ig.ech.e`
* welt `wet` `wet.ha` `E.wet.L`
  * *umwelt* `um-t` `um-t,eu-L` `.um-t,u`
* wem ← wer
* wen ← wer
* wen ← wein
* -**wend**- `w` `w.ed` `.an.w` `AU.w` `E.w` `E.w-UNG` `.ot.w.i` ← wind
  * *notwendig* `(o)w.i`
  * -**wandt**- `wa` `w.ad` `w.adt.e` `be.wa.is` `N.wa` `G.wa` `E.wa`
    * *bewandtnis* `be.wa-t.is`
* wenig ← wein
* w~~enn~~ `w`
* ***wer*** `wer`
  * wem `w.em`
  * wen `w.en`
  * wes `w.es` `w.es.ha`
* -**werb**- `w` `w.eb` `w.eb-er` `w.eb-UNG` `w.eb.L` `be.w` `be.w-er` `er.w` `er.w-er` `er.w.os` `er.w.os.T` `G.w.e`
  * *erwerber* `er.w.eb-er`
  * *wettbewerb* `w°` `w°-er`
  * *gewerblich* `G.L`
  * -**wirb**- `w.ib`
  * -**warb**- `w.ab`
  * -**worb**- `w.ob`
* *werd* `(w)`
  * *wird*, *wirst* `(wr)`
  * *word* `(O)` `(G)(O)`
  * *wurd* `(U)`
    * *würd* `(Ü)`
* werf `w.ef` `w.ef-er` `eNT.w.ef` `aCH.w.ef` `E.w.ef.L`
  * wirf `w.if`
  * warf `w.af`
  * worf `w.of` `O.w.of`
  * → wurf
* werk `w.ek` `b.eg.w.ek` `G.w.ek.FT` `hi.w.ek`
  * *gewerkschaft* `(k).FT`
  * -*werk* `.ek` → hand wasser
* wert `wet` `wet.Ol` `.ab.wet`
  * -*wert* `(-t)` → hoch nahr nenn staun streb
* wes ← sein
* **west** `w` `w.est` `w.est.L`
  * *westlich* `w.L`
* wetter `w.et-er`
* **wicht** `wi` `wi.i` `wi.i-T` `bru-o,wi` `G,wi` `ha.schw,wi` `schla,wi` `UN,wi.i`
* wicht `w.icht` `b.ös.w.icht`
  * bösewicht* `b.ös.e,wi`
* **wick** `wi` `wi.el` __`(eNT)wi.el`__ __`T.er:(eNT)wi.el`__ `(E)wi.el`
* *wid****m*** `(m)`
* wie `w.i`
  * -*wie* `I` `.i-I` `(o)I`
* *wieder*, *wider*, *widr* `W` `W.ig` `W,u`
  * -*wieder* `i` → auch immer schon
  * *widerspruch* `W-u`
  * -*widrig* `i` → gesetz
* wieg `w.ig` `O.w.ig.ed` `M Ü.w.ig.ed-er m.H`
  * *vorwiegend* `O.ig.ed`
  * *mit.überwiegender.mehrheit* `M-Ü.w.H`
  * wog `w.og`
  * → waag 
* wind ← weh
* wind `w.id`
  * wand `w.ad`
    * *wandel* `wa.el`
  * wund `w.ud`
  * → wend
* winn `w.in` `G.w.in`
  * *gewinn* `G.in`
  * wann `w.an` `G.w.an`
  * wonn `w.on` `G.w.on`
* *winter* `wi-er` `wi-er.L` `Ü,wi-er`
* *wir* `wr` `wr-I` `d.wr`
  * *dass.wir* `DR` @(3-stufiges R)
* wirbel `w.ib.el` `G.w.ib.elt` `w.ib.el,eu`
  * *wirbelsäule* `w.ib,eu`
* *wir****k*** `(k)` `(k)t` `(k).L` `(k).L.T` `(k)-Sa.T` `(k).os` `(k).Ol` `(.S)(k)` `be,(k)` `N,(k)` `eNT.g,(k)` `rÜ-k.ed` `UN,(k).L` `dar-AU n.k`
  * *wirk* `w.ik` `w.ikt` ≠ markt
  * *wirksamkeit* `(k)-s.am.T`
  * *bewirk* `b,(k)`
  * *entgegenwirk* `eNT.k`
  * *darauf.hinwirken* `dar-AU.k`
* **wirt** `wit`
  * *wirt*- `(wr)`
    * *wirt****schaft*** `FT` `FT.er` `FT.N` `la.FT` `FT.L.e,ig` `G.L.e FT` @-schaft
      * *landwirtschaft* `(a-wr).FT` ≠ landschaft
      * *gewerbliche.wirtschaft* `G.L.FT`
* *wiss* `.iss` `.iss-t.L` `.iss,(-t)` `G.iss` `G.iss.H` `E.-G.iss` `E.-G.iss-t.e` `s.ig.iss` `UN.iss.ed` `UN.iss.H` `UN-G.iss` `UN-G.iss.H` `sI=iss` `w.i-sI=iss` `ga.iss`
  * *vergewisser* `E.-G` `E.-G-t.e`
  * *wissenschaft* `wi.FT`
  * *weiß* `.eiss` `.I.eiss`
  * *wuss* `.uss` `.usst.e` `be.usst` `be.usst.N`
    * *wusste* `w.usst.e` ≠ musste
    * *bewusstsein* `be,u-N`
    * -*bewusst* `u` `u-N` → mir pfleg selbst wort
  * *wüss* `.üss` `.üsst.e`
    * *wüsste* `w.üsst.e` ≠ müsste
* **witw** `wi` `wi-er`
* witz `w.iz` `w.iz.i` `w.iz.os`
  * -*witzig* `(g)` → wahn
* **wo** `wo` __`.i-wo`__
  * **wo**-, **wor**- `wo`
  * **war** `war` `war-u`
* **woch** `wo` `wo-e`
* wohl `w.ol` `G.Sa.w.ol` `.ob.w.ol`
* **wohn** `wo` `wo-UNG`
  * *wohnung* `wo-u` ≠ woche
  * **wöhn** `wö` `G.wö` `G.wö.L`
  * *gewöhnlich* `G-ö-L`
* *woll* `(.ol)`
  * *will* `(.l)`
* wort `wot`
  * *ant*~~wort~~ `aNT` `(be)aNT` `(E)aNT` `(E)aNT.L` `(E)aNT,u-N`
  * -*wort* `o` → bind schlag sprech
* wund ← wind
* **wund** `wu` `wu-er` `wu-er.Ol` `be.wu-er`
  * *wundervoll* `wu-ol`
* **wunsch** `wu`
  * **wünsch** `wÜ` `er.wÜ`
* würd `w.üd` `w.üd.e` `w.üd.i`
* -**wurf**- `wu` `w.uf` `eNT.wu` `O.wu` ← werf
  * *ent*<i></i>*w****u****rf* `eNT-u` `-G:eNT-u`
  * -**würf**- `wÜ` `w.üf.e` `w.üf.el` `eNT.wÜ.e` `-G-eNT.wÜ.e` `O.wÜ.e` `zer.wÜ.is`
    * *entwürf* `eNT-Ü` `eNT-Ü.e` `-G-eNT-Ü.e`
      * *gesetzentwürfe* `-G-Ü.e`
    * *zerwürfnis* `zer-Ü.is`

### [WR](#wörterbuch)

* wrack `wr.ak` `.ab.wr.akt`
* wring `wr.ing`
  * wrang `wr.ang`
  * wrung `wr.ung` `G.wr.ung`

## [X](#wörterbuch)

## [Y](#wörterbuch)

## [Z](#wörterbuch)

| [ZR](#zr) | [ZW](#zw) |
|:-:|:-:|

* **zahl** `za` `za*` `ra.za*` `(.ei)tra.za*`
  * **zähl** __`zä`__ `er.zä` `UN.z.i`
    * *zählen* `z.el`
    * *erzählen* `er.z`
* **zahn** `za`
  * **zähn** `zä` __`zä-e`__
    * *zähne* `z.en.e`
* zart `zat`
  * zärt `zet` `zet.L.T`
* zauber `z.aub-er` `z.aub-er.er` `z.aub-er.ei`
* ***zed*** `.ed` `ko.ed.ir` `(o).ed.ur`
  * *prozedur* `(o)z.ur`
  * → zess
* zehn `z.en` `ja.z.ent`
* ***zeich*** `.eich` `.eich.UNG` `E.eich.is` `s.az.eich`
  * zeichn `z.eich` ≠ reichen
* ***zeig*** `.eig`
* **zeih** → verzeih bezichtig
* zeil `z.eil`
* ***zeit*** `T` `T.UNG` `Ts.T` `u-er-er.T` `(I)r.let-T` `IN,(-st)-T` `O.eNGR.T`
  * *frage.der.zeit* `fra-e.T` 
* zend `z.ed` `tra.z.ed.eNT`
* zent `zet` `zet.er`
  * *prozent* `o` `o.al` `3.o` `5.o` `I 5.o,au.el`
    * -*prozentig* `(g)`
* zent ← kant → akzent
* zentaur `zet.aur`
* *zentr* `zr` `zr.ir` `zr.um` `.e-zr.ik-er` `(ko)zr.iO` `(ko)zr.ir`
  * zentr `z.etr` `z.etr.ir` `.e-z.etr.ik-er`
  * *zentral* `zra` `zra.e` `zra.ir` `zra.ir-UNG` `zra.iO` `zra.iS` `zra.iS.i` `zra.ba` `(d.e)zra.ir` `(d.e)zra.iO`
* **zept** `z` `ko.z` `ko.z.iO` `r.z` `r.z.iO` ← zip
  * *konzept* `ko-t` `ko-t.iO`
* *zer*- `zer`
* zern `z.en` `ko.z.en`
* *zert* `zer` `zer.if.at` `zer.if.ir` __`(ko)zer`__ ← zern
* ***zess*** `.ess` `ko.ess` `(o).ess` `(o).ess.ir` `(T),(o).ess` __`(o)du-o,(o).ess`__ ← zed @-sion
* zettel `z.et.el`
  * -*zett****el*** `(.el)` → hand merk wahl
* **zeug** `.eug` `fa.eug` `kra.fa.eug` `er.eug.iss.e` `Ü.eug` `f.ür-UNG.eug.is` `ach.m.ei-er-Ü.eug`
  * *kraftfahrzeug* `kra.eug`
  * *nach.meiner.überzeugung* `ach.m.ei-er-Ü`
* **zieh** `.ih` `Z.ra.ih` `I ha.el.be.ih.en`
  * *die.handelsbeziehungen* `I ha.el.ih.en`
  * **zog** `.og`
    * **zög** `.ög` `.ög-er`
  * **zug** `.ug` `.ug.ug`
    * **züg** `.üg` `.üg.e` `.üg.el` `.üg.i` __`gr.üg.i`__ __`ach.üg.el.er`__ __`O.üg.L`__
  * **zucht** `.ucht`
    * -z**u**cht `u` → vieh
    * **zücht** `.ücht`
* ***ziel*** `.il` `.il,u
* zie~~m~~ `z.i` `z.i-L`
* *zig****ar*** `(ar)` `(ar).et.e` `(ar).ill.o` ← zigarr
* *zig****arr*** `(arr)` `(ar).e` `(arr),rau` → zigar
* **zil** `.il` `ko.il`
* zi~~mm~~ `z.i`
* **zip** `.ip` `ko.ip.ir`
  * *prinzip* `(.ip)` `(.ip).i-en` `(.ip).al` `(.ip).ieL` `gru.ip` `.im.ip`
    * **prinz** `pri`
  * → zept
* zipl `z.ipl`
  * *disziplin* `ds`
* zirk `z.ik` `z.ik.a` `z.ik.el` `z.ik.iO` `z.ik.ir` `z.ik.us` `be.z.ik`
  * → bezirk
* zirkonium __`z.ik.on.um`__
  * → zr
* *ziv****il*** `(.il)`
* zog ← zieh
* *zu* `Z` `Z.ei-ad-er` `.a-Z` `.a-z.ür` `(.e)Z` `(n)Z` `Z.ma`
  * *da*<i></i>*zu* `a-Z` `a-Z-Ür`
  * *nahezu* `(e)Z`
  * **z**um `z`
  * *zur* `zr*`
* zucht ← zieh
* zug ← zieh
* zünd `z.üd` `IN.it.iaL,z.üd-UNG`

### [ZR](#wörterbuch)

* zr `zr` ← zirkonium

### [ZW](#wörterbuch)

* **zwar** `zwa` `&:zwa`
* **zweif** `.eif` `.eif.el` `.eif.el.os` @zw
  * *zweifellos* `.eif-s`
* *zw****eig*** `(.eig)`
* -**zweck**- `zw` `zw.ek` `zw.ek,di.L` `be.zw` `Z.iS:zw`
* **zwing** `.ing` `be.ing`  `er.ing` @zw
  * **zwang** `ang` `ang,eu` `v.is.um.ang` @zw
  * **zwung** `.ung` `G.ung` `er.ung` `UN-G.ung` `UN-G.ung.H` @zw
* **zw**ischen `zw` `.a-zw` `(I)zw`
  * *dazwischen* `a-zw`

# Ausnahmen

* *aber.auch* `ber-aY`
* auch → aber.auch oder.auch
* acht → gutachten gutachter hochachtung
* *aerodynamisch* `.er.o-d.yn.i`
* *aeronautik* `.er.o-n.ik`
* *agentur* `.ag.ur`
* akt `.akt` `r.akt.or` ≠ rentner
  * *akti* `(.I)` `(.I).e` `(.I).en` __`(.I).on.är`__
    * *aktionär* `(.I).är`
      * *aktionär* `(.är)`
* *akzent* `.ak.z.eNT`
* *amerika* `.am.a` `I E.ig .at.en v .am.a`
* amt
  * *beamtenschaft* `be.FT`
* ander
  * *unter.anderem* `u.ad-er`
* *anfang* `.an.fa`
* *anfänger* `.an.f-er`
* -ANISCH `.an.isch`
  * *kubanisch* `k.ub.an.i` ≠ kubisch
  * *organisch* `.og.an.i`
  * *romanisch* `r.om.an.i`
* *arbeitsam* `T.Sa`
* *arbeitsfähig* `T.f.i`
* *arbeitsunfähigkeit* `(T),UN.T`
* *archivar* `.ach.ar`
* *architektur* `.ach.ur`
* *argument* `.ag.u-met`
* *arithmetik* `.ar.it.ik`
* *arterie* `.at.er.e`
* *asien* `.as.i-en`
  * *asiat* `.as.at` `.as.at.i`
* *atlantik* `.at.l-t.ik`
* *auffiel* `AU.f.il`
* aus
  * *auswärtiges.amt* `.S.a`
  * *darüberhinaus* `dar-Ü-aY`
  * *nach.außen.hin* `ach.auss:hn`
* ausdruck
  * *ausdruck* `.S-u`
    * *ausdrücklich* `.S-Y.L`
  * *zum.ausdruck.gebracht* `z.bra`
  * *zum.ausdruck.kommen* `z.ko`
* *autobus* `.aut.us`

* *bedeut* `be-eu-T`
* be*dräng* `be.dr`
* bedürf
  * *bedürfnis* `be-Ü.is` `be-Ü.iss.e`
  * *bedürftig* `be-Ü-t.ig`
* *befangen* `be.fa` ≠ begangen
* *beginn* `be.in`
* bei
  * *dabei*  `a-b.ei`
  * *herbei* `h.ei` `h.ei-Ür`
* *bibliographie* `b.ibl.o+gra.I`
* *bibliothekarisch* `(t.ek).i`
* biet
  * *hoheitsgebiet* `o-H.it`
  * *aufgabengebiet* `AU.en.it`
  * *auf.dem.gebiet* `AU.it`
  * *auf.diesem.gebiet* `AU-i.it`
* bild
  * *bilder* `bi.er` ≠ bildner
  * *bildner* `bi.n.er` ≠ bilder
* bind `b.id`
   * *bänder* `b.ed-er` ≠ aber
   * *bändig* `b.änd.i`
   * *verbände* `E.b.e`
* *biographie* `b.i-o+gra.I`
* *bitt* `.it` `.it.e` `.it.ed` `f.it.e` `UN.er.it.L`
* *blätter* `bl.et-er`
* bleib
  * *blieb* `bl.i`
* blick
  * *im.hinblick.auf* `.im:AU`
  * *augenblick* `au.ik` `au.ik.L`
* bracht
  * *beigebracht* `(b.ei)bra`
  * *bräch* `br` ≠ brächte
* brauch
  * *ver*<i></i>*br****au****ch* `E-au`
* brech
  * *ununterbrochen* `UN.UNTER`
* bring
  * *bring* `bri` __`(er)bri`__
    * *erbring* `er,bri`
* *brüderlich* `brÜ-L`
* buch
  * *geschäftsbücher* `GT-Ü-er`
  * *jahrbuch* `ja.b.uch`
* bürg
  * *bürgerliches.gesetzbuch* `bÜ.bu`
  * *mitbürger* `M-Ü-er`

* *dächte* `d.ächt` ≠ datei
* *dadurch* `a-dr` ≠ ander = __drang__
* *dagegen* `da.g` ≠ schlag
* *daraufhin* `dar-AU-n`
* *defin* `d.ef` `d.ef.ir` `d.ef.iv` `d.ef-o`
* *defizit* `d.e-it`
* *determinier* `d.e-t.em.ir`
* *diener* `di.er`
* *dienlich* `d.in.L` ≠ dienstlich
* *diktion* `d.ikt.iO` ≠ dition
* *dissonanz* `d.is-s.o`
* *dokumentarisch*  `d.ok.i`
* *dozent* `doz`
* drang `dr.ang` `dr.ang-ol` `dr.ang.s.al` ≠ dadurch
  * *dräng* `dr.eng` `dr.eng.el`
* *drucker* `dr.uk-er`
* *drück* `dr.ük`
* *du.erfährst.es* `dU er.f .e`
* *dutzend* `d.uz`

* *ebenfalls* `.e-fa-s`
* *efizient* `.e-f.iz.eNT`
* *eigentlich* `.ei-L`
* *eindrücke* `N-Ük.e`
* *empfohl* `.emp`
* *entwickel* `eNT-i` `T.er:eNT-i`
* *erklär* `er+r` ≠ __ernähren__
* *explosion* `.e-pl-o`

* *fabrikation* `fa.iO`
* *fächer* `f.ech-er` ≠ feder fehler
* *fähre* `f.är.e`
* *fahrer* `f.ar-er`
* *familie* `f.am`
* *finalistin* `fi.al.in`
* *finanzminister* `fi-er`
* *flamme* `fl.am.e`
* *flämmen* `fl.äm`
* front
  * *konfrontation* `ko.fr.ot.iO`
  * *konfrontier* `ko.fr.o`

* geb
  * geber `g.eb-er` `.an.g.eb-er` → rat
  * ~~geb~~
    * *ausschlaggebend* `.S.ag.ed`
    * *ergebnis* `er.is` `er.is.os`
  * *geb*
    * `g` `G.g` `G.g.H` `.ab.g` `be.g.H` `er.g` `er.g-st` `ach.g` `Ü.g`  `E.g-s` `E.g.L` `u-rÜ:g`
    * `-G.g.UNG` `ku:g`
    * __`n.S.ag.g`__
  * gab
    * -~~gab~~- `AU.e` `AU.it` `.S.e`
      * *aufgabengebiet* `AU.en.it` ≠ auf.dem.gebiet
* *gefängnis* `G.f.is`
* *gegner* `g.eg-er` `g.eg-er.in` `g.eg-er.i` `g.eg-er.FT`
* gemein
  * *allgemeingültig* `.a-G-Ü` @Aufstrichüberkreuzung
  * *verallgemeiner* `E.al.G`
* geo
  * *geograph* `g.e-o+gra`
  * *geometr* `g.e-m.etr` `g.e-m.etr.I` `g.e-m.etr.i`
* *gutachten* `u.a` = __unter.anderem__
* *gutachter* `gu,er`

* *handumdrehen* `ha.um`
* *heißblütig* `h.eiss-Ü.i`
* *hinein* `hi.N` ≠ nein
* hochachtung
  * *mit.vorzüglicher.hochachtung* `M-Ü.ho`

* *inflation* `IN.fla` `IN.fla.är` `IN.fla.iS.i`
* *interesse* `(I)t-er`
* *investment* `IN.w` = invest

* *koeffizient*  `ko.e-f.ent`
* *konfekt* `ko.f.ekt`
* *konsonant* `ko-s.aNT`
* *kontinent* `ko,ti`
* *kontingent* `ko,ti`
* *kontinuierlich* `ko,ti.L`
* krank
  * *krankenwagen* `kr.ak.wa` ≠ kraftwagen
  * *krankheit* `a-H` = schaffung
  * *kränk* `kr.ek`
    * *kränker* `kr.ek-er`
* *kultur* `u-ur` `u-ur.ell` = __struktur__ __strukturell__

* lat
  * relat*ion* `r.l.at.iO` ≠ _religion_ ← lat
* leg
  * *ver*leger `E.l.eg-er`
  * *angelen.sein.lass* `.an-G.ass`
  * *auswertige.angelegenheiten* `.S.H.en`
  * *bei.jeder.sich.bietenden.gelegenheit* `(b.ei)j.CH.H`
* lig
  * *relig* `r.i` `r.i-iO` `r.i.ös` `r.i.os.ät` = __relation__ ← lig
* *luxus* `l.ux`
* *luxuriös* `l.ux.ös`

* *nachdrücklich* `ach-Ü.L`
* nahr
  * *ernähr* `er.ä` ≠ __erklären__
* *nati****on*** __`(.on)`__ __`(.ön).ch.en`__ __`kut.ur+(.on)`__
  * *nat****ion*** `(.iO)` `(.iO).ch.en` `kut.ur,(.iO)` = __region__
    * *kulturnation* `u-ur.iO`

* *oder.auch* `.od-er-au` `.od-er-aY`

* *perfekt* `p.fet`
* *produkt* `(o)dut`
* *produktion* `o-u-O`
* *produktionsprozess* `(o)du.iO,(o).ess`

* rat
  * ***rat***geber `ra.g.eb-er`
  * *berat* `Ra` = __errat__
  * *er*rat `er-r.at` ≠ _beraten_
* re
  * *realisieren* `r.ir`
* reg
  * *regel* `r.el` `G.r.elt` `r.el.os` `r.el.i` __`r.el.i-T`__
    * *regelmäßigkeit* `r.el.ess.i-T`
  * reg*ion* `r.eg.iO` `r.eg.i-aL` ≠ religion
  * *regul* `r.ul` `r.ul.är` `r.ul.ir` `r.ul.iv` ≠ erklären ernähren relativ* sicht
  * *gesicht* `G.icht` ≠ gesetzwidrig*
*struktur* `str.uk` __`str.uk.ell`__ ≠ _kultur_ _kulturell_
  * *strukturell* `str.ell`
* *residenz* `r.es.ez`
* *rindfleisch* `ri.eisch`
* *rindvieh* `ri.ih` `(i),(i)`

* *simulation* `s.im.ul.iO`
* *simultan* `s.im.an`
* *sondern.auch* `S-au` `S-aY`

* tag
  * arbeits*tag* `.ab.T-tag`
  * *freitag* `.ei-tag`
* *tendenziös* `t.ez.ös`
* *thema* `t.a` 
* *tradition* `tr.ad-o` `tr.ad.i-eL`

* *umfang* `.um:fa` __`.um:fa.eich`__
* *umfangreich* `.um:fa.r.ei`
* *unbedingt* `UN-t` ≠ flüchtling
* *ungefähr* `UN.G`

* *verdoppel* `E.do`
* *verdräng* `E.dr`
* *vereinbar* `E.bar`
* *vereinig* `E.ig`
* *vervielfältigen* `E.il.f.i`
* *von.anfang.an* `v.an.an`

* wasser
  * *ge*<i></i>*wäss*er `G.wes-er` ≠ gewächs
  * wässr*ig* `w.ess.r.i`
* *weit.entgegenkommen* `w.T.g.ko`
* *wichtigtuerei* `wi.i,tu.er.ei`
* *wieder.einmal* `W.N`

* zert
  * **kon**zert `ko.zet`
* zug
  * ~~z~~**ü**~~g~~ `Ü` `gr-Ü.i` `ach-Ü.el.er` `O-Ü.L` → acht
* *zunächst.einmal* `-Z-N`

## TODOs

* treff (triff, troff)
* sprech (sprich, sproch, spruch)

* fin & finanz

* fügsam @`.üg-Sa` aktion

* **jed**e `j.e` (Eilschrift)
* *schönheit* `ö-H` (Verschattung, Redeschrift (?))
* ? (Verschattung + Red Herring, Praxis (?))
* Ausweichen in beide Richtungen: (erklären, ernähren), (seelisch, selig)

@-a → afrika amerika
@verkehrsschrift → alt arg visit vision
@-anisch → -ANISCH
@-ent → akzent
@r+r → erklär aber
@au → aber oder sondern urlaub
