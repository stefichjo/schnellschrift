 ...
 * akt → `.akt`
    ...
    * *akt* → `-t`
       * : *akt**ion* → `-tiO`
       * : *akt*ivier → `-tiv.ir`
       * : *in**akt*ivität → `IN-tiv.ät`
       * : re*akt**ion*är → `r.e-tiO.är`
       * : re*akt*or → `r.e-tor` ! `r.e-akt-or`
       * : **trans***akt**ion* → `tra-tiO`
    * ~~akt~~***i*** → `:I` E
 ...
 * : *zu***samm**en~~arb~~***eit*** → `Z-sa.T`
 * ~~arb~~***eit***s**los** → `;Ts.os`
    * ~~arbeits~~***los*** → `;l.os`
* arch → `.ach`
 * : architekt → `.ach.i-t.ekt` > `.ach.t.ekt` ~ `.ach.ekt`
 * : architektur → `.ach.i-t.ekt-UR` × `.ach.i-UR` > `.ach.UR`
 * : archiv → `.ach.iv`
...
* **buch** → `bu`
 * : **büch**er → `bÜ-er`
 * : *geschäft*s**büch**er → `GT.bÜ-er` * `GT-Ü-er`
* **bunt** → `but` E
* **burg** → `bu` ! `b.ug` (E)
 ...
 * : allerdi**ngs** → `;ngs`
 * : be***ding*** → `b.e-ing` ~ `;ng`
 * : *un*be***ding***t → `UN-b.e-ingt` × `UN-b.e-t` < `UN-t`
 * : *un*be***ding***t.***not*****we**~~nd~~**ig** → `UN-b.e-ingt .ot.w.i` ~ `UN-t o>w.i` ~ `UN-b.e-o-w.i`
* *dis*- → `ds` E
* *diplom* → `di.om`
 * *diplom*at → `di.om.at`
 * *diplom*atie → `di.om.I`
...
* **dürf** → `dÜ` E
 * : be**dürf** → `b.e-dÜ`
 * : be**dürf*****nis*** → `b.e-dÜ.is` * `b.e-Ü.is` ~ `b-Ü.is`
 ...
 * : *famili*är → `f.am.är` E
* ~~f~~***ang*** → `:ang` ! `f.ang`
 * : an~~f~~***ang*** → `.an.ang` ~ `.an.fa`
 * : be~~f~~***ang*** → `b.e-ang` ~ `b.e-fa`
 * : emp~~f~~***ang*** → `.emp.ang` * `.emp-a`
 * : ein~~f~~***ang*** → `N.ang`
 * : um~~f~~***ang*** → `.um<ang` ~ `.um.fa`
 * ~~f~~***äng*** → `:eng` ! `f.eng`
    * : an~~f~~***äng***er → `.an.eng-er` ~ `.an.f-er`
    * : emp~~f~~***äng*** → `.emp.eng` * `.emp`
    * : *ge*~~f~~***äng******nis*** → `G-eng.is` ~ `G-f.is`
    * : *ver*~~f~~***äng****lich* → `E.eng.L`
 * ~~f~~***ing*** → `I>;ng` ! `f.ing`
    * : emp~~f~~***ing*** → `.emp.ing` * `.emp-i`
* farm → `f.am` ! `f.arm` E
* ~~f~~***ass*** → `:ass` ! `fass`
 * um~~f~~***ass*** → `.um.ass`
* *fast* → `Fa`
 ...
 * **glänz** → `glä`
* ***glaub*** → `.aub` E
 * : *ich*.***glaub*** → `.I.aub` * `.I-au`
 * : *wir*.***glaub*** → `wr.aub` * `wr-au`
 * ***gläub*** → `.äub` E
* **gl**eich → `gl` E
* ***glied*** → `.id` ≠ ?
 * : *mit****glied*** → `M.id` * `M-i`
...
* ~~h~~***äng*** → `:eng` ! `h.eng`
 * : ab~~h~~***äng*****ig** → `ab.eng.i`
 * : an~~h~~***äng***er → `.an.eng-er`
 * : an~~h~~***äng*****ig** → `.an.eng.i`
 ...
 * ~~h~~***ang*** → `:ang` ! `h.ang`
    ...
    * : an~~h~~***ang*** → `.an.ang`
    * : *vor*~~h~~***ang*** → `O.ang`
    * : *zu***samm**en~~h~~***ang*** → `Z-sa.ang` * `u-ang`
* **haupt** → `hau` E
 * : **häupt** → `häu` ! `h.äupt` (E)
* ***haus*** → `.aus` E
 * ***häus*** → `.äus` E
...
* ***jug*** → `.ug`
 * : ***jug***end → `.ug.ed` » `.ug` ! `j.ug`
 * : ***jug***end*lich* → `.ug.ed.L` » `.ug.L`
* jug → `j.ug`
 * : **kon**jugat*ion* → `ko>j.ug.iO`
 * junkt → `j.ukt`
    * : **kon**junkt*ion* → `ko>j.ukt-iO`
    * : **kon**junktur → `ko>j.ukt-UR` * `ko,u-UR` E * `o-u-UR`
* (just) → `j.u+st`
 ...
 * : **bund**es*republik*.~~deut~~***sch***land → `bu<r.e-ik:sch.l.ad` × `bu:sch.l.ad` * `bu:sch-a`
 * länd → `l.ed`
 ...
 * : ***läng***er → `eng-er` ~ `NGR`
* ~~l~~***ass*** → `:ass` ! `l.ass`
 * : *aus*.an~~l~~***ass*** → `.S.an.ass`
    * *aus*.an~~lass~~ → `.S.an`
 * : *hinter*~~l~~***ass*** → `I>t-er.ass`
 * : *ver*an~~l~~***ass*** → `E.an.ass`
    * *ver*an~~lass~~ → `E.an`
 * ***läss*** → `ess` ! `l.ess`
    * : ***läss*****ig** → `ess.i` ! `l.ess.i`
    * : an***läss****lich* → `.an.ess.L` × `.an.L`
...
* *lini* → `;IN` ! `l.in` E
 * : *lini*e → `;IN.e` ! `l.in` E
 * : *in*.erster.*lini*e → `IN.est-IN.e` × `IN.IN.e`
* *list* → `l.i` ! `l.ist`
 * : *list*e → `l.i`
* **liter** → `li` ! `l.it-er`
...
* ~~masch~~***in*** → `I>;n`
 * : ~~masch~~***in***e → `I>;n.e` ! `m.asch.in.e` » `m.asch` E
 * : ~~masch~~***in***ell → `I>;n.ell` ! `m.asch.in.ell` » `m.asch.ell` E
* ***mass*** → `ass` E
 * : ***mass***e → `ass.e`
 ...
 * : *ge*~~m~~***ein*** → `G-N` E ~ `G-;N`
    ...
    * : im.*all**ge*~~m~~***ein***en → `.im.al.G-N` = `.im.al.G.N` * `.im.a-G.N` ~ `.im.a-G`
* **meist** → `m.ei`
* meld → `m.ed`
* ***meng*** → `eng`
* *mensch* → `NSCH`
...
* merk → `m.ek`
 * *auf*merk**sam** → `AU.m.ek-sa` × `AU-sa` ~ `AU.Sa` E
 ...
 * *minist* → `mi`
    ...
    * : *minist*erium → `mi-er.I.um` > `mi.um` E
       * *aussen***minist**erium → `.auss,mi-er.I.um` > `.auss,mi.um` × `.auss.um`
 ...
 * *miss* → `ms`
    ...
    * : *kom**miss**ion* → `ko-ms-o` * `ko-i.iO` » `ko-i`
 ...
 * : an*ge***nehm** → `.an.G-em`
    * an*ge*~~nehm~~ → `.an.G` E
 * : *unter*nehm*ung* → `UNTER.em-UNG` × `UNTER-UNG`
 * ~~nehm~~***e*** → `:e`
 * ~~nehme~~***n*** → `;n`
    * : ab~~nehme~~***n*** → `.ab;n`
    * : *auf*~~nehme~~***n*** → `AU;n`
    * : t**eil**~~nehme~~***n*** → `:eil;n`
    * : *über*~~nehme~~***n*** → `Ü;n` ! `Ü.em.en` × `Ü.en`
    * : *unter*~~nehme~~***n*** → `UNTER;n` ! `UNTER.em.en` × `UNTER.en`
 ...
 * ~~nehme~~***r*** → `;r`
    * : ~~arb~~***eit***~~nehme~~***r*** → `;T;r`
    * : ***teil***~~nehme~~***r*** → `:eil;r`
    * : *unter*~~nehme~~***r*** → `UNTER;r` ! `UNTER.em-er` × `UNTER-er`
 * ~~nehm~~***st*** → `;-st`
    * : *vor*~~nehm~~***st*** → `O;-st`
 * ~~nehm~~***ung*** → `UNG`
...
* **nutz** → `nu`
 * : **nütz** → `nÜ`
 * : eigen**nütz****ig** → `.eig.nÜ.i` * `.eig-Ü.i`
* oder → `.od-er`
* off → `.of`
 ...
 * ~~poli~~***tik*** → `;-tik` ~ `-tik` ! `p.ol.ik` (E)
    * : **fin**anz~~poli~~***tik*** → `fi.az-tik` » `fi-tik`
...
* **prüf** → `prÜ`
 * : ~~her~~***bst*****prüf***ung* → `;bst,prÜ.UNG` * `;bst-Ü.UNG`
 * : *über***prüf** → `Ü>prÜ` * `Ü-Ü`
* *publik* ?
 * *publikum* → `p.ub` E
 ...
 * : **reg**ier*ung* → `r.ir-UNG` E ~ `ir-UNG`
    * : **bund**es**reg**ier*ung* → `bu.r.ir-UNG` * `u-r.ir-UNG` ~ `u-r-UNG`
 * : **reg**ulier → `r.ul.ir`
 * : **reg**el → `r.l` ~ `:el`
    * : ***spiel*****reg**el → `.il.r.l` ~ `.il:l`
    * : *in*.~~der~~.**reg**el → `IN.r.l` × `IN.l`
 * : **reg***ion* → `r.iO` ! `r.eg.iO`
 * **rekt** → `r` ! `r.ekt`
    * : **rekt**or → `r.or`
    * : di**rekt** → `d.i-r` ~ `di.ekt` E ~ `di-t`
    * : di**rekt***ion* → `d.i-r.iO` ~ `di.ekt-iO` ~ `di-tiO` × `di-o` ! `di.iO` E 
 ...
 * : um~~f~~***ang*****reich** → `.um<ang.r.ei` × `.um.r.ei` ~ `.um<eich`
* reif → `r.eif`
 * -***reif*** → `.eif`
    * : **hoch*****schul******reif***e → `ho.ul.eif.e`
* reih → r.eih
 * **ein**e.~~reihe~~.**v**on → `.ei-v`
* reit → `r.T`
 * : bereit → `b.e-r.T`
    * *bereit* → `b.e-T`
       * *bereit*s → `b.e-Ts` « `Ts`
 ...
 * *korrespond* → `k.or-r`
    ...
    * : *korrespond*enz → `k.or-r.ez` E × `k.o-ez`
 ...
 * : **richt****ig** → `ri.i` E
 * : amts*ge****richt*** → `.amt,G,ri` * `.amt,G,i` < `.amt,i`
 * : *ge***richt** → `G,ri` * `G,i`
 * : *ge***richt***lich* → `G,ri.L` * `G,i-L` ~ `G-I>;L`
...
* ~~sch~~***ein*** → `;N`
 * : ~~sch~~***ein*****bar** → `;N.bar`
 * : an~~sch~~***ein***end → `.an.N.ed`
 * : be~~sch~~***ein***ig → `b.e-N.i`
 ...
 * : ***schul***e → `.ul.e`
 * : ***schül***er → `.ül-er`
 * : **hoch*****schul***e → `ho.ul.e` * `ho-u`
* **schutz** → `schu` = `.uz`
 * : umwel**t****schutz** → `.um-t,.uz` * `.um-t,u`
 * **schütz** → `schÜ`
* schwed → `schw.ed`
 ...
 * sah → `s.ah`
 * ~~s~~***icht*** → `I>;cht` ! `s.icht`
    * : ab~~s~~***icht*** → `.ab.icht` * `.ab-i`
    * : **an**~~s~~***icht*** → `.an.icht`
    * : *auf*~~s~~***icht*** → `AU.icht` * `AU-i`
    * : *ge*~~s~~***icht*** → `G-icht`
    * : *hin*~~s~~***icht*** → `hn.icht` * `hn-i`
    * : **rück**~~s~~***icht*** → `rÜ.icht` * `rÜ-i`
    * : *vor*~~s~~***icht*** → `O.icht` * `O-i`
    * : *zu**ver*~~s~~***icht*** → `Z-E.icht` * `Z-E-i`
    * *ge*~~sicht~~ → `G`
    * *hin*~~sicht~~ → `hn`
    * **rück**~~sicht~~ → `rÜ`
* **sehr** → `s` E
...
* siech → `s.ich`
 * **sucht** → su
    * : bleich**sucht** → `bl.eich,su` * `bl.eich,u`
    * **sücht** → `sÜ` = `s.ücht`
       * bleich**sücht****ig** → `bl.eich.sÜ.i` = `bl.eich.sücht.i` * `bl.eich-Ü.i`
       * eigen**sücht****ig** → `.eig.sÜ.i` = `.eig.sücht.i` * `.eig-Ü.i` ≠ eigen**nütz****ig** ! `.eig.ücht.i`
* *siedel* → `s.i-l`
* sind ← sein
* (sist) → `s.i+st`
 ...
 * : **f**ür*sorg* → `f-s.o` * `f,o`
 * : ***jug***end**f**ür*sorg* → `.ug.ed<f-s.o` * `.ug.ed.f,o` » `.ug.f,o` * `.ug-Ü.o`
 * : *ver**sorg* → `E-s.o`
 * : *ver**sorg*er → `E-s.o-er` * → `E,o-er`
 * : *vor**sorg*e → `O-s.o`
* sort → `s.ot`
 * : sorte → `s.ot.e` (E)
 * : sortier → `s.ot.ir` (E)
 ...
 * **sproch** → `.och` E
 * **spruch** → `.uch` E
    * : an**spruch** → `.an.uch` * `.an,u`
    * : ein**spruch** → `N.uch` * `N,u`
    * : *wider***spruch** → `W.uch` * `W-u`
 * **sprüch** → `.üch` E 
* st → `st`
 * : **kon**stant → `ko.st.aNT`
 * : **kon**stanz → `ko.st.az`
 ...
 * ***stat*** → `.at`
    ...
    * : ***stat***ist → `.at.ist`
       * ~~stat~~***ist*** → `I>;-st`
          * : ~~stat~~***ist***ik → `I>;-st-ik`
          * : ~~stat~~***ist***isch → `I>;-st-isch` ~ `sta.iS.i`
 ...
 * **st**it → `st`
    * : *in***st**itut → `IN.st.ut` E
    * : *in***st**itut*ion* → `IN.st*` ! `IN.st.ut.iO` × `IN.ut.iO`
 ...
 * *ständ* → `.end` E
    * *selbst**ständ***ig** → `-st-end.i` × `-st-ig` E
    * *selbst**ver**ständ***lich* → `-st-E.end.L` × `-st-L` E
 * stund → `st.ud` (E)
    * : stunde → `st.ud.e` * `u.e`
    * : stünd → `st.üd` (E)
 ...
 * -~~straß~~***e*** → `:e`
    * : **haupt**straß**e** → `hau:e`
    * : milchstraß**e** → `m.ich:e`
* streit (?)
 * stritt (?)
* stroph → `.str.of`
 * -**stroph**- → `.of`
    * : *kata***stroph**e → `k.at.of.e` × `k.at.e` E
    * : *kata***stroph**al → `k.at.of.al` × `k.at.al` E
* str**u** → `u`
 ...
 * **tens** → `t` E
    * *in***tens**iv → `IN.t.iv` E
    * *in***tens**ivier → `IN.t.iv.ir` E
    * *in***tens**ität → `IN.t.ät`
* ~~ter~~***min*** → `;m.in` ! `t.em.in` (E)
 * : ~~ter~~***min***al → `;m.in.al`
 * : de~~ter~~***min***ier → `d.e;m.in.ir` ! `d.e-t.em.ir`
* teuer → `t.eu-er` (E)
 * : teurer → `t.eur-er` (E) ?
* ***text*** → `ext` ≠ *hatt*
 * ***text***il → `ext-il` ~ `t.il` E
* theater → `t.e-at-er`
...
* **trau** → `trau`
 * : **trau**er → `trau-er`
 * : *miss***trau** → `ms.trau` * `ms-au`
 * : *ver***trau** → `E>trau` * `E-au`
* ***treff*** → `.ef` ≠ **träg** ! `tr.ef`
 * ***triff*** → `.if`
 * ***traf*** → `.af`
 * ***troff*** → `.of`
* **tret** → `tr.e` ! `tr.et`
 * **tritt** → `tri` = `tr.i` ≠ **trieb** ~ `.it`
 * **trat** → `tra`
 ...
 * unikum → `.un.ik.um` !ik! `un.um` ≠ *univers*um
 ...
 * **v**ent → `w` ! `w.et` E
    ...
    * : evtl. → `.e-v-t-l`
    * : *in****v***entar → `IN.w.ar` ! `IN.w.et.ar` (E) ~ `IN-tar`
    * : *in***v**entur → `IN.w-UR` ! `IN.w.et.UR` (E) ~ `IN-tUR`
 ...
 * *vol***u***t* → `u` ?
    ...
    * : **konter**re*vol***u***t*ion → `ko-ter,r.e-u-O`
* **v**on → `v` V
 * : **da****v**on → `da.v` * `a-v`
 * *vom* → `v°` V
* *vor* → `O` V
 ...
 * wurf → `w.uf` (E)
    * -**wurf**- → `wu`
       * : *ent***wurf** → `eNT.wu` * `eNT-u`
       * : *ent***würf**e → `eNT.wÜ.e` * `eNT-Ü.e`
       * : *vor***wurf** → `O.wu`
       * : *vor***würf**e → `O.wÜ.e`
 ...
 * **we**~~nd~~ → `w` = `w.ed` (E)
    * : an**we**~~nd~~ → `.an.w`
    * : *auf***we**~~nd~~ → `AU.w`
    * : ***not*****we**~~nd~~**ig** → `.ot.w.i` ~ `o>w.i`
    * : *ver***we**~~nd~~ → `E.w`
    * : *ver***we**~~nd~~*ung* → `E.w-UNG`
    * **wa**~~nd~~ → `wa` = `w.ad` (E)
       * : wandte → `wa-t.e` ! `w.adt.e` (E)
       * : be**wa**~~nd~~t**nis** → `b.e-wa-t-is`
       * : ein**wa**~~nd~~ → `N.wa`
       * : *ge***wa**~~nd~~t → `G-wa-t` » `G-wa`
       * : *ver***wa**~~nd~~t → `E.wa-t` » `E.wa`
* **wint** → `wit`
 * : **wint**er → `wit-er` (E)
* *wir* → `wr` V
* ~~wir~~***k*** → `;k` ! `w.ik` (E)
 ...
 * *will* → `:l` V
    * : *will*st → `:l` V
* wort → `wot` (E)
 * : *ant*wort → `aNT.wot` »× `aNT` E
 * : wörter → `wöt-er` (E)
 * : **bind**ewort → `bi.wot` * `bi-o`
 * : **haupt**wort → `hau.wot` * `hau-o`
 * : *macht*wort → `aCHT.wot` * `aCHT-o`
 * : ***schlag***wort → `.ag.wot` * `.ag-o`
 * : **sprich**w**o**rt → `.ich.wot` * `.ich-o`
 * : **sprich**w**ö**rt*lich* → `.ich.wöt.L` * `.ich-ö.L`
* wund ← wind
* **wund** → `wu`
 * : **wund**er → `wu-er` E
...
* **wunsch** → `wu` E
 * **wünsch** → `wÜ` E
    * : *die* besten **wünsch**e → `I b.est wÜ.e` * `I b.est-Ü.e`
* würd → `w.üd` (E)
 * : würde → `w.üd.e` (E)
* **zahl** → `za` E
 * : **zahl***ung* → `za*`
 ...
 * pr**o**zentual → `o>zet-al` × `o-al` E
* *zentr* → `zr` ! `z.etr` (E)
 * : *zentr*al → `zr.al` ! `z.etr.al` (E)
    * *zentral* → `zr-a`
 * : **kon***zentr*at*ion* → `ko>zr*` ! `ko>zr.iO` ! `ko.z.etr.iO` (E)
* **zer**- → `zer` V
* zern → `z.en`
 * : **kon**zern → `ko.z.en`
 * *zert* → `zer`
 ...
 * ***zug*** → `.ug`
    * : **gr**oß***züg*****ig** → `gr.üg.i` * `gr-Ü.i`
    * : *vor****züg****lich* → `O.üg.L` * `O-Ü.L`
* **ziem** → `z.i`
 * : **ziem***lich* → `z.i-L`
* **zil** → `.il`
 * : **kon****zil** → `ko.il`
...
* ***z****ipl* → `z`
 * *dis****z****ipl*in → `ds.z.in` × `ds.in` » `ds`
* zirk → `z.ik`
 * : zirka → `z.ik.a`
 * : zirkel → `z.ik.l`
 * : zirkus → `z.ik.us`
 * : bezirk → `b.e-z.ik` (E) ~ `b.e-ik`
 * *zirkul* → `z.ik`
    * : *zirkul*ier → `z.ik.ir`
