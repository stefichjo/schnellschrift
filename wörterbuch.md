...
* **graph** `gra` : `gra.ik` `gra.i` `gra.it` `f.on.gra.i` `t.e-gra`
  ...
  * : steno**graph** `st.en.gra` ! `st.en.o+gra`
    * ~~*steno*~~**graph** `(gra)`
  * *paragraph* `pa`
  * gramm `gr.am` : `gr.am.f` `f.on.gr.am` `(o)gr.am` `(o)gr.am.ir` `t.e-gr.am`
    * *grammophon* `gr.am.on`
...
* **scheid** `.eid` : `b.eid` `eNT.eid`
  * *entscheid* `eNT.ei`
  * **schied** `.id` ! `sch.id` : `.ab.id` `E.ab.id` `b.id` `eNT.id` `UNTER.id` `E.id` `(N),E.id.H` `M.al.eNT.id.H`
...
* **spez** `sp` : `sp.iaL` `sp.iaL.ir` `sp.iaL.ist-en` `sp.ieL` `sp.if-o` `sp.if.ir` `sp.if.ir-UNG` `sp.if.i` `sp.if.ät`
  * *spezif* `sp.ez` : `sp.ez.iO` `sp.ez.ir` `sp.ez.ir-UNG` `sp.ez.i` `sp.ez.ät`
...
* **teu**er `teu` : `b.teu` `E.teu` @-er
  * : **teu**rer `teu-er` ! `t.eur-er`
* ***text*** `ext`
  * *tex****til*** `(t.il)`
    * *textil* `t.il`
...
* **treff** `tr` : `tr.L` `(b)tr` `(Ü)tr` `UN-Ü.tr.L` `(O)tr.L` `Z.tr.ed`
  ...
  * **traf** `tra` : `(Ü)tra`
    * ! `tr.af` : `(b)tr.af`
    * **träf** `tr.e`/`trä` STS
      * : **träf**e `trä-e` ! `tr.äf.e` @ä @e
  * **troff** `.of` : `G.of` `.an.of` `b.of` `N.of` `Ü.of` `UN-Ü.of` @an-aus
* ***tret*** `.et` STS
...
* winn `w.in`
  ...
  * wann `w.an` : `G.w.an`
  * wonn `w.on` : `G.w.on`
* *winter* `wi-er` : `wi-er.L` `Ü,wi-er` @-er
* *wir* `wr`: `wr-I`
